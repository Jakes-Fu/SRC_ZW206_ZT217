...
LOCAL uint8 s_is_charg_first_in = 0; 
...
{
    4270,//4120,       //rechg_vol --
    4400,//4200,         //chg_end_vol--
    4500,//4330, --
    CHARGER_CURRENT_400MA,      //standard_chg_current
    CHARGER_CURRENT_400MA,      //usb_chg_current
...
#define LOW_TEMP 1000
#define HIGH_TEMP 1045
#define HOT_TEMP 1055
...
{
    4200,                  // hot_chg_end_vol;
    4100,                  // hot_rechg_vol;
    4200,                   // high_chg_end_vol;
    4100,                   // high_rechg_vol;
    4320,                   // standard_chg_end_vol;
    4270,                   // standard_rechg_vol;
    4320,                   // specific_chg_end_vol;
    4270,                   // specific_rechg_vol;
    4200,                   // low_chg_end_vol;
    4000,                   // low_rechg_vol;
    4200,                   // cold_chg_end_vol;
    4000,                   // cold_rechg_vol;
    CHARGER_CURRENT_300MA, //hot_chg_current;
    CHARGER_CURRENT_300MA, // high_chg_current;
    CHARGER_CURRENT_400MA, // standard_chg_current;
    CHARGER_CURRENT_400MA, // specific_chg_current;
...
PUBLIC uint32 CHGMNG_VoltagetoPercentumVBat (uint32 voltage)
{
    return _CHGMNG_VoltagetoPercentum(voltage,FALSE,FALSE);
}
/*****************************************************************************/
...
//        SCI_TRACE_LOW("_CHGMNG_VbatMonitorRoutine is shutdown");
    ...
    if(s_is_charg_first_in != 0)
    {
        if( module_state.charging_temperature >= HOT_TEMP) 
        {
            if(RESTART_BY_CHARGE == restart_condition)
            {
                if(module_state.charging_stop_reason != CHGMNG_HOTTEMP)
                {
                    _CHGMNG_FSMProcess(CHGMNG_FSM_EVENT_STOP_CHG, CHGMNG_HOTTEMP);
                }
                return;
            }
            else
        ...
        else if( module_state.charging_temperature >= HIGH_TEMP) 
        {
        ...
        else if(module_state.charging_temperature <=  LOW_TEMP) 
        {
            if(module_state.charging_stop_reason != CHGMNG_LOWTEMP)
            {
                _CHGMNG_FSMProcess(CHGMNG_FSM_EVENT_STOP_CHG, CHGMNG_LOWTEMP);
            }
            return;
        }
    }
...
{
    if (batChargingVol >= (chg_param.chg_end_vol + 50)) //old: 30
    {
        ovp_detect_bat_cnt ++;
    }
    else
...
LOCAL void _CHGMNG_ChargeMonitorRoutine(void)
{
     ...
     || (isBatOVP== TRUE))
    {
        if ((module_state.charging_stop_reason != CHGMNG_OVERVOLTAGE)
            ...
            &&(module_state.charging_stop_reason != CHGMNG_LOWTEMP))
        {
    ...
    {
        if (module_state.charging_stop_reason == CHGMNG_OVERTEMP || module_state.charging_stop_reason == CHGMNG_LOWTEMP || module_state.charging_stop_reason == CHGMNG_HOTTEMP)
        {
            if( module_state.charging_temperature >= HOT_TEMP)
            {
                 return;
            }
            if( module_state.charging_temperature >= HIGH_TEMP) 
            {
                 return;
            }
            if( module_state.charging_temperature <= LOW_TEMP) 
            {
                 return;
            }
    ...
    {
        CHG_PHY_SetChgCurrent (CHARGER_CURRENT_350MA);
        CHG_PHY_TurnOn();
        return ;
    }
...
//            SCI_TRACE_LOW("_CHGMNG_ChargeMonitorRoutine start------ctr0=0x%x", CHG_PHY_ChgCtl0());
    ...
    if (module_state.bat_statistic_vol >= (pse_param[CHGMNG_AdjustTempRegion()].chg_end_vol - 20)) // 4300 = 4320 - 20
    {
        _CHGMNG_FSMProcess(CHGMNG_FSM_EVENT_STOP_CHG, CHGMNG_CHARGEDONE);
        return;
    }
#else
    /* cv-status 在功耗高时不能截止充电，改成软件判断截止充电 */
    if (module_state.bat_statistic_vol >= (chg_param.chg_end_vol -100)) // 4300 = 4400-100
    {
        _CHGMNG_FSMProcess(CHGMNG_FSM_EVENT_STOP_CHG, CHGMNG_CHARGEDONE);
        return ;
    }
...
LOCAL void _CHGMNG_StopCharge (CHGMNG_STOPREASON_E reason)
{
    ...
    switch (reason)
    {
        ...
        case CHGMNG_OVERTEMP:
            _CHGMNG_SendMessagetoClient (CHR_CHARGE_OVERTEMP, 0);
            break;
            
        case CHGMNG_HOTTEMP:
            _CHGMNG_SendMessagetoClient (CHR_CHARGE_HOTTEMP, 0);
            break;
            
        case CHGMNG_LOWTEMP:
            _CHGMNG_SendMessagetoClient (CHR_CHARGE_LOWTEMP, 0);
            break;
...
LOCAL void _CHGMNG_SetChargeCurrent (CHGMNG_ADAPTER_TYPE_E mode)
{
    switch(mode)
    {
        ...
        default:
            CHG_PHY_SetChgCurrent (CHARGER_CURRENT_400MA);
    }
}
/*****************************************************************************/
...
PUBLIC uint32  CHGMNG_Init (void)
{
    ...
    _CHGMNG_TempQueueInit(_CHGMNG_GetCurrentTemp());
#endif
    CHG_PHY_SetChgCurrent(CHARGER_CURRENT_400MA);   // Set default current
...
uint32 g_zdt_temp_adc_val = 0;
uint32 g_zdt_temp_val = 0;
int32 g_zdt_temp_du = 0;
...
PUBLIC int32 CHGMNG_GetVbatTempVolValue (void)
{
    ...
    SCI_TRACE_LOW("CHGMNG_GetVbatTempVolValue licaho");
#endif
