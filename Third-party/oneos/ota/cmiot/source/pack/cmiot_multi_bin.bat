...
call:findstr1 CMIOT_FOTA_SERVICE_PRODUCT_ID %def_path% cms_id
...
for /f tokens^=^1^,^2^,^3^ delims^=^"^  %%i in ('findstr  "CMIOT_FOTA_FILENAME" %def_path%') do (
    ...
    set flag=0
    for /f "tokens=2" %%m in ('findstr "CMIOT_FOTA_USE_FULL!idx!" %def_path%') do (
        set tmp=%%m
        set newidx=!tmp:CMIOT_FOTA_USE_FULL=!
        if !idx!==!newidx! (
            set flag=1
        )
    )
