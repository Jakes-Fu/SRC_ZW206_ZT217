...
// some platform use macros to trace memory allocation, calling duer_xxx() here make it useless

#include "os_api.h"
#define DUER_MALLOC(_s)          SCI_ALLOCA(_s)
#define DUER_CALLOC(_s, _n)      SCI_ALLOCAZ((_s) * (_n))
#define DUER_REALLOC(_p, _s)     SCI_REALLOC(_p, _s)
#define DUER_FREE(_p)            SCI_FREE(_p)
...
typedef void* (*duer_calloc_f)(duer_context context, duer_size_t nmemb, duer_size_t size);
 ...
 * @Param f_callc, duer_calloc_f, the memory calloc function for user
 *
...
#ifdef DUER_MEMORY_USE_CALLOC
                                  duer_free_f f_free,
                                  duer_calloc_f f_calloc);
#else
                                  duer_free_f f_free);
#endif // DUER_MEMORY_USE_CALLOC
 ...
 *
 * @Param size, duer_size_t, the expected size of the memory
 * @Param nmemb, duer_size_t, number of memory blocks
 * @Param module, duer_memory_hm_module_e, the tag for where the memroy used
 * @Return the alloced memory pointer
 */
DUER_INT void* duer_calloc_hm(duer_size_t size, duer_size_t nmemb, duer_memory_hm_module_e module);
#else
/*
 * cmalloc function zero-initialize the memory before return
 *
 ...
 * @Return the alloced memory pointer
 */
DUER_INT void* duer_calloc_ext(duer_size_t size, duer_size_t nmemb, const char* file, duer_u32_t line);
#else
/*
 * malloc function, zero-initialize before return
 *
 ...
 *
 * @Param nmemb, duer_size_t, number of memory blocks
 * @Param size, duer_size_t, the expected size of a block
 * @Return the alloced memory pointer
 */
DUER_INT void* duer_calloc(duer_size_t nmemb, duer_size_t size);
#else
/*
 * cmalloc function zero-initialize the memory before return
 *
