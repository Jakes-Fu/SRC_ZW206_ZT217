                            ...
                            size_t data_len)
{
    ...
    if (isInstance == true)
    {
        LOG("hdrType = _PRV_TLV_TYPE_RESOURCE_INSTANCE ");
        hdrType = _PRV_TLV_TYPE_RESOURCE_INSTANCE;
    }
    else
    {
        LOG("hdrType =  prv_getHeaderType(type); ");
        hdrType = prv_getHeaderType(type);
        LOG_ARG("hdrType :0x%x",hdrType);
    }
              ...
              lwm2m_data_t ** dataP)
{
    ...
    *dataP = NULL;
    ...
    while (0 != (result = lwm2m_decode_TLV((uint8_t*)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        ...
        LOG_ARG(" debug,type: %s  ,id  : %d,dataIndex %d,,dataLen %d ",type,id,dataIndex,dataLen);
        ...
        *dataP = newTlvP;
        LOG_ARG(" debug,type: %s  ,id  : %d ",type,id);
        ...
        LOG_ARG(" debug size %d ,index %d ,result %d",size,index,result);
        size++;
        index += result;
    }
                  ...
                  uint8_t ** bufferP)
{
    ...
    *bufferP = (uint8_t *)lwm2m_malloc(length);
    ...
    for (i = 0 ; i < size && length != 0 ; i++)
    {
        ...
        LOG_ARG("isInstance: %d , dataP[i].type :%d",isInstance,dataP[i].type);
        switch (dataP[i].type)
        {
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            isInstance = true;
