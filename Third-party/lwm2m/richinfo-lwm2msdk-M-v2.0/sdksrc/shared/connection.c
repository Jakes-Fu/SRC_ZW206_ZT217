...
{
#ifdef LWM2M_SUPPORT_4//add by ak.wang
    int s = -1;
    struct sci_sockaddr addr;
    int ret = 0;

    uint16 port = atoi(portStr);
    s = dm_socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (s >= 0)
    {
        addr.family = AF_INET;
        addr.port = htons(atoi(portStr));
        addr.ip_addr = sci_gethostaddress_ext(socket_getNetId());
        if (-1 == dm_bind(s, &addr, sizeof(struct sci_sockaddr)))
        {
            dm_close(s);
            s = -1;
        }
...
#else
        temp_addr = &(connP->addr);
        LOG_M("addr=0x%x ,addr->sa_family :0x%x ,addr->sa_data :%s[]\n", addr,addr->sa_family,addr->sa_data);
                                 ...
                                 int addressFamily)
{
    ...
    for(p = servinfo ; p != NULL && s == -1 ; p = p->ai_next)
    {
        ...
        if (s >= 0)
        {
            sa = p->ai_addr;
            sl = p->ai_addrlen;
             LOG_M("host=%s, port=%s, ret=%d, addressFamily=%d\n", host, port, ret, addressFamily);
                    ...
                    size_t length)
{
    ...
    s[0] = 0;
#ifdef LWM2M_SUPPORT_4
    if (AF_INET == connP->addr.sa_family)
    {
        struct sockaddr_in *saddr = (struct sockaddr_in *)&connP->addr;
        inet_ntop(saddr->sin_family, &saddr->sin_addr, s, INET6_ADDRSTRLEN);
        port = saddr->sin_port;
    }
    else if (AF_INET6 == connP->addr.sa_family)
    {
