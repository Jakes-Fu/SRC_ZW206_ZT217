...
#define SIGPIPE (1)
#ifdef LWM2M_SUPPORT_4
#define LWM2M_THREAD_PRIORITY           (221) // priority should be lower than DAPS(current is 67)
#else
#define LWM2M_THREAD_PRIORITY           (68) // priority should be lower than DAPS(current is 220)
#endif
...
Options_V4 g_opt_v4 = {NULL};

extern Options_V4 opt_v4 ;
#endif
...
#ifdef LWM2M_SUPPORT_4
    if(strlen(g_opt_v4.szPwd)>0)
    {
        passwd = g_opt_v4.szPwd;
        LOG_M(" v4 use pwd is:%s[]\n", passwd);
    }
#else
...
#ifdef LWM2M_SUPPORT_4
PUBLIC int strfindchar(char * input,char synbol)
{
    int num = 0;
    int i = 0;

    LOG_M("input=%s , char=%c",input, synbol);
...
{ 
    ...
    *outbuff = NULL;
    ...
    do{
        ...
        memcpy(*outbuff,szvalbuf,strlen(szvalbuf));
        LOG_M("val=%s,outbuff=%s\n",szvalbuf,*outbuff);
#endif
...
#ifdef  LWM2M_SUPPORT_4
    if (strlen(g_opt_v4.szSrvIP)>=0)
        {
            server=g_opt_v4.szSrvIP;
        }
    
        if (g_opt_v4.nSrvPort > 80 ) 
        {
            sprintf(szsrvport,"%d",g_opt_v4.nSrvPort);
            serverPort = szsrvport;     
        }
    
        if(g_opt_v4.nAddressFamily==4) 
        {
            data.addressFamily = AF_INET;
        }
    
        if ( g_opt_v4.nLifetime > 10 ) 
        {
            lifetime=g_opt_v4.nLifetime;
        }
    
        //if (g_opt.nLocalPort > 49152 ) 
        {
            sprintf(szlocalport,"%d",g_opt_v4.nLocalPort);
            localPort = szlocalport;        
        }
    
        if(strlen(g_opt_v4.szCMEI_IMEI)>0)
        {    
            strcpy(imeiFlag,g_opt_v4.szCMEI_IMEI);
        }
        else
        {    
            strcpy(imeiFlag,g_opt_v4.szCMEI_IMEI2);
        }
        LOG_M("IMEI:%s", imeiFlag);
        //brand||model||appKey||sdkVersion||apiVersion||apiType||templateId||imei1||imei2||***||***||***
        memset(epnametmp,0,sizeof(epnametmp));
        sprintf(epnametmp,"%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s",
        strlen(g_opt_v4.brand)>0?g_opt_v4.brand:szStars_v4,
        strlen(g_opt_v4.model)>0?g_opt_v4.model:szStars_v4,
        strlen(g_opt_v4.szAppKey)>0?g_opt_v4.szAppKey:szStars_v4,
        strlen(g_opt_v4.sdkVersion)>0?g_opt_v4.sdkVersion:szStars_v4,
        strlen(g_opt_v4.apiVersion)>0?g_opt_v4.apiVersion:szStars_v4,
        strlen(g_opt_v4.apiType)>0?g_opt_v4.apiType:szStars_v4,
        strlen(g_opt_v4.templateId)>0?g_opt_v4.templateId:szStars_v4,
        strlen(g_opt_v4.szCMEI_IMEI)>0?g_opt_v4.szCMEI_IMEI:szStars_v4,
        strlen(g_opt_v4.szCMEI_IMEI2)>0?g_opt_v4.szCMEI_IMEI2:szStars_v4,
        szStars_v4,
        szStars_v4,
        szStars_v4
        );
...
#ifdef LWM2M_SUPPORT_4
    if(strstr(server,"coap://") == NULL && strfindchar(server,':') ==0)
    {
        //server is only host ,no port
        sprintf (serverUri, "coap://%s:%s", server, serverPort);
        LOG_M("servierUri is %s\r\n",serverUri);
    }
    else if(strstr(server,"coap://") == NULL && strfindchar(server,':') ==1)
    {
        //server is only host ,with  port
        sprintf (serverUri, "coap://%s", server);
        LOG_M("servierUri is %s\r\n",serverUri);
    }
    else if(strstr(server,"coap://") != NULL && strfindchar(server,':') ==1)
    {
        //server is coap:// host ,no  port
        sprintf (serverUri, "%s:%s", server,serverPort);
        LOG_M("servierUri is %s\r\n",serverUri);
    }
    else if(strstr(server,"coap://") != NULL && strfindchar(server,':') ==2)
    {
        //server is coap:// host ,with  port
        sprintf (serverUri, "%s", server);
        LOG_M("servierUri is %s\r\n",serverUri);
    }
#else
    sprintf (serverUri, "coap://%s:%s", server, serverPort);
...
#ifdef LWM2M_SUPPORT_4
    objArray[3] = get_object_dm_write();
    if (NULL == objArray[3])
    {
        LOG_S("Failed to create dm_write object\r\n");
        return -1;
    }

    objArray[4] = get_object_dm_read();
    if (NULL == objArray[4])
    {
        LOG_S("Failed to create dm_read object\r\n");
        return -1;
    }
#else
...
#ifdef LWM2M_SUPPORT_4
    lwm2mH->bUseSDKRegUpdate = g_opt_v4.bUseSDKRegUpdate;
#else
            ...
            struct sockaddr_storage addr;
                ...
                */
                ...
                while ((numBytes = dm_recvfrom(data.sock, buffer, MAX_PACKET_SIZE, 0, (struct sockaddr *)&addr, &addrLen)) > 0)
                {
                    ...
                    LOG_M("Data recv addrLen=%d\n", addrLen);
#ifdef LWM2M_SUPPORT_4
                    connP = connection_find(data.connList, (struct sockaddr *)&addr, addrLen);
#else
...
#ifdef LWM2M_SUPPORT_4
    //other valid check
    if(strcmp(g_opt.szSrvIP,"shipei.fxltsbl.com")==0 || g_opt.nSrvPort == 39977)
    {
        if(g_opt.nLifetime<120) g_opt.nLifetime=120;
    }
    if(strcmp(g_opt.szSrvIP,"m.fxltsbl.com")==0 || g_opt.nSrvPort == 39979)
    {
...
int  LWM2M_SDK_INIT_V4(Options_V4* options,OptFuncs *optfuncs)
{
    ...
    strcpy(g_opt_v4.brand,options->brand);
    strcpy(g_opt_v4.model,options->model);
    strcpy(g_opt_v4.sdkVersion,options->sdkVersion);	
    strcpy(g_opt_v4.apiVersion,options->apiVersion);
    strcpy(g_opt_v4.apiType,options->apiType);
    strcpy(g_opt_v4.templateId,options->templateId);

    strcpy(g_opt_v4.szCMEI_IMEI,options->szCMEI_IMEI);
    strcpy(g_opt_v4.szCMEI_IMEI2,options->szCMEI_IMEI2);
    strcpy(g_opt_v4.szIMSI,options->szIMSI);	
    strcpy(g_opt_v4.szDMv,options->szDMv);
    strcpy(g_opt_v4.szAppKey,options->szAppKey);
    strcpy(g_opt_v4.szPwd,options->szPwd);

    strcpy(g_opt_v4.szSrvIP,options->szSrvIP);
    g_opt_v4.nAddressFamily = options->nAddressFamily;
    g_opt_v4.nSrvPort = options->nSrvPort;
    g_opt_v4.nLifetime = options->nLifetime;
    g_opt_v4.nBootstrap = options->nBootstrap;	
    g_opt_v4.nLocalPort = options->nLocalPort;
#if 1 //modify by unisoc
    g_opt_v4.bUseSDKRegUpdate = options->bUseSDKRegUpdate;
#endif
    g_sdk_opt_funcs = *optfuncs;
...
{
    int i =0;
    if(g_ruleConfig == NULL)
    {
        LOG_S("input error");
        return 0;
    }
    for(i = 0 ; i< RULECONFIG_NUM;i++)
    {
        if(target_value == g_ruleConfig[i].resid && g_ruleConfig[i].value != 0)
        {
            LOG_M("target config [%d] is : %s ,value is :%d",i, g_ruleConfig[i].rulename,g_ruleConfig[i].value);
            return g_ruleConfig[i].value;
        }
    }
    LOG_M("not found target value :%d",target_value);
    return 0;
}

extern struct addressConfig g_addressConfig;
PUBLIC struct addressConfig * LWM2M_SDK_UPDATE_AddressConfig(void)
{
    int i =0;
    if(&g_addressConfig == NULL)
    {
        LOG_S("input error");
        return NULL;
    }
    for(i = 0 ; i< 1;i++)
    {
        if(NULL != g_addressConfig.value )
        {
            LOG_M("target g_addressConfig value is :%s\n", g_addressConfig.value);
            {
                char  * str  =NULL;
                str = strrchr(g_addressConfig.value,':');
                if(str != NULL)
                {
                    LOG_M("str is :%s\n", str);
                    g_addressConfig.port = atoi(str+1);
                    LOG_M("g_addressConfig.port is :%d\n", g_addressConfig.port);
                }
            }
            return &g_addressConfig;
        }
    }
    LOG_S("not found addressConfig : NULL\n");
    return NULL;
}
#endif
LOCAL void LWM2M_CreateTimer(int msec)
