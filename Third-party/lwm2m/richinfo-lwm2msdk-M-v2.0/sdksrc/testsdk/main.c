...
PUBLIC void  myNotifyMsg(OptNotifyParam *optNotifyParam)
{
...
PUBLIC int myDMReadInfo(int resId,char **outbuff)
{
    int iret = 0;
    char buff[512];
    ...
    *outbuff = NULL;
    ...
    case 6614://Location
        ...
        break;
    case 6615://IMSI
        snprintf(buff,buflen,"460022225441927");
        break;
    case 6616://sn
        snprintf(buff,buflen,"vivop11");
        break;
    case 6617://routerMac
        snprintf(buff,buflen,"AA:BB:CC:DD:EE:FF");
        break;
    case 6618://bluetoothMac
        snprintf(buff,buflen,"GG:HH:II:JJ:KK:MM");
        break;
    case 6619://gpu
        //snprintf(buff,buflen,"MT6580M");
        snprintf(buff,buflen,"***");
        break;
    case 6620://board
        snprintf(buff,buflen,"VIVO_BOARD_1");
        break;
    case 6621://resolution
        snprintf(buff,buflen,"1280x720");
        break;
#else
    case 6614://Location
...
#endif
{
    ...
    Options opt = 
    {
    .szCMEI_IMEI="861111003929109", //CMEI/IMEI  ;和IMEI2选一个填，参考规范文档;设备没有的话向移动申请；
    .szCMEI_IMEI2="",                 //CMEI/IMEI2 ;和IMEI选一个填，参考规范文档
    .szIMSI="460041850403690",  //IMSI;没有可设为空
    .szDMv="v2.0",             //DM版本号;固定v2.0
    .szAppKey="M100001047",       //appkey;  需申请
    .szPwd="04145Mk2u4KxlZ4qIADB9l0iR3118VLJ",//pwd秘钥; 需申请
	...
	else
	{
            ...
            while(s_quit==0)
            {
                ...
                if(s_trytimes<0) sleeptime=10000;
#ifdef LWM2M_SUPPORT_4
                printf("[lwm2m]to run in created thread, s_trytimes=%d\n", s_trytimes);
                ...
                while (s_quit == 0)
                {
                    ...
                    {
                        ret = LWM2M_SDK_STOP();
                        printf("[lwm2m]state error, state=%d, sleep=%d, s_trytimes=%d\n", s_state, sleeptime, s_trytimes);
                        SCI_Sleep(sleeptime * 1000);
                        s_trytimes--;
                        s_state = 0;
                        break;
