...
// use next three interface
//#define BF20A6_OUTPUT_MODE_CCIR656_1BIT
#define BF20A6_OUTPUT_MODE_CCIR656_2BIT
//#define BF20A6_OUTPUT_MODE_CCIR656_4BIT

//#define BF20A6_LOAD_FROM_T_FLASH
extern uint8 sjtc_camera_pix8;
...
LOCAL uint32 BF20A6_AE_Enable(uint32 ae_enable);
 ...
 **					Local Variables								 *
 **---------------------------------------------------------------------------*/
#ifdef BF20A6_LOAD_FROM_T_FLASH
#include "sfs.h"
#include "ffs.h"
...
__align(4) const SENSOR_REG_T BF20A6_YUV_640X480[] =
{
	...
	{0xe6,0x24},
	{0xe7,0x64},
	{0xe8,0xf6},//VDDIO=1.8v 0xe8[2]=1,//VDDIO=2.8v 0xe8[2]=0  //0xf2
	{0x4a,0x00},
	{0x00,0x03},
	{0x1f,0x02},
	{0x22,0x02},
...
if(enable)
{
	BF20A6_WriteReg( 0xa0 , 0x08 );
	BF20A6_WriteReg( 0x5f , 0x19 );
	BF20A6_WriteReg( 0x23 , 0x33 );
	BF20A6_WriteReg( 0x01 , 0x13 ); ///B GAIN
	BF20A6_WriteReg( 0x02 , 0x1e ); ///R GAIN
}
else
{
	BF20A6_WriteReg( 0xa0 , 0x09 );
	BF20A6_WriteReg( 0x5f , 0x29 );
	BF20A6_WriteReg( 0x23 , 0x33 );
	BF20A6_WriteReg( 0x01 , 0x19 );
	BF20A6_WriteReg( 0x02 , 0x15 );
}
...
//>>>>>>> cd10b7b105ff268fd1ef9c3e6dd1df7d80bebeb1
...
LOCAL void BF20A6_WriteReg( uint8  subaddr, uint8 data )
{
	...
	//SCI_TRACE_LOW("SENSOR: BF20A6_WriteReg reg/value(%x,%x) !!", subaddr, data);
...
LOCAL uint8 BF20A6_ReadReg( uint8  subaddr)
{
	...
	//SCI_TRACE_LOW("SENSOR: BF20A6_ReadReg reg/value(%x,%x) !!", subaddr, value);
...
LOCAL uint32 BF20A6_Power_On(uint32 power_on)
{
	...
	if(SCI_TRUE==power_on)
	{
		...
		{
			BF20A6_Delayms(2);
			GPIO_SetFrontSensorPwdn(power_down);//GPIO_SetFrontSensorPwdn(!power_down);
			BF20A6_Delayms(2);
			GPIO_SetSensorPwdn(!power_down);//GPIO_SetSensorPwdn(power_down);
			BF20A6_Delayms(2);
			//GPIO_SetSensorPwdn(power_down);
		      // BF20A6_Delayms(2);
			//GPIO_SetSensorPwdn(!power_down);
	...
	SCI_TRACE_LOW("set_BF20A6_Power_On");
	return SCI_SUCCESS;
}
...
LOCAL uint32 BF20A6_Identify(uint32 param)
{
#define BF20A6_PID_VALUE    0xfc
#define BF20A6_PID_ADDR     0x20
#define BF20A6_VER_VALUE    0xfd
#define BF20A6_VER_ADDR     0xa6
	...
	BF20A6_Power_On(TRUE);

	SCI_TRACE_LOW("BF20A6_Identify");
//	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_BF20A6_725_112_2_18_0_30_46_781,(uint8*)"");
	for(i = 0; i<2; )
	{
		nLoop = 1000;
		...
		if( ret != value[i])
		{
			...
			if(err_cnt>3)
			{
				SCI_TRACE_LOW("It is not BF20A6");
			//	SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_BF20A6_735_112_2_18_0_30_46_782,(uint8*)"");
				return SCI_ERROR;
			}
			else
			{
	...
	//SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_BF20A6_753_112_2_18_0_30_46_783,(uint8*)"");
    sjtc_camera_pix8 = DCAMERA_CFG_30W_240X320;
...
LOCAL uint32 Set_BF20A6_Brightness(uint32 level)
{
    ...
    SCI_TRACE_LOW("set_BF20A6_brightness: level = %d", level);
...
LOCAL uint32 Set_BF20A6_Contrast(uint32 level)
{
    ...
    SCI_TRACE_LOW("set_BF20A6_contrast: level = %d", level);

    return 0;
}


__align(4) const SENSOR_REG_T BF20A6_image_effect_tab[][6]=
{
		// effect normal
			{
				{0xd5,0x20},
...
#if 1
{
    ...
    SCI_TRACE_LOW("set_BF20A6_image_effect: effect_type = %d", effect_type);
...
LOCAL uint32 Set_BF20A6_Ev(uint32 level)
{
    ...
    SCI_TRACE_LOW("set_BF20A6_ev: level = %d", level);
...
LOCAL uint32 Set_BF20A6_Anti_Flicker(uint32 mode)
{
	uint8 reg_val=BF20A6_ReadReg(0x13);
    ...
    SCI_TRACE_LOW("set_BF20A6_anti_flicker-mode=%d",mode);
    OS_TickDelay(100);
...
LOCAL uint32 Set_BF20A6_Preview_Mode(uint32 preview_mode)
{
	...
	SCI_TRACE_LOW("set_BF20A6_preview_mode: level = %d", preview_mode);
	return 0;
}
...
LOCAL uint32 Set_BF20A6_AWB(uint32 mode)
{
	...
	SCI_TRACE_LOW("set_BF20A6_awb_mode: mode = %d", mode);
...
{
	uint8 reg_val = 0;
	reg_val = BF20A6_ReadReg(0x13);

	if(0x01==ae_enable)
	{
		BF20A6_WriteReg(0x13, reg_val|0x01);
	}
  ...
  SCI_TRACE_LOW("SENSOR: BF20A6_AE_Enable: ae_enable = %d", ae_enable);
	return 0;
}
...
LOCAL uint32 BF20A6_After_Snapshot(uint32 para)
{
	...
	SCI_TRACE_LOW("SENSOR_BF20A6: After Snapshot");
