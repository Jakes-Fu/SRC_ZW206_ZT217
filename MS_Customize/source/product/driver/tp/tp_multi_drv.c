...
LOCAL SCI_TIMER_PTR      s_tp_check_timer = NULL;
LOCAL TPDSVR_SIG_T tpc_backup_data;
...
LOCAL TPD_TO_TPSVR_MSG_T* TPC_Alloc_SignalBuffer()
{
...
void HandleTPTimeOutMessage(void)
{
    //ZDT_LogTrace("TP: HandleTPTimeOutMessage SignalCode=0x%x, x=%d,y=%d",tpc_backup_data.SignalCode,tpc_backup_data.x_key,tpc_backup_data.y_key);
    _TPM_Set_LastStatus(TP_UP_MSG);
    tpc_backup_data.SignalCode = TP_UP_MSG;
    tpc_backup_data.gesture_type = 398;
    SCI_SendEventToClient(TP_SERVICE, TP_UP_MSG, (void *)&tpc_backup_data);
    SCI_TPC_EnableDeepSleep (ENABLE_APB_SLEEP);
    return;
}

LOCAL void TPC_StatusCheckReset()
{    
    TPD_TO_TPSVR_MSG_T *tp_to_tpsvr_sig_ptr    =  SCI_NULL;

    tp_to_tpsvr_sig_ptr = TPC_Alloc_SignalBuffer();
    	
    tp_to_tpsvr_sig_ptr->SignalCode = TP_ENTER_INTO_TIMER_HANDLER;
    tp_to_tpsvr_sig_ptr->tpd_msg_valu = TP_ENTER_INTO_TIMER_HANDLER;

    SCI_SendSignal((xSignalHeader)tp_to_tpsvr_sig_ptr, P_TPP);
}

LOCAL void TPC_StatusCheckTimer_Out(uint32 param)
{	    
    TPC_StatusCheckReset();
    if(s_tp_check_timer != NULL)
    {
        SCI_DeactiveTimer(s_tp_check_timer);
    }
    return ;
}

void TPC_StatusCheckTimer_Start()
{
    if(s_tp_check_timer == NULL)
    {
        s_tp_check_timer = SCI_CreateTimer("TPCheck_Timer", TPC_StatusCheckTimer_Out,
                                    1, TIMER_PERIOD_TPCHECK, SCI_AUTO_ACTIVATE);
    }
    else
    {
        SCI_ChangeTimer(s_tp_check_timer, TPC_StatusCheckTimer_Out, TIMER_PERIOD_TPCHECK);
        SCI_ActiveTimer(s_tp_check_timer);	
    }
}

void TPC_StatusCheckTimer_Stop()
{
    if(s_tp_check_timer != NULL)
    {
        if(SCI_IsTimerActive(s_tp_check_timer))
        {
            SCI_DeactiveTimer(s_tp_check_timer); 
        }
        SCI_DeleteTimer(s_tp_check_timer);
        s_tp_check_timer = NULL;
    }
}
...
void HandleTPMessage(void)
{
...
#if 1
    if((tpc_total_data.SignalCode == TP_DOWN_MSG) && (_TPM_Get_LastStatus() != TP_UP_MSG))
    {
        tpc_backup_data.SignalCode = TP_UP_MSG;
        _TPM_Set_LastStatus(TP_UP_MSG);
        SCI_SendEventToClient(TP_SERVICE, TP_UP_MSG, (void *)&tpc_backup_data);	
	...
	if (app_queue_avilable > 12)
	{
		...
		else if(((_TPM_Get_LastStatus() != TP_UP_MSG) && (tpc_total_data.SignalCode == TP_UP_MSG)) || (tpc_total_data.SignalCode != TP_UP_MSG))//down,up msg
		{
              ...
              else if(tpc_total_data.SignalCode == TP_UP_MSG)
              {
                    TPC_StatusCheckTimer_Stop();
              }
		    _TPM_Set_LastStatus(tpc_total_data.SignalCode);
			SCI_SendEventToClient(TP_SERVICE, tpc_total_data.SignalCode, (void *)&tpc_total_data);	
		}
	...
	if(tpc_total_data.SignalCode == TP_UP_MSG)
	{
      ...
      tpc_backup_data = tpc_total_data;
	GPIO_TPIRQCtrl(SCI_TRUE);
}
