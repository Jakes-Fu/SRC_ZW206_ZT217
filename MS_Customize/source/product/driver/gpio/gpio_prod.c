...
#if defined(ZDT_GSENSOR_SUPPORT)//def ZDT_PCBA_ZW31_SUPPORT // wuxx add form ZW95 ZW05 for W217 ZW206
PUBLIC void GPIO_ZDT_GSensorIntHandler (uint32 gpio_id, uint32 gpio_state)
{
    ...
    {
        is_high = TRUE;
        GPIO_ClearIntStatus((uint32)cfg_ptr->gpio_num);
        _GPIO_PROD_SetInterruptSense (GPIO_PROD_GSENSOR_INT_ID, GPIO_INT_LEVEL_LOW);
    }
    else
    {
    ...
    if (cfg_ptr->gpio_cb_fun)
    {
        cfg_ptr->gpio_cb_fun (is_high);
    }
...
PUBLIC void GPIO_EnableCodec (BOOLEAN b_enable)
{
   // _GPIO_PROD_SetVal (GPIO_PROD_CODEC_EN_ID, b_enable);
    ...
    //GPIO_SetValue (27, 0); // wuxx add  CODEC_1V8_EN
#endif
    GPIO_SetValue (11, 1);
