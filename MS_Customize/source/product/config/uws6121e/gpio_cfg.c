...
#ifdef ZDT_GSENSOR_SUPPORT // wuxx add 20230923
extern uint32 ZDTGSensor_GPIO_IntHandler(BOOLEAN is_high);
...
#ifdef ZDT_GSENSOR_SUPPORT // wuxx 20230923
    {GPIO_PROD_GSENSOR_INT_ID,			GPIO_PROD_TYPE_BB0, GPIO_PROD_HIGH_LEVEL, 	2,  (GPIO_CB)ZDTGSensor_GPIO_IntHandler},	//G-SENSOR INT1
...
LOCAL uint32 _GPIO_OpenVibrator(BOOLEAN is_open)
{
    if(is_open)
    {
        ...
        GPIO_SetValue(3, 1); // add by bao for vib 
        ANA_SetDevValule(ANA_DEV_ID_VIBRATOR, ANA_DEV_VAL_DEF);
    }
    else
    {
        GPIO_SetValue(3, 0); // add by bao for vib 
...
PUBLIC void GPIO_CustomizeInit(void)
{
    ...
    GPIO_PROD_RegGpio(
		...
		SCI_FALSE,
		0,
		(GPIO_PROD_CALLBACK)GPIO_ZDT_GSensorIntHandler
		);	
#endif
