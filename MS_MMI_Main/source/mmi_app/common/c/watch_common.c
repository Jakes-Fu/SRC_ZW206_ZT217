...
PUBLIC void WATCHCOM_DisplayWindowBackground( MMI_WIN_ID_T win_id, GUI_COLOR_T color, MMI_IMAGE_ID_T image_id)
{
    // TODO : win_id for different window requirement
    GUI_RECT_T         	disp_rect   		= MMITHEME_GetFullScreenRect();
    GUI_LCD_DEV_INFO  lcd_dev_info 	= {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    if(image_id == NULL)
    {
        LCD_FillRect(&lcd_dev_info, disp_rect, color);
    }
    else
    {
...
PUBLIC void WATCHCOM_DisplayTitleWithBg( MMI_WIN_ID_T win_id, MMI_TEXT_ID_T title_id)
{
    // TODO : win_id for different window requirement
    GUI_RECT_T         	disp_rect   		= WINDOW_TITLE_RECT;
    GUI_LCD_DEV_INFO  lcd_dev_info 	= {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    ...
    MMI_STRING_T   title_str     = {0};
    GUISTR_STYLE_T text_style = {0};
    GUIRES_DisplayImg(&point, PNULL, PNULL, win_id,res_title_bg, &lcd_dev_info); 
    text_style.font = DP_FONT_26;
    text_style.font_color = MMI_WHITE_COLOR;//MMI_BACKGROUND_COLOR;//changed by James li 20200511
    text_style.align = ALIGN_HVMIDDLE;//ALIGN_VMIDDLE;
    MMIRES_GetText(title_id, win_id, &title_str);
