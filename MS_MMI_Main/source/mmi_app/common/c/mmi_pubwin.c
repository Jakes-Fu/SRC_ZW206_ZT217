 ...
 **                         MACRO DEFINITION                                 *
 **--------------------------------------------------------------------------*/
#define WATCH_STRING_FONT            DP_FONT_24
...
//        win_info_ptr->prompt_image_id = image_id;
		...
		{
            MMI_STRING_T    *string_array[MMIPUB_TEXT_MAX] = {0};
#ifdef MAINLCD_DEV_SIZE_128X128 //解决128*128 提示框 softkey 在底部问题
            GUI_RECT_T rect = {0,0,MMI_MAINSCREEN_WIDTH,MMI_MAINSCREEN_HEIGHT};
            GUI_BOTH_RECT_T		both_rect = {0};
            rect.bottom = rect.bottom - MMITHEME_GetSoftkeyHeight()/3;
            both_rect.h_rect = rect;
            both_rect.v_rect = rect;
            MMK_SetWinBothRect(win_handle, &both_rect);
#endif           
                             ...
                             )
{
    ...
    GUI_RECT_T      title_rect = {0};
    GUI_RECT_T      client_rect = {0};
#if defined(MMIPUBWIN_PDASTYLE_SUPPORT) || defined(MAINLCD_DEV_SIZE_128X128)
...
//     ClearPubWinLayer(lcd_dev_info);
    ...
    if ( MMIPUBWIN_WAITING != win_info_ptr->type )
    {
        ...
        else
        {
            ...
            image_rect.bottom = bg_rect.bottom;// - MMITHEME_GetSoftkeyHeight()/2;//GetPubWinBottomBGHeight( win_info_ptr, is_exist_skb );
            //yangyu add
            if(win_info_ptr->title_string.wstr_len == 0)
            {
            ...
            image_rect.bottom = bg_rect.bottom + MMITHEME_GetSoftkeyHeight()+10;
