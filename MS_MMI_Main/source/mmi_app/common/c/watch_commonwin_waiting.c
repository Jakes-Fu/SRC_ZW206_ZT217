                                                     ...
                                                     )
{
    ...
    {
        MMI_STRING_T tip = {0};
        MMIRES_GetText(TXT_LOADING, win_id, &tip);  
        WatchCOM_WaitingWin_1Icon_1Str_EX(win_id,&tip, PNULL, picImage, PNULL, userFunc);
        return;
    }   
                                                     ...
                                                     )
{
    ...
    else
    {
        ...
        img_rect.left = (MMI_MAINSCREEN_WIDTH-img_width)/2;
        img_rect.right = img_rect.left + img_width;
        img_rect.top = (MMI_MAINSCREEN_HEIGHT-img_height)/2;
    ...
    {
        styleLayout.layoutWaiting1Icon1Str.tipRect.left = 0;
        styleLayout.layoutWaiting1Icon1Str.tipRect.right = MMI_MAINSCREEN_WIDTH;
        styleLayout.layoutWaiting1Icon1Str.tipRect.top = styleLayout.layoutWaiting1Icon1Str.picRect.bottom + 10;
        styleLayout.layoutWaiting1Icon1Str.tipRect.bottom = styleLayout.layoutWaiting1Icon1Str.tipRect.top + 30;
    }
...
//  Description: enter waiting window (full screen)
//  Parameter: [In] win_id : commonwin ID
//             [In] tip_str : picture display string
//             [In] pic_image : picture display image
//             [In] softkey_text: the softkey text IDs for display
//             [In] userFunc : custom callback API
                                                     ...
                                                     )
{
    ...
    {
        img_id = image_watch_waitingwin_fullscreen_pic;
    }
    if(tip_str == PNULL)
    {
        MMIRES_GetText(TXT_LOADING, win_id, &tip);
        tip_str = &tip;
        TRACE_APP_COMMON("waitingwin's tip_str null");
    }
    pWinInfo = CommonWin_WaitingWin_1Icon_1Str_CreateData( win_id, tip_str, img_id, userFunc);
    if (PNULL == pWinInfo)
    {
    ...
    if(tip_str != PNULL)
    {
        TRACE_APP_COMMON("waitingwin's tip_str not null str len:%d top:%d left:%d buttom:%d",tip_str->wstr_len,pWinInfo->selfInfo.info1icon1str.tipRect.top,pWinInfo->selfInfo.info1icon1str.tipRect.left,pWinInfo->selfInfo.info1icon1str.tipRect.bottom);
        CommonWin_SetTextBoxStyle(
                                WATCH_COMMON_WAITINGWIN_TIP_TEXTBOX_CTRL_ID,
                                pWinInfo->selfInfo.info1icon1str.tipRect,
                                tip_str,
