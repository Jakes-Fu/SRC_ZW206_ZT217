...
#include "cjson.h"
#include "dal_time.h"
#include "dsl_word_id.h"
#include "dsl_word_main.h"
#include "dsl_word_text.h"
...
LOCAL GUI_RECT_T word_left_rect = {10, MMI_MAINSCREEN_HEIGHT-WORD_CARD_LINE_HIGHT, 3*WORD_CARD_LINE_WIDTH-10, MMI_MAINSCREEN_HEIGHT-2};//单词学习/掌握
LOCAL GUI_RECT_T word_right_rect = {3*WORD_CARD_LINE_WIDTH, MMI_MAINSCREEN_HEIGHT-WORD_CARD_LINE_HIGHT, MMI_MAINSCREEN_WIDTH-10, MMI_MAINSCREEN_HEIGHT-2};//生词本/未掌握
...
LOCAL GUI_RECT_T word_msg_rect = {WORD_CARD_LINE_WIDTH, WORD_CARD_LINE_HIGHT, MMI_MAINSCREEN_WIDTH-WORD_CARD_LINE_WIDTH, MMI_MAINSCREEN_HEIGHT-WORD_CARD_LINE_HIGHT};//界面的信息提示
LOCAL GUI_RECT_T word_tip_rect = {2*WORD_CARD_LINE_WIDTH,2*WORD_CARD_LINE_HIGHT,MMI_MAINSCREEN_WIDTH-2*WORD_CARD_LINE_WIDTH,4*WORD_CARD_LINE_HIGHT};//弹框提示
...
LOCAL GUI_RECT_T word_auto_play_rect = {3*WORD_CARD_LINE_WIDTH, 0, 4*WORD_CARD_LINE_WIDTH, WORD_CARD_LINE_HIGHT};//自动播放图标
...
LOCAL GUI_RECT_T word_text_rect = {5,3*WORD_CARD_LINE_HIGHT,MMI_MAINSCREEN_WIDTH-5,MMI_MAINSCREEN_HEIGHT-WORD_CARD_LINE_HIGHT};//单词释义

#define MAX_WORD_NUM 10
#define MAX_BOOK_NUM 20
#define MAX_CHAPTER_NUM 30
#define MAX_AUDIO_NUM 50
LOCAL DSL_BOOK_T *books[MAX_BOOK_NUM];
LOCAL DSL_CHAPTER_T *chapters[MAX_CHAPTER_NUM];
LOCAL DSL_WORD_T *words[MAX_WORD_NUM];
LOCAL DSL_WORD_AUDIO_T * words_audio[MAX_AUDIO_NUM];
LOCAL DSL_PAGE_INFO *pageInfo;
...
LOCAL void saveRecord(void)
{
	...
	books[pageInfo->bookIdx+dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM]->id,pageInfo->chapterIdx,pageInfo->curPage,pageInfo->realIdx,pageInfo->curIdx);
	if(!dsl_dir_exsit("E:/Word"))
	{
		dsl_dir_creat("E:/Word");
	}
	dsl_file_save("E:/Word/userdata",json,strlen(json),FALSE);
}
...
WINDOW_TABLE(MMI_WORD_WIN_TAB) = {
    ...
    //CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_MAIN_BUTTON_REFRESH),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_MAIN_LABEL_BACK),
    //CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_MAIN_PAD_LABEL_BACK),
    //CREATE_BUTTON_CTRL(IMG_BACK, MMI_DSL_WORD_MAIN_BUTTON_BACK),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMI_DSL_WORD_MAIN_LIST_CONTENT),
	CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_WORD_MAIN_LABEL_NUM_ID),
	WIN_HIDE_STATUS,
    END_WIN};
WINDOW_TABLE(MMI_WORD_CH_WIN_TAB) = {
    WIN_ID(MMI_WORD_CH_WIN_ID),
    WIN_FUNC((uint32)HandleWordCHWinMsg),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_CH_BUTTON_REFRESH),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_CH_LABEL_BACK),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_CH_PAD_LABEL_BACK),
    //CREATE_BUTTON_CTRL(IMG_BACK, MMI_DSL_WORD_CH_BUTTON_BACK),
	CREATE_BUTTON_CTRL(IMG_POETRY_AUTOPLAY,MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY),
    CREATE_BUTTON_CTRL(IMG_AUTO_PLAY,MMI_DSL_WORD_CH_BUTTON_AUTOPLAY),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMI_DSL_WORD_CH_LIST_CONTENT),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_CH_BUTTON_EXERCISE),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_CH_BUTTON_PACTISE),
	CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_WORD_CH_LABEL_NUM_ID),
	WIN_HIDE_STATUS,
    END_WIN};
WINDOW_TABLE(MMI_WORD_CARD_WIN_TAB) = {
    WIN_ID(MMI_WORD_CARD_WIN_ID),
    WIN_FUNC((uint32)HandleWordCardWinMsg),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_BUTTON_REFRESH),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_LABEL_TITLE_ID),
    //CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_PAD_LABEL_TITLE_ID),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_BUTTON_OK_ID),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_WORD_BUTTON_CACEL_ID),
    //CREATE_BUTTON_CTRL(IMG_BACK, MMI_DSL_WORD_BUTTON_BACK_ID),
	CREATE_BUTTON_CTRL(WORD_NEXT_ICON, MMI_DSL_WORD_BUTTON_NEXT_ID),
    CREATE_BUTTON_CTRL(WORD_PRE_ICON, MMI_DSL_WORD_BUTTON_PRE_ID),
    CREATE_BUTTON_CTRL(WORD_DELETE_ICON, MMI_DSL_WORD_BUTTON_DELETE_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_WORD_LABEL_NUM_ID),
	CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DSL_WORD_MSG_LABEL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DSL_WORD_MSG_SP1_ID),
	CREATE_BUTTON_CTRL(IMG_DIS_AUTO_PLAY, MMI_DSL_WORD_MSG_SP1_BTN_ID),
    CREATE_TEXT_CTRL(MMI_DSL_WORD_INFO_TEXT_ID),
	CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, MMI_DSL_WORD_LABEL_MESSAGE_ID),
	WIN_HIDE_STATUS,
    END_WIN};
...
LOCAL void requestWordCh(){
	...
	if(is_load_local)
	{
		loadLocalBookCH(books[pageInfo->bookIdx+dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM]->path,books[pageInfo->bookIdx+dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM]->basepath);
	}
	else
	{
...
LOCAL MMI_RESULT_E HandleWordWinMsg(MMI_WIN_ID_T win_id,MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
  ...
  switch (msg_id) {
    case MSG_OPEN_WINDOW:
    {
		...
		if (pageInfo == PNULL) {
			pageInfo = SCI_ALLOCA(sizeof(DSL_PAGE_INFO));
		}
		requestBookInfo();
        ...
        GUIBUTTON_SetRect(MMI_DSL_WORD_MAIN_LABEL_BACK, &word_title_rect);
		GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_MAIN_LABEL_BACK, closeWordWin);
		GUIBUTTON_SetFont(MMI_DSL_WORD_MAIN_LABEL_BACK, &font);
		GUIBUTTON_SetTextAlign(MMI_DSL_WORD_MAIN_LABEL_BACK,ALIGN_HVMIDDLE);
	
		GUILIST_SetRect(MMI_DSL_WORD_MAIN_LIST_CONTENT, &word_list_rect);
		GUILIST_SetListState(MMI_DSL_WORD_MAIN_LIST_CONTENT,GUILIST_STATE_SPLIT_LINE, FALSE);
		GUILIST_SetListState(MMI_DSL_WORD_MAIN_LIST_CONTENT,GUILIST_STATE_NEED_HIGHTBAR, FALSE);
		GUILIST_SetTopItemOffset(MMI_DSL_WORD_MAIN_LIST_CONTENT, 10);
		GUILIST_SetMaxItem(MMI_DSL_WORD_MAIN_LIST_CONTENT, DSL_WORD_MAIN_DISPLAY_NUM, FALSE);
        GUILIST_SetTextFont(MMI_DSL_WORD_MAIN_LIST_CONTENT, SONG_FONT_16, MMI_BLACK_COLOR);

		title_rect.top = word_title_rect.top + 2;
		GUILABEL_SetRect(MMI_DSL_WORD_MAIN_LABEL_NUM_ID, &title_rect, FALSE);
		GUILABEL_SetFont(MMI_DSL_WORD_MAIN_LABEL_NUM_ID, SONG_FONT_16,MMI_BLACK_COLOR);
        GUILABEL_SetAlign(MMI_DSL_WORD_MAIN_LABEL_NUM_ID, GUILABEL_ALIGN_RIGHT);
	...
	{
		if(dsl_word_main_display_idx+1<(dsl_word_main_display_count+DSL_WORD_MAIN_DISPLAY_NUM-1)/DSL_WORD_MAIN_DISPLAY_NUM)
		{
			dsl_word_main_display_idx++;
	...
	{
		if(dsl_word_main_display_idx>0)
		{
			dsl_word_main_display_idx--;
			MMK_PostMsg(MMI_WORD_WIN_ID, MSG_FULL_PAINT, PNULL, 0);
		}
	...
	case MSG_FULL_PAINT: 
	{
		int i = dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM;
		GUILIST_EMPTY_INFO_T empty_info = {0};

		GUI_LCD_DEV_INFO lcd_dev_info = {GUI_MAIN_LCD_ID,GUI_BLOCK_MAIN};

		GUI_FillRect(&lcd_dev_info,word_win_rect,MMI_WHITE_COLOR);
		
		GUIBUTTON_SetVisible(MMI_DSL_WORD_MAIN_BUTTON_REFRESH,FALSE,FALSE);

		GUIBUTTON_SetTextId(MMI_DSL_WORD_MAIN_LABEL_BACK, WORD_TITLE);

		empty_info.text_id = WORD_LOADING;
		GUILIST_SetEmptyInfo(MMI_DSL_WORD_MAIN_LIST_CONTENT, &empty_info);
		GUILIST_SetBgColor(MMI_DSL_WORD_MAIN_LIST_CONTENT,MMI_WHITE_COLOR);
		GUILIST_RemoveAllItems(MMI_DSL_WORD_MAIN_LIST_CONTENT);
		for (;i<(dsl_word_main_display_idx+1)*DSL_WORD_MAIN_DISPLAY_NUM&&i < dsl_word_main_display_count; i++) 
		{
			if (books[i] != PNULL) 
			{
				AppendDSLListItemByTextId(MMI_DSL_WORD_MAIN_LIST_CONTENT,IMG_DSL_CONTACT_ICON, i);
			}
		}
		if(dsl_word_main_display_count>0)
		{
			setNumberText(MMI_DSL_WORD_MAIN_LABEL_NUM_ID,dsl_word_main_display_idx, (dsl_word_main_display_count+DSL_WORD_MAIN_DISPLAY_NUM-1)/DSL_WORD_MAIN_DISPLAY_NUM);
			if(is_record==2)
			{
				OpenWordRecordTips();
			}
		...
		else if(dsl_word_main_display_count==-1)
		{
			...
			GUILIST_SetEmptyInfo(MMI_DSL_WORD_MAIN_LIST_CONTENT, &empty_info);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_MAIN_BUTTON_REFRESH,TRUE,TRUE);
		}
    ...
    case MSG_CTL_PENOK:
	{
		...
		pageInfo->bookIdx =GUILIST_GetCurItemIndex(MMI_DSL_WORD_MAIN_LIST_CONTENT);
		openChRequest();
	}
	break;
...
LOCAL MMI_RESULT_E HandleWordCHWinMsg(MMI_WIN_ID_T win_id,MMI_MESSAGE_ID_E msg_id, DPARAM param) 
{
  ...
  switch (msg_id) {
    case MSG_OPEN_WINDOW: 
	{
		...
		GUIBUTTON_SetRect(MMI_DSL_WORD_CH_PAD_LABEL_BACK, &word_title_rect);
		GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_PAD_LABEL_BACK, closeWordCHWin);
        
		GUIBUTTON_SetRect(MMI_DSL_WORD_CH_LABEL_BACK, &word_title_rect);
		GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_LABEL_BACK, closeWordCHWin);
		...
		GUILIST_SetListState(MMI_DSL_WORD_CH_LIST_CONTENT,GUILIST_STATE_NEED_HIGHTBAR, FALSE);
		GUILIST_SetMaxItem(MMI_DSL_WORD_CH_LIST_CONTENT, MAX_CHAPTER_NUM, FALSE);
        GUILIST_SetBgColor(MMI_DSL_WORD_CH_LIST_CONTENT, MMI_WHITE_COLOR);
        GUILIST_SetTextFont(MMI_DSL_WORD_CH_LIST_CONTENT, SONG_FONT_16, MMI_BLACK_COLOR);

		GUIBUTTON_SetRect(MMI_DSL_WORD_CH_BUTTON_PACTISE, &word_left_rect);//单词学习
		GUIBUTTON_SetRect(MMI_DSL_WORD_CH_BUTTON_EXERCISE, &word_right_rect);//生词本
		GUIBUTTON_SetBorder(MMI_DSL_WORD_CH_BUTTON_PACTISE, &btn_border, FALSE);
		GUIBUTTON_SetBorder(MMI_DSL_WORD_CH_BUTTON_EXERCISE, &btn_border, FALSE);
		GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_PACTISE,openWordCardWin);
		//SCI_TRACE_LOW("%s: is_load_local = %d", __FUNCTION__, is_load_local);
		if(is_load_local)
		{
			GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_EXERCISE,openWordCardWin);
		}else
		{
			GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_EXERCISE,openNewWordCardWin);
		}

		font.font = SONG_FONT_16;
		font.color = MMI_BLACK_COLOR;
		GUIBUTTON_SetFont(MMI_DSL_WORD_CH_BUTTON_EXERCISE, &font);
		GUIBUTTON_SetFont(MMI_DSL_WORD_CH_BUTTON_PACTISE, &font);
		
        title_rect = word_title_rect;
        title_rect.top += 2;
		GUILABEL_SetRect(MMI_DSL_WORD_CH_LABEL_NUM_ID, &title_rect, FALSE);
		GUILABEL_SetFont(MMI_DSL_WORD_CH_LABEL_NUM_ID, SONG_FONT_16,MMI_BLACK_COLOR);
		dsl_chapter_select_idx=0;

		LayoutItem(MMI_DSL_WORD_CH_BUTTON_REFRESH, 72, 168, 317, 346);
		InitButton(MMI_DSL_WORD_CH_BUTTON_REFRESH);
		GUIBUTTON_SetTextId(MMI_DSL_WORD_CH_BUTTON_REFRESH, WORD_REFRESH);
		GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_REFRESH,wordChRefresh);
	...
	case MSG_LIST_NEXT_PAGE:
	{
		if(dsl_chapter_display_idx+1<(dsl_chapter_display_count+DSL_CHAPTER_DISPLAY_NUM-1)/DSL_CHAPTER_DISPLAY_NUM)
		{
			dsl_chapter_display_idx++;
			dsl_chapter_select_idx=0;
			MMK_PostMsg(MMI_WORD_CH_WIN_ID, MSG_FULL_PAINT, PNULL, 0);
		}	
	}
    ...
    case MSG_FULL_PAINT: 
	{
		int i = dsl_chapter_display_idx*DSL_CHAPTER_DISPLAY_NUM;
		GUILIST_EMPTY_INFO_T empty_info = {0};
		GUI_LCD_DEV_INFO lcd_dev_info = {GUI_MAIN_LCD_ID,GUI_BLOCK_MAIN};
        
		GUI_FillRect(&lcd_dev_info,word_win_rect,0xffff);

		GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_REFRESH,FALSE,FALSE);
		if(is_autoplay)
		{
			GUIBUTTON_SetRect(MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY, &word_auto_play_rect);
			GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY, clickDisAutoPlay);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY,TRUE,TRUE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_AUTOPLAY,FALSE,FALSE);
		}
		else
		{
			GUIBUTTON_SetRect(MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY, &word_auto_play_rect);
			GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_CH_BUTTON_AUTOPLAY, clickAutoPlay);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_AUTOPLAY,TRUE,TRUE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_DISAUTOPLAY,FALSE,FALSE);
		}

		empty_info.text_id = WORD_LOADING;
		GUILIST_SetEmptyInfo(MMI_DSL_WORD_CH_LIST_CONTENT, &empty_info);

		GUIBUTTON_SetTextId(MMI_DSL_WORD_CH_LABEL_BACK, WORD_TITLE);
		
		GUIBUTTON_SetTextId(MMI_DSL_WORD_CH_BUTTON_PACTISE, WORD_PACTISE);
		GUIBUTTON_SetTextId(MMI_DSL_WORD_CH_BUTTON_EXERCISE, WORD_EXERCISE);
		GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_EXERCISE,FALSE,FALSE);
		GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_PACTISE,FALSE,FALSE);
		
		
		GUILIST_RemoveAllItems(MMI_DSL_WORD_CH_LIST_CONTENT);
		for (; i<(dsl_chapter_display_idx+1)*DSL_CHAPTER_DISPLAY_NUM&&i < dsl_chapter_display_count; i++) {
			if (chapters[i] != PNULL) {
				if (i-dsl_chapter_display_idx*DSL_CHAPTER_DISPLAY_NUM==dsl_chapter_select_idx) {
					appendCHItem(MMI_DSL_WORD_CH_LIST_CONTENT, chapters[i]->name,IMG_DSL_SELECTED);
				}
				else
				{
					appendCHItem(MMI_DSL_WORD_CH_LIST_CONTENT, chapters[i]->name,IMG_DSL_UNSELECTED);
				}
			}
		}
		...
		if(dsl_chapter_display_count>0)
		{
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_EXERCISE,TRUE,TRUE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_PACTISE,TRUE,TRUE);
			setNumberText(MMI_DSL_WORD_CH_LABEL_NUM_ID,dsl_chapter_display_idx, (dsl_chapter_display_count+DSL_CHAPTER_DISPLAY_NUM-1)/DSL_CHAPTER_DISPLAY_NUM);
		}
		else if(dsl_chapter_display_count==-2)
		{
			GUILIST_SetRect(MMI_DSL_WORD_CH_LIST_CONTENT, &word_list_rect);
			empty_info.text_id = WORD_NO_DATA;
			GUILIST_SetEmptyInfo(MMI_DSL_WORD_CH_LIST_CONTENT, &empty_info);
		}
		else if(dsl_chapter_display_count==-1)
		{
			...
			empty_info.text_id = WORD_LOADING_FAILED;
			GUILIST_SetEmptyInfo(MMI_DSL_WORD_CH_LIST_CONTENT, &empty_info);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_CH_BUTTON_REFRESH,TRUE,TRUE);
		}
		...
		if(is_autodisplay_tip!=0)
			{
				...
				GUI_FillRect(&auto_tip_layer,word_tip_rect,0xffff);

				LCD_DrawRect(&auto_tip_layer, word_tip_rect, MMI_BLACK_COLOR);
	...
	{
		uint index = GUILIST_GetCurItemIndex(MMI_DSL_WORD_CH_LIST_CONTENT);
...
LOCAL void LoadWordInfo(int position) {
    GUI_RECT_T uk_rect = {5, 36, 125, 52};
	MMI_STRING_T text = {0, };
	...
	//单词
    GUILABEL_SetRect(MMI_DSL_WORD_MSG_LABEL_ID, &word_word_rect, FALSE);
	...
	GUILABEL_SetText(MMI_DSL_WORD_MSG_LABEL_ID, &text_word, FALSE);
				
	//音标1
	GUILABEL_SetRect(MMI_DSL_WORD_MSG_SP1_ID, &word_uk_rect, FALSE);
	...
	GUILABEL_SetText(MMI_DSL_WORD_MSG_SP1_ID, &text_en, FALSE);
	text_width_piex=GUI_CalculateStringPiexlNum(text_en.wstr_ptr, text_en.wstr_len, SONG_FONT_16, 1);

	if(text_width_piex>100)
	{
		GUILABEL_SetVisible(MMI_DSL_WORD_MSG_SP1_ID,FALSE,FALSE);
		text_width_piex=GUI_CalculateStringPiexlNum(text_word.wstr_ptr, text_word.wstr_len, SONG_FONT_20, 1);
		if(text_width_piex>100)
		{
	...
	{
		GUILABEL_SetVisible(MMI_DSL_WORD_MSG_SP1_ID,TRUE,FALSE);
		GUIBUTTON_SetVisible(MMI_DSL_WORD_MSG_SP1_BTN_ID,TRUE,FALSE);
	...
	//单词信息
	GUITEXT_SetRect(MMI_DSL_WORD_INFO_TEXT_ID, &word_text_rect);
	GUI_UTF8ToWstr(wstr, 2048, words[position]->explain, strlen(words[position]->explain)+1);
	text.wstr_len = MMIAPICOM_Wstrlen(wstr);
	text.wstr_ptr = wstr;
	GUITEXT_SetString(MMI_DSL_WORD_INFO_TEXT_ID, text.wstr_ptr,text.wstr_len, TRUE);
  ...
  MMI_RESULT_E recode = MMI_RESULT_TRUE;
	switch (msg_id) 
	{
		case MSG_OPEN_WINDOW: 
		{
			...
			font.font = SONG_FONT_24;
			font.color = MMI_BLACK_COLOR;

			GUIBUTTON_SetRect(MMI_DSL_WORD_LABEL_TITLE_ID, &word_title_rect);
			GUIBUTTON_SetFont(MMI_DSL_WORD_LABEL_TITLE_ID, &font);
			...
			//单词信息
			GUITEXT_SetRect(MMI_DSL_WORD_INFO_TEXT_ID, &word_text_rect);
			GUITEXT_SetFont(MMI_DSL_WORD_INFO_TEXT_ID, &text_font,&text_color);
			GUITEXT_IsDisplayPrg(FALSE, MMI_DSL_WORD_INFO_TEXT_ID);
			GUITEXT_SetHandleTpMsg(FALSE, MMI_DSL_WORD_INFO_TEXT_ID);
			GUITEXT_SetClipboardEnabled(MMI_DSL_WORD_INFO_TEXT_ID,FALSE);
			GUITEXT_IsSlide(MMI_DSL_WORD_INFO_TEXT_ID,FALSE);

			LayoutItem(MMI_DSL_WORD_BUTTON_REFRESH, 72, 168, 317, 346);
			InitButton(MMI_DSL_WORD_BUTTON_REFRESH);
			GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_REFRESH, WORD_REFRESH);
			GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_REFRESH,wordRefresh);
		...
		case MSG_TP_PRESS_UP:
		{
			if(pageInfo->total>0&&words[pageInfo->curIdx] != PNULL&&pageInfo->curPageNum>0)
			{
				...
				if(ABS(tp_offset_x) <= ABS(tp_offset_y))
				{
					if(tp_offset_y > 40)
					{
						MMK_PostMsg(MMI_DSL_WORD_INFO_TEXT_ID, MSG_KEYREPEAT_UP, PNULL, 0);
					}
					else if(tp_offset_y < -40)
					{
						MMK_PostMsg(MMI_DSL_WORD_INFO_TEXT_ID, MSG_KEYREPEAT_DOWN, PNULL, 0);
					}
				}
			}
		}
		...
		case MSG_FULL_PAINT: 
		{
			...
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_REFRESH,FALSE,FALSE);

			GUILABEL_SetFont(MMI_DSL_WORD_LABEL_MESSAGE_ID, SONG_FONT_16,MMI_BLACK_COLOR);              
			if(is_new_word)
			{
				GUIBUTTON_SetTextId(MMI_DSL_WORD_LABEL_TITLE_ID, WORD_EXERCISE);
			}
			else
			{
				...
				GUIBUTTON_SetText(MMI_DSL_WORD_LABEL_TITLE_ID, title_text.wstr_ptr,title_text.wstr_len);
			}
			
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_CACEL_ID,FALSE,FALSE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_OK_ID,FALSE,FALSE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_NEXT_ID,FALSE,FALSE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_PRE_ID,FALSE,FALSE);
			GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_DELETE_ID,FALSE,FALSE);
			GUILABEL_SetVisible(MMI_DSL_WORD_LABEL_NUM_ID,FALSE,FALSE);
			
			GUILABEL_SetVisible(MMI_DSL_WORD_LABEL_MESSAGE_ID,TRUE,TRUE);
			GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, WORD_LOADING, FALSE);
			if(pageInfo->total==-1)
			{
				GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, WORD_LOADING_FAILED,FALSE);
				GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_REFRESH,TRUE,TRUE);
			}else if(pageInfo->total==-2)
			{
				if(is_new_word)
				{
					GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, NEW_WORD_BOOK_NO_DATA,FALSE);
				}else
				{
					GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, WORD_NO_DATA,FALSE);
				}
			}else if(pageInfo->total>0)
			{
				if(pageInfo->realIdx<pageInfo->total)
				{
					setNumberText(MMI_DSL_WORD_LABEL_NUM_ID,pageInfo->realIdx, pageInfo->total);
					GUILABEL_SetVisible(MMI_DSL_WORD_LABEL_NUM_ID,TRUE,TRUE);
					if (words[pageInfo->curIdx] != PNULL&&pageInfo->curPageNum>0) 
					{
						GUILABEL_SetVisible(MMI_DSL_WORD_LABEL_MESSAGE_ID,FALSE,TRUE);
						...
						if(is_new_word)
						{
							...
							{
								GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_PRE_ID, &word_pre_rect);
								GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_PRE_ID,openNewWordPreCard);
								GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_PRE_ID,TRUE,TRUE);
							}
							if(pageInfo->realIdx+1<pageInfo->total)
							{
								GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_NEXT_ID, &word_next_rect);
								GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_NEXT_ID,openNewWordNextCard);
								GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_NEXT_ID,TRUE,TRUE);
							}
							GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_DELETE_ID, &word_del_rect);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_DELETE_ID,deleteNewWordCard);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_DELETE_ID,TRUE,TRUE);							
						}else
						{
							GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_OK_ID, &word_left_rect);
							InitButton(MMI_DSL_WORD_BUTTON_OK_ID);
							GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_CACEL_ID, &word_right_rect);
							InitButton(MMI_DSL_WORD_BUTTON_CACEL_ID);
							GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_CACEL_ID, WORD_CACEL);
							GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_OK_ID, WORD_OK);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_CACEL_ID,TRUE,TRUE);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_OK_ID,TRUE,TRUE);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_CACEL_ID,setWordToNew);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_OK_ID, openNextCard);
						}
					}
				}
				else
				{
					if(is_new_word)
					{
						GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, NEW_WORD_BOOK_FINISH, TRUE);
					}else
					{
						if(chapters[pageInfo->chapterIdx+1]!=PNULL)
						{ 
							GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, WORD_FINISH, TRUE);
							GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_OK_ID, &word_left_rect);
							InitButton(MMI_DSL_WORD_BUTTON_OK_ID);
							GUIBUTTON_SetRect(MMI_DSL_WORD_BUTTON_CACEL_ID, &word_right_rect);
							InitButton(MMI_DSL_WORD_BUTTON_CACEL_ID);
							GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_CACEL_ID, WORD_NEXT_CH);
							GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_OK_ID, WORD_BACK_CH);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_CACEL_ID,TRUE,TRUE);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_OK_ID,TRUE,TRUE);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_CACEL_ID,openNextCh);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_OK_ID, closeWordCardWin);
						}
						else
						{
							GUILABEL_SetTextById(MMI_DSL_WORD_LABEL_MESSAGE_ID, WORD_BOOK_FINISH, TRUE);
							LayoutItem(MMI_DSL_WORD_BUTTON_OK_ID, 72, 168, 317, 346);
							InitButton(MMI_DSL_WORD_BUTTON_OK_ID);
							GUIBUTTON_SetTextId(MMI_DSL_WORD_BUTTON_OK_ID, WORD_BACK_CH);
							GUIBUTTON_SetVisible(MMI_DSL_WORD_BUTTON_OK_ID,TRUE,TRUE);
							GUIBUTTON_SetCallBackFunc(MMI_DSL_WORD_BUTTON_OK_ID, closeWordCardWin);
...
LOCAL void requestWordInfo() {
  ...
  if(is_new_word)
  {
  	sprintf(url,"v1/card/word/collect?cardId=%s&bookId=%s&sectionId=%s&F_page=%d&F_pagesize=%d",BASE_DEVICE_IMEI,books[pageInfo->bookIdx+dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM]->id, chapters[pageInfo->chapterIdx]->id, pageInfo->curPage,MAX_WORD_NUM);
  }else
  {
	sprintf(url,"v1/card/book/wordList?F_book_id=%s&F_section_id=%s&F_page=%d&F_limit=%d",books[pageInfo->bookIdx+dsl_word_main_display_idx*DSL_WORD_MAIN_DISPLAY_NUM]->id, chapters[pageInfo->chapterIdx]->id, pageInfo->curPage,MAX_WORD_NUM);
  }
  SCI_TRACE_LOW("%s: url = %s", __FUNCTION__, url);
...
LOCAL void addChapterInfo(uint8 position, char *name, int id,char * path) {
	...
	chapters[position] = SCI_ALLOCA(sizeof(DSL_CHAPTER_T));
	SCI_MEMSET(chapters[position],0,sizeof(DSL_CHAPTER_T));
...
LOCAL void addBookInfo(uint8 position, char *name , char *editionName, int id,char *path,char *basepath) {
	int size=0;
	char bookid[11]={0};
	books[position] = SCI_ALLOCA(sizeof(DSL_BOOK_T));
	SCI_MEMSET(books[position],0,sizeof(DSL_BOOK_T));
	size=strlen(name) +1;
...
LOCAL parseMp3Response(BOOLEAN is_ok,uint8 * pRcv,uint32 Rcv_len,uint32 err_id)
{
	...
	for(i=0;i<MAX_WORD_NUM;i++)
	{
		if(words[i]!=PNULL)
		{
			/*if(words[i]->us_audio_request_idx==idx)
			{
				if (is_ok) {
					if(Rcv_len>0&&pRcv!=PNULL)
					{
						words[i]->us_audio_data_len=Rcv_len;
						...
						words[i]->us_audio_data = SCI_ALLOCA(Rcv_len);
						SCI_MEMSET(words[i]->us_audio_data, 0, Rcv_len);
						SCI_MEMCPY(words[i]->us_audio_data, pRcv, Rcv_len);
					}
					else
					{
						words[i]->us_audio_data_len=-2;
					}
				}
				else
				{
					int size=0;
					words[i]->us_audio_data_len=-1;
					size=strlen(data->data_url)+1;
					if(words[i]->us_audio_data!=PNULL)
					{
						SCI_FREE(words[i]->us_audio_data);
						words[i]->us_audio_data=PNULL;
					...
					words[i]->us_audio_data = SCI_ALLOCA(size);
					SCI_MEMSET(words[i]->us_audio_data, 0, size);
					SCI_MEMCPY(words[i]->us_audio_data, data->data_url, size);
				}
				SCI_TRACE_LOW("%s i %d us len %d", __FUNCTION__,i,words[i]->us_audio_data_len);
				break;
			}*/
			//if(words[i]->uk_audio_request_idx==idx)
			{
				...
				/*else
				{
					...
					words[i]->uk_audio_data = SCI_ALLOCA(size);
					SCI_MEMSET(words[i]->uk_audio_data, 0, size);
					SCI_MEMCPY(words[i]->uk_audio_data, data->data_url, size);
				}*/
...
LOCAL void addWordInfo(uint8 position, char *word,char *sentence,char *sentence_explain, char *us,char *us_sound, char *uk,char *uk_sound,char *explain) {
	...
	if(is_homework_task && (strlen(homework_task_id) > 0 || word_task_msg_id != NULL)){
		...
		words_audio[index] = SCI_ALLOCA(sizeof(DSL_WORD_AUDIO_T));
		SCI_MEMSET(words_audio[index],0,sizeof(DSL_WORD_AUDIO_T));
	}
	if(us!=PNULL)
	{
		size=strlen(us)+1;
		...
		if(us_sound!=PNULL)
		{
			//words[position]->us_audio_request_idx=++mp3_idx;
			//#ifdef WIN32
	...
	if(uk!=PNULL)
	{
		...
		if(uk_sound!=PNULL)
		{
			//words[position]->uk_audio_request_idx=++mp3_idx;
			//#ifdef WIN32
