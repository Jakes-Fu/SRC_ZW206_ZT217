...
LOCAL void Display_Contact_List(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id, VIDEO_USER_INFO *user_info)
{
    ...
    const uint16 h_gap = DP2PX_VALUE(5);
    const uint16 w_gap = DP2PX_VALUE(5);
    const uint16 img_height = DP2PX_VALUE(60);
    const uint16 img_width = DP2PX_VALUE(60);
    ...
    new_style.content[1].font= new_style.content[1].font_focus = DP_FONT_26;
...
LOCAL MMI_RESULT_E  HandleVideoContactListWin(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
            ...
            GUILIST_SetListState( ctrl_id, GUILIST_STATE_NEED_HIGHTBAR, FALSE ); //不画高亮条
