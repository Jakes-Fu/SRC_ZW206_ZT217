...
uint8 video_call_ten_timer_id = 0;
uint8 video_call_charging_delay_timer_id = 0;
...
#define VIDEO_CALL_TEN_TIME_OUT 10*60*1000 //最多通话10分钟
#define VIDEO_CALL_CHARGING_DELAY_TIME_OUT 3000
...
#ifdef WIN32
int avc_osd_blend(uint8 osd_id,bool enable,uint8_t *rgba8888,uint16_t left,uint16_t top,uint16_t right,uint16_t bottom){return 0;};
...
{
    baidu_videocall_open(appid,channel_name);
    return 0;
}

PUBLIC int video_call_stop(void)
{
    baidu_videocall_close();
    return 0;
}
...
{
    #ifdef BAIDU_VIDEOCHAT_SUPPORT
    //brtc_sdk_set_osd_area(show, (const char*)&gImage_icon_videocall_connect_ringoffBtn_brtc, left, top, right, bottom); // 90, 156, 149, 211
    //brtc_sdk_set_osd_area(show,  (const char*)&gImage_icon_videocall_connect_ringoffBtn_brtc, 90, 156, 149, 215); // 90, 156, 149, 211
    baidu_videocall_set_incall_hangup_button(show, left, top, right, bottom);
    #else
...
LOCAL void Video_Call_TimeOut_CallBack(uint8 timer_id, uint32 param)
{
    ...
    if(winId == WATCH_VIDEO_CALL_INCOMING_WIN_ID) //app call设备超时不接听
    {
        ...
        {
            SCI_TraceLow("wuxx, Video_Call_TimeOut_CallBack, video_call_info->video_id=%s", video_call_info->video_id);
...
LOCAL void Stop_Call_Ten_Timer()
{
    if(video_call_ten_timer_id != 0)
    {
        MMK_StopTimer(video_call_ten_timer_id);
        video_call_ten_timer_id = 0;
    }
}
...
{
    if( video_call_charging_delay_timer_id != 0)
    {
        MMK_StopTimer(video_call_charging_delay_timer_id);
        video_call_charging_delay_timer_id = 0;
    }
}

//视频通话中充电 视频结束后恢复充电动画
LOCAL void Video_Call_Charging_Delay_CallBack(uint8 timer_id, uint32 param)
{
    Stop_Charging_Delay_Timer();
    MMIZDT_CheckOpenChargingWin();
}

//视频通话中充电 视频结束后恢复充电动画
LOCAL void Start_Charging_Delay_Timer(uint32 time_out)
{
    Stop_Charging_Delay_Timer();
    video_call_charging_delay_timer_id = MMK_CreateTimerCallback(time_out,Video_Call_Charging_Delay_CallBack, PNULL, FALSE);
}
...
LOCAL void draw_head_image_and_name(MMI_WIN_ID_T win_id, GUISTR_STYLE_T text_style, GUI_LCD_DEV_INFO lcd_dev_info,MMI_VIDEO_CALL_STATE_E type)
{
    ...
    GUI_POINT_T point = {0,40};// #define VIDEO_CALL_HEAD_IMG_Y 40
...
#if 1// APP关闭数据流量, 使用定时器挂断
// 如WIFI切4G时,APP会重新给手表发视频过来,不用挂断
// 但是某种情况持续不重新发视频过来时手表需要挂断操作
// wuxx 20240104
static uint8 s_baidu_video_offline_to_hangup_timer_id = 0;

BOOLEAN baidu_video_offline_to_hangup_delay_StartTimer(uint32 duaration);

void baidu_video_offline_to_hangup_delay(void)
{
    SCI_TraceLow("TIMER CHECK,baidu_video_offline_to_hangup_delay.");

    SCI_TraceLow("TIMER CHECK,baidu_video_offline_to_hangup_delay. to Video_Call_Remote_Hangup");
...
BOOLEAN baidu_video_offline_to_hangup_delay_StartTimer(uint32 duaration)
{
    ...
    if(0 != s_baidu_video_offline_to_hangup_timer_id)
    {
        MMK_StopTimer(s_baidu_video_offline_to_hangup_timer_id);
        s_baidu_video_offline_to_hangup_timer_id = 0;
    }

    s_baidu_video_offline_to_hangup_timer_id = MMK_CreateTimerCallback(duaration, 
                                                                        baidu_video_offline_to_hangup_delay, 
                                                                        (uint32)0, 
                                                                        FALSE);
    return TRUE;
}

BOOLEAN baidu_video_offline_to_hangup_delay_StopTimer(void)
{
    if (s_baidu_video_offline_to_hangup_timer_id != 0)
    {
        MMK_StopTimer(s_baidu_video_offline_to_hangup_timer_id);
        s_baidu_video_offline_to_hangup_timer_id = 0;
        return TRUE;
...
#ifdef BAIDU_VIDEOCHAT_SUPPORT// wuxx 20231218 modify struct

//视频通话状态回调// baidu
LOCAL void Video_Call_Status_Callback(brtc_msg_t *p_msg)
{
    ZDT_LOG("[%s]recv msg. code=%d", __FUNCTION__, p_msg->code);
    switch (p_msg->code) {
        case BRTC_MSG_JOIN_CHANNEL_SUCCESS: /* local本端成功加入channel消息 */
            //MMIVIDEO_OpenVideoWin();
            //DrawCtrl_In_Video_Call(FALSE);
            break;
        case BRTC_MSG_USER_JOINED: /* remote远端成功加入channel消息 */
            if(MMK_IsOpenWin(WATCH_VIDEO_CALL_INCOMING_WIN_ID))
            {
                //MMK_SendMsg(WATCH_VIDEO_CALL_INCOMING_WIN_ID,MSG_FULL_PAINT,PNULL);
            }   
            break;
        case BRTC_MSG_USER_OFFLINE: /* remote远端已下线 */
            if(MMK_IsOpenWin(WATCH_VIDEO_CALL_INCOMING_WIN_ID))
            {
               //MMK_SendMsg(WATCH_VIDEO_CALL_INCOMING_WIN_ID,MSG_CLOSE_WINDOW,PNULL);
               //MMK_CloseWin(WATCH_VIDEO_CALL_INCOMING_WIN_ID);
               //Video_Call_Show_Toast(TXT_VIDEO_CALL_REMOTE_HANGUP);
               SCI_TraceLow("Video_Call_Status_Callback, wuxx, BRTC_MSG_USER_OFFLINE");

               ////Video_Call_Remote_Hangup(); // 如WIFI切4G时,APP会重新给手表发视频过来,不用挂断 
               // wuxx 20240104
               // 如WIFI切4G时,时间过长, 或者连接真断开,定时判断处理挂断,手表要退出画面
               // 收到RTC_MESSAGE_ROOM_EVENT_REMOTE_LEAVING  会比较慢,所以使用5 秒定时
               baidu_video_offline_to_hangup_delay_StartTimer(5000);
            }
        ...
        case BRTC_MSG_CONNECTION_LOST: /* local本端因网络原因掉线 */
            break;
        case BRTC_MSG_INVALID_APP_ID: /* appId不合法 */
            break;
        case BRTC_MSG_INVALID_CHANNEL_NAME: /* channel name不合法 */
            break;
        case BRTC_MSG_INVALID_TOKEN: /* token不合法 */
            break;
        case BRTC_MSG_TOKEN_EXPIRED: /* token已过期 */
            break;
        case BRTC_MSG_FIRST_VIDEO_FRAME_RECV: /* 加入channel后接收到remote远端第一帧视频数据 */
            {
                GUI_RECT_T hangup_in_vodeo_rect = VIDEO_CALL_IN_VIDEO_HANGUP_RECT;
                ...
                ////SCI_TraceLow("Video_Call_Status_Callback,wuxx, g_baidu_video_call_timestamp_start=%d", g_baidu_video_call_timestamp_start);
                
                SCI_TraceLow("Video_Call_Status_Callback, wuxx, BRTC_MSG_FIRST_VIDEO_FRAME_RECV");
            }
            break;
        default:
        ...
        case AVC_MSG_FIRST_VIDEO_FRAME_RECV: /* 加入channel后接收到remote远端第一帧视频数据 */
            {
                ...
                SCI_TraceLow("Video_Call_Status_Callback, wuxx, AVC_MSG_FIRST_VIDEO_FRAME_RECV");
            }
            break;
        default:
            ZDT_LOG("[%s] no msg handler, code %d", __FUNCTION__, p_msg->code);
...
#ifdef BAIDU_VIDEOCHAT_SUPPORT
PUBLIC void Baidu_Video_Call_Status_Callback(RtcMessageType msgType)
{
    brtc_msg_t msg = {0};
    ZDT_LOG("Baidu_Video_Call_Status_Callback type=%d", msgType);
    switch (msgType)
    {
    case RTC_MESSAGE_ROOM_EVENT_LOGIN_OK:
        {
            SCI_TraceLow("Baidu_Video_Call_Status_Callback,wuxx, RTC_MESSAGE_ROOM_EVENT_LOGIN_OK");
            break;
        }
    ...
    case RTC_ROOM_EVENT_ON_USER_JOINED_ROOM:
        {
            SCI_TraceLow("Baidu_Video_Call_Status_Callback,wuxx, RTC_ROOM_EVENT_ON_USER_JOINED_ROOM");
            // wuxx 20240104
            // 如WIFI切4G时,时间过长, 或者连接真断开,定时判断处理挂断,手表要退出画面
            // 重新进入ROOM , 停止baidu_video_offline_to_hangup_delay_StopTimer
...
LOCAL void Baidu_Video_CallFrameObserver(int64_t feedid, const char *img, int len, RtcImageType imgtype, int w, int h)
{
...
//拒接或者挂断视频通话
/**************************************/
LOCAL void Video_Call_Reject_Or_Hangup(MMI_WIN_ID_T win_id)
{
    VIDEO_CALL_INFO *video_call_info = NULL;
    ...
    {
        SCI_TraceLow("wuxx, Video_Call_Reject_Or_Hangup, video_call_info->video_id=%s", video_call_info->video_id);
        YX_Net_Send_UPWATCHHANGUP(video_call_info->video_id); //未接通时挂断需要通知APP
        MMIAPISET_StopRing(MMISET_RING_TYPE_CALL);
        MMK_CloseWin(win_id);// HANG UP msg IN close win 20231208
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
LOCAL void Video_Call_Accept_And_Hangup(GUI_POINT_T tp_up, MMI_WIN_ID_T win_id)
{
        ...
        else
        {
            ...
            if(video_call_info != NULL && !g_is_incomeing && video_call_info->call_type == VIDEO_CALL_INCOMING)
            {
                ...
                if (MMIZDT_Net_Rssi_IsLow())
                {
                    ...
                    Video_Call_Show_Toast(TXT_VIDEO_CALL_LOW_SIGNAL); // wuxx add // TXT_VIDEO_CALL_CONNECT_TIMEOUT
                    #if 1 // WUXX_20231111
                    Video_Call_Close(WATCH_VIDEO_CALL_INCOMING_WIN_ID);
                    MMK_CloseWin(win_id);
                    #else
                ...
                g_is_incomeing = TRUE;
    #ifdef BAIDU_VIDEOCHAT_SUPPORT
                video_call_start(video_call_info->appId, video_call_info->romId, Video_Call_Status_Callback);
    #else
    ...
    else if(GUI_PointIsInRect(tp_up, hangup_rect)) //挂断
    {
        ...
        SCI_TraceLow("Video_Call_Accept_And_Hangup,wuxx 01, g_is_incomeing=%d", g_is_incomeing);
        if(!g_is_incomeing)
        {
           SCI_TraceLow("Video_Call_Accept_And_Hangup,wuxx 02, g_is_incomeing=%d", g_is_incomeing);
            Video_Call_Reject_Or_Hangup(win_id);//来电拒接
        }
    }
    else if(GUI_PointIsInRect(tp_up, hangup_in_video_rect) && g_is_video_call_show_hangup)// (GUI_PointIsInRect(tp_up, hangup_in_video_rect) && g_is_video_call_show_hangup) //视频通话中挂断
    {
    ...
    case MSG_TP_PRESS_UP:
        {
            ...
            SCI_TraceLow("HandleVideoIncommingWin,wuxx,MSG_TP_PRESS_UP, g_is_incomeing=%d", g_is_incomeing);
    ...
    case MSG_CLOSE_WINDOW:
        {
            ...
            //External_CloseDCApplet();// 防止在相机界面来视频接听挂断死机,须先退出CAMER CLOSE // 百度的不要添加
#endif
#if 1 // here
            video_call_info = (VIDEO_CALL_INFO *) MMK_GetWinUserData(win_id);
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (g_video_call_remote_hangup_but_not_invideo == TRUE)
            {
                //还未接通时, APP 远端挂断, 不在视频中时的远端挂断标志// msg 已经在 YX_Net_Recevice_DOWNAPPHANGUP 回复过
            }
            else
            {
                //YX_Net_Send_UPWATCHHANGUP(video_call_info->video_id); //努比亚服务器要求接通后挂断不要通知APP
            }
            //if(video_call_info != NULL && (video_call_info->call_type == VIDEO_CALL_INCOMING || video_call_info->call_type == VIDEO_CALL_OUT))
            ...
            SCI_TraceLow("HandleVideoIncommingWin, wuxx, MSG_CLOSE_WINDOW");
            if(MMIZDT_ShouldStayInChargingWin()) //连接着充电线
            {
...
PUBLIC void Video_Call_Remote_Hangup()
{
    ...
    SCI_TraceLow("Video_Call_Remote_Hangup, wuxx");
    if (MMK_IsOpenWin(win_id))
    {
        SCI_TraceLow("Video_Call_Remote_Hangup, wuxx,MMK_CloseWin");
...
// wuxx add  for BAIDU VIDEO , WUXX_20231208
PUBLIC BOOLEAN Baidu_video_is_incoming_call_screen_win(void)
{
	if(WATCH_VIDEO_CALL_INCOMING_WIN_ID== MMK_GetFocusWinId())
	{
		return TRUE;
	}
...
PUBLIC BOOLEAN Baidu_video_is_out_call_screen_win(void)
{
	if(WATCH_VIDEO_CALL_OUT_WIN_ID== MMK_GetFocusWinId())
	{
		return TRUE;
	}
...
PUBLIC void Baidu_video_incoming_call_screen_red_key_msg(void)
{
        // RED KEY 中回复挂断消息等
	MMK_SendMsg(WATCH_VIDEO_CALL_INCOMING_WIN_ID, MSG_KEYUP_RED, PNULL);
}

PUBLIC void Baidu_video_out_call_screen_red_key_msg(void)
{
	MMK_SendMsg(WATCH_VIDEO_CALL_OUT_WIN_ID, MSG_KEYUP_RED, PNULL);
}
