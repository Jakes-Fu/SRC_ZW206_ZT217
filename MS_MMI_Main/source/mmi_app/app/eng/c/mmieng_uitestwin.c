...
typedef enum
{
    AUDIO_STREAM_PLAY_OR_RECORD_OR_EXPRESS = 0,//对于AMR录制来说，PLAY＝录音?PPAUSE＝录音 结束
...
PUBLIC uint32 zdt_get_dev_sn1(char *sn)    //
{
    uint32 len = 0;
    SCI_TRACE_LOW("mmieng_uitestwin.c zdt_get_dev_sn1 SP09_MAX_PHASE_BUFF_SIZE is %d",SP09_MAX_PHASE_BUFF_SIZE);
    ...
    SCI_MEMCPY(sn,s_mmieng_product.SN1,len);
    SCI_TRACE_LOW("mmieng_uitestwin.c zdt_get_dev_sn1 len : %d  sn:%s",len,sn);
    return  len;
}

PUBLIC uint32 zdt_get_dev_sn2(char *sn)    //
{
    
    uint32 len = 0;
    SCI_TRACE_LOW("mmieng_uitestwin.c zdt_get_dev_sn2 SP09_MAX_PHASE_BUFF_SIZE is %d",SP09_MAX_PHASE_BUFF_SIZE);
    ...
    SCI_MEMCPY(sn,s_mmieng_product.SN2,len);
    SCI_TRACE_LOW("mmieng_uitestwin.c zdt_get_dev_sn2 len : %d",len);
    return len;
}
