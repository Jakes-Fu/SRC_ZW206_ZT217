...
LOCAL BOOLEAN CL_ConvertNvInfoToArrayInfo(MMICL_CALL_BCDINFO_T* nv_info, WATCHCL_CALLINFO_T* array_info)
{
    ...
    if((PNULL == nv_info) || (PNULL == array_info))
    {
        ...
        //disp_wchar
    teleNumLen = MMICL_GenDispNumber(nv_info->call_type,nv_info->number_type,nv_info->number_len,nv_info->number,teleNum, PHONE_NUM_MAX_LEN + 2);
    array_info->photo_image_id = ZDT_PB_GetDispInfoContent((uint8 *)teleNum, teleNumLen, &array_info->disp_wchar, FALSE);
#else
    //disp_wchar
    ...
    array_info->call_type = nv_info->call_type;
    array_info->call_start_time = nv_info->call_start_time[nv_info->call_times-1];
    //bcd number
...
//	Note:  今天的用时间，其他的用日期
/*****************************************************************************/
LOCAL BOOLEAN GetTimeAndDateContent(MMI_STRING_T* in_str,uint32 call_start_time)
{
    ...
    tm = MMIAPICOM_Second2Tm(call_start_time);
...
//fixed by bingjie.chen
    ...
    {   
        ...
        MMI_STRNTOWSTR(in_str->wstr_ptr,32,(const uint8*)date_str,
    ...
    else   //use Date
    {
        ...
        MMI_STRNTOWSTR(in_str->wstr_ptr,32,(const uint8*)date_str,(uint32)strlen((char*)date_str),(uint32)strlen((char*)date_str));
    }
...
LOCAL void UpdateCallLogWinList(void)
{
    ...
    MMI_STRING_T                dateTimeStr = {0};
    wchar datetime_wstr[32 + 1] = {0};
    ...
    item_t.item_style = GUIITEM_STYLE_2STR_2LINE_LAYOUT1;//GUIITEM_STYLE_2STR_2LINE_LAYOUT1;
    ...
    for ( i = 0; i < arraycallInfo->record_num; i++ )
    {
        ...
        SCI_MEMSET(&dateTimeStr,0,sizeof(MMI_STRING_T));
        dateTimeStr.wstr_ptr = datetime_wstr;
        if(arraycallInfo->call_info[i].call_type == MMICL_CALL_MISSED)
        {
            item_data.item_content[0].font_color_id=MMITHEME_COLOR_RED;
            item_data.item_content[0].is_default =TRUE;        
            //item_data.item_content[0].custom_font_color_focus = MMI_BLUE_COLOR;  
        }
        else
        {
            item_data.item_content[0].font_color_id=MMITHEME_COLOR_NONE;
        }
        item_data.item_content[0].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
        item_data.item_content[0].item_data.text_buffer.wstr_ptr = numberStr.wstr_ptr;
        item_data.item_content[0].item_data.text_buffer.wstr_len = numberStr.wstr_len;
        if (0<arraycallInfo->call_info[i].call_start_time)
        {
            GetTimeAndDateContent(&dateTimeStr,arraycallInfo->call_info[i].call_start_time);
            item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
            item_data.item_content[1].item_data.text_buffer.wstr_ptr = dateTimeStr.wstr_ptr;
            item_data.item_content[1].item_data.text_buffer.wstr_len = dateTimeStr.wstr_len;
        }
                                          ...
                                          )
{
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
            ...
            GUILIST_SetListHighlightImage(WATCHCL_CALLLOG_LIST_CTRL_ID, image_watch_list_highlight_bg);
...
#endif
            WATCHCOM_DisplayTitleEx(win_id,STXT_MAIN_CL,40,MMI_WHITE_COLOR,ALIGN_HVMIDDLE);
