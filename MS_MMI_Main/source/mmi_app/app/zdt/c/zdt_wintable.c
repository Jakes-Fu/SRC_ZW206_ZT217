...
{
  GUI_POINT_T         dis_point = DP2PX_POINT(0,56);
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID,GUI_BLOCK_MAIN};
	...
	GUIRES_DisplayImg(&dis_point,PNULL,PNULL,win_id,IMAGE_ZDT_NUMBER_BG,&lcd_dev_info);
    for(i=0;i<12;i++)
    {
                                            ...
                                            )
{
	...
	case MSG_FULL_PAINT:
	{
	        GUI_RECT_T anim_rect = CHARGING_ANIMATION_RECT;
	        MMI_IMAGE_ID_T img_id_list[7] = 
	        {
    ...
    case MSG_TP_PRESS_UP:
        {
            ...
            if(point.x < MMI_MAINSCREEN_WIDTH/3 && point.y > MMI_MAINSCREEN_HEIGHT/2)
            {
              click_count += 1;
            }
            if(click_count >= 9)
...
LOCAL void DrawClassModeTime(MMI_WIN_ID_T win_id)
{
    ...
    uint16  mmi_main_time_y = DP2PX_VALUE_Y(124);//(MMI_MAINSCREEN_HEIGHT - img_height)/2 -1;
