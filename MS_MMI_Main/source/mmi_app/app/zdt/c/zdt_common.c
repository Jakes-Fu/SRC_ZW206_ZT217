...
uint16 ZDT_uint16_to_Str16(
                           uint16*  wstr_ptr, //[IN]  String的源指针
                           uint16  wstr_len, //[IN]
                           uint8*  dest_ptr, //[OUT] String的目标指针
                           uint16 dest_max_len
                           )
{
    ...
    if((wstr_len*4) > dest_max_len)
    {
        wstr_len = dest_max_len/4;
    }
    for(i = 0; i < wstr_len; i++)
    {
        high = wstr_ptr[i]>>8;
        dest_ptr[j++] = ZDT_Nibble2HexChar(high>>4);
        dest_ptr[j++] = ZDT_Nibble2HexChar(high&0x0F);
        low = wstr_ptr[i] & 0x00FFF;
        dest_ptr[j++] = ZDT_Nibble2HexChar(low>>4);
        dest_ptr[j++] = ZDT_Nibble2HexChar(low&0x0F);
    }
    dest_ptr[j] = 0;
    return j;
}

uint16 ZDT_UTF8_to_UCS_Str16(
                           uint8*  src_ptr, //[IN]  String的源指针
                           uint16  src_len, //[IN]  源String的长度，byte数
                           uint8*  dest_ptr, //[OUT] String的目标指针
                           uint16 dest_max_len
                           )
{
    ...
    uint32 ucs_len = 0;
    len = src_len*2;
    tmp_buf = (uint16 *)SCI_ALLOC_APPZ(len+2);
    ucs_len = GUI_UTF8ToWstr(tmp_buf,src_len, src_ptr, src_len);
    tmp_len = ZDT_uint16_to_Str16(tmp_buf,ucs_len,dest_ptr,dest_max_len);
    SCI_FREE(tmp_buf);
    return tmp_len;
}
...
#define MAX_REMAIN_SIZE  (50*1024) //50K
BOOLEAN ZDT_File_MemFull(BOOLEAN is_tf)
{
    uint32          free_space_high     = 0;
    uint32          free_space_low      = MAX_REMAIN_SIZE;//50k 语音是在服务器端本地只存连接 102400;
