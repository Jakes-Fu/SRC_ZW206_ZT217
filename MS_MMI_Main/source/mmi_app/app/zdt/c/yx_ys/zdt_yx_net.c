...
{
    uint8 send_buf[1048] = {0};// deng, wuxx add. 加大数组，避免NET SMS 数组超出死机,表现为来电挂断或者长短信死机 300--1048
    uint8 sms_buf[1025] = {0};// deng, wuxx add. 加大数组，避免NET SMS 数组超出死机,表现为来电挂断或者长短信死机 256--1025
    uint16 send_len = 0;
    uint32 send_hande = 0;
    YX_APP_T * pMe = &g_yx_app;
    if(pMe->m_yx_is_reg == FALSE)
    ...
    send_len = strlen((char *)send_buf);
    U8_To_Hex(sms,sms_size,sms_buf,1024);// deng, wuxx add. 加大数组，避免NET SMS 数组超出死机,表现为来电挂断或者长短信死机 256--1024
    ZDT_LOG("YX_Net_Send_SMS sms=%s len:%d",sms_buf, send_len);
    //SCI_MEMCPY(send_buf+send_len,sms,sms_size);
    sprintf(send_buf+send_len,"%s",sms_buf);
    ZDT_LOG("YX_Net_Send_SMS buf=%s len:%d",send_buf,strlen((char *)send_buf));
    send_len =  strlen((char *)send_buf);
