...
#define YX_VOICE_RECORD_TIME_OUT 11
...
#if 1
YX_GROUP_INFO_DATA_T m_vchat_all_group_info_arr[YX_DB_APPUSER_MAX_SUM+YX_DB_FRIEND_MAX_SUM+1] = {0};
...
uint16 m_vchat_all_group_info_index[YX_DB_APPUSER_MAX_SUM+YX_DB_FRIEND_MAX_SUM+1] = {0};
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        for(i = 0 ; i < SCI_STRLEN(buffer) ; i++)
        {
            ...
            else
            {
                ...
                if(line_index >= MAX_YX_VOC_SAVE_SIZE)
                {
                    break;
                }
            }
...
PUBLIC void YX_VCHAT_GetCurGroupInfo_Status(uint8 * status_fullname,YX_GROUP_INFO_DATA_T * p_group_info)
{
    ...
    SCI_MEMSET(p_group_info->status_arr,0,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
    if(ZDT_File_Read((const uint8*)status_fullname , p_group_info->status_arr, (sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1), &read_len))
    {
...
BOOLEAN YX_VCHAT_GetAllGroupInfo(void)
{
    ...
    uint16 j = 0;
    uint16 k = 0;
    SCI_MEMSET(m_vchat_all_group_info_arr,0,sizeof(YX_GROUP_INFO_DATA_T)*(YX_DB_APPUSER_MAX_SUM+YX_DB_FRIEND_MAX_SUM+1));
    ...
    {
        if(yx_DB_AppUser_Reclist[i].db_id != 0 && SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_id) > 0)
        {
            YX_VCHAT_GetCurGroupInfo(yx_DB_AppUser_Reclist[i].appuser_id,&m_vchat_all_group_info_arr[j]);
            SCI_MEMCPY(m_vchat_all_group_info_arr[j].group_name,yx_DB_AppUser_Reclist[i].appuser_name,SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_name));
            SCI_MEMCPY(m_vchat_all_group_info_arr[j].group_num,yx_DB_AppUser_Reclist[i].appuser_num,SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_num));
            m_vchat_all_group_info_arr[j].user_type = YX_GROUP_USER_BIND;
    ...
    for ( i = 0; i < YX_DB_FRIEND_MAX_SUM; i++ )
    {
        if(yx_DB_Friend_Reclist[i].db_id != 0 && SCI_STRLEN(yx_DB_Friend_Reclist[i].friend_id) > 0)
        {
            ...
            m_vchat_all_group_info_arr[j].user_type = YX_GROUP_USER_FRIEND;
            m_vchat_all_group_info_index[j] = i+1+YX_DB_APPUSER_MAX_SUM;
            j++;
        }
    }
...
YX_GROUP_INFO_DATA_T * YX_VCHAT_GetGroupInfoByID(uint8 * group_id)
{
    uint32 len = 0;
    uint16 i = 0;
    if(group_id != PNULL)
...
BOOLEAN YX_VCHAT_UpdateGroupInfoByFileName(uint8 * full_name,uint8 * buffer,uint32 buf_len,YX_VOC_STATUS_DATA_T * status_line)
{
    YX_GROUP_INFO_DATA_T * pInfo =  YX_VCHAT_GetGroupInfoByFileName(full_name);
    if(pInfo != PNULL)
    {
        SCI_MEMSET(pInfo->status_arr,0,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
        SCI_MEMCPY(pInfo->status_arr,status_line,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
...
uint32 YX_VCHAT_GetGroupUnread(YX_GROUP_INFO_DATA_T * group_info)
{
    ...
    {
        if(group_info->status_arr[i].is_read != 1)
        {
            unread_sum++;
        }
    }
...
uint32 YX_VCHAT_GetAllGroupUnread()
{
    uint16 i=0;
    uint32 unread_sum = 0;
    ...
    for ( i = 0; i < m_vchat_all_group_sum; i++ )
    {
        ...
        unread_sum += YX_VCHAT_GetGroupUnread(pGroupInfo);
    }
    return unread_sum;
}   
...
static BOOLEAN YX_VCHAT_RCV_GetTxtFileName(YX_APP_T * pMe,uint16 voc_idx,uint8 * utf8_group_id)
{
	uint32 time_ms = ZDT_Get_TimeSecond();
    uint8 * rcv_group_patch = YX_VCHAT_GetGroup_Patch(utf8_group_id);
    ...
    if(rcv_group_patch != PNULL)
    {
        ZDT_LOG("YX_VCHAT_RCV_GetTxtFileName voc_idx=%d,time=%3.0lf,%s",voc_idx,time_ms,rcv_group_patch);
        SCI_MEMSET(pMe->m_yx_pFileRcvVocName, 0, MAX_YX_VOC_FILENAME_SIZE+1);

        sprintf( pMe->m_yx_pFileRcvVocName,
                 "%s\\R%d_%ld.txt",
                 rcv_group_patch,voc_idx,time_ms);
        ZDT_LOG("YX_VCHAT_RCV_GetTxtFileName %s",pMe->m_yx_pFileRcvVocName);
        return TRUE;
    }
    return FALSE;
}
...
BOOLEAN YX_VCHAT_CheckOneGroupUser(uint8 * utf8_group_id)
{
    ...
    for ( i = 0; i < YX_DB_APPUSER_MAX_SUM; i++ )
    {
        if(yx_DB_AppUser_Reclist[i].db_id != 0 && SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_id) > 0)
        {
            if(strcmp(yx_DB_AppUser_Reclist[i].appuser_id,utf8_group_id) == 0)
            {
                res = TRUE;
                break;
            }
...
BOOLEAN YX_VCHAT_CheckOneGroupUserApp(uint8 * utf8_group_id)
{
    BOOLEAN res = FALSE;
    uint16 i = 0;
    if(utf8_group_id == PNULL)
...
BOOLEAN YX_VCHAT_CheckOneGroupUserFriend(uint8 * utf8_group_id)
{
    BOOLEAN res = FALSE;
    uint16 i = 0;
    if(utf8_group_id == PNULL)
...
BOOLEAN YX_VCHAT_CheckTxtExsit(uint8 * group_id,char * msg)
{
    uint16 i = 0;
    char * p = PNULL;
    uint8 cur_status = 0;
    YX_GROUP_INFO_DATA_T * pInfo = YX_VCHAT_GetGroupInfoByID(group_id);
    ZDT_LOG("YX_VCHAT_CheckTxtExsit group_id=%s",group_id);
    if(pInfo != PNULL)
    {
        SCI_TRACE_LOW("YX_VCHAT_CheckTxtExsit pInfo->file_num=%d",pInfo->file_num);
        for(i = 0; i < pInfo->file_num; i++)
        {
            cur_status = pInfo->status_arr[i].msg_type;
            if(cur_status == 0  && YX_VOC_IsRcvFile(pInfo->file_arr[i].fullname))
            {
                ZDT_LOG("YX_VCHAT_CheckTxtExsit TRUE cur_status=%d",cur_status);
                return TRUE;
            }
        }
    }
...
PUBLIC void YX_VocFileCheckAndRemoveOverItem(uint8 * full_filename)
{
    ...
    YX_VOC_STATUS_DATA_T line_status[MAX_YX_VOC_SAVE_SIZE+1] = {0};
    YX_VOC_STATUS_DATA_T valid_line_status[MAX_YX_VOC_SAVE_SIZE+1] = {0};
    ...
    pInfo =  YX_VCHAT_GetGroupInfoByFileName(full_filename);
    ...
    if(ZDT_File_Read((const uint8*)status_file_name , line_status, (sizeof(YX_VOC_STATUS_DATA_T) * (MAX_YX_VOC_SAVE_SIZE+1)), &read_len))
    {
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        ...
        if(valid_line_count > MAX_YX_VOC_SAVE_SIZE)
        {
            ...
            for(i = del_item_count; i < valid_line_count; i++)
            {
                valid_line_status[l++] = line_status[valid_line[i]];
            }
            if(pInfo != PNULL)
            {
                pInfo->file_num = MAX_YX_VOC_SAVE_SIZE; //解决越界死机问题
            }
            YX_VocFileStatusWrite(status_file_name,valid_line_status);
            YX_VocFilePathSaveFullBufToTxt(full_filename,buffer2, j);
            YX_VCHAT_UpdateGroupInfoByFileName(full_filename,buffer2,j,valid_line_status);
        }
        else if(valid_line_count != line_index)
        {
            j = 0;
            for(i = 0; i < valid_line_count; i++)
            {
                 SCI_MEMCPY(buffer2 + j, line_data[valid_line[i]], SCI_STRLEN(line_data[valid_line[i]]));
                 j += SCI_STRLEN(line_data[valid_line[i]]);
                 buffer2[j] = '\n'; //next line
            ...
            for(i = 0; i < valid_line_count; i++)
            {
                if(i < MAX_YX_VOC_SAVE_SIZE)
                {
                    valid_line_status[i] = line_status[valid_line[i]];
                }
            }
            if(pInfo != PNULL)
            {
                pInfo->file_num = valid_line_count; //解决越界死机问题
            }
            YX_VocFileStatusWrite(status_file_name,valid_line_status);
            YX_VocFilePathSaveFullBufToTxt(full_filename,buffer2, j);
            YX_VCHAT_UpdateGroupInfoByFileName(full_filename,buffer2,j,valid_line_status);
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        ...
        if(idx < line_index)
        {
            ...
            if(j == 0)
            {
                ...
                SCI_MEMSET(m_pCurGroupInfo->status_arr,0,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
            }
            else
            {
                YX_VOC_STATUS_DATA_T  status_arr[MAX_YX_VOC_SAVE_SIZE+1] = {0};
                if(idx != 0)
                {
                    SCI_MEMCPY(status_arr,m_pCurGroupInfo->status_arr,(sizeof(YX_VOC_STATUS_DATA_T) * idx));
                }
                SCI_MEMCPY(&status_arr[idx],&(m_pCurGroupInfo->status_arr[idx+1]),(sizeof(YX_VOC_STATUS_DATA_T) * (MAX_YX_VOC_SAVE_SIZE-idx)));
                YX_VocFilePathSaveFullBufToTxt(m_pCurGroupInfo->filename.fullname,buffer2, j);
                //YX_VocFileStatusWrite(status_arr);
                SCI_MEMSET(m_pCurGroupInfo->status_arr,0,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
                SCI_MEMCPY(m_pCurGroupInfo->status_arr,status_arr,(sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1));
                YX_VocFileStatusWrite(m_pCurGroupInfo->status_name.fullname,status_arr);
...
LOCAL void YX_VocFilePathSaveToTxt(char* data_full_name,uint8 * friend_id,uint8 msgType,uint8 druation)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16  tmp_len = 0;
    ...
    ZDT_LOG("YX_VocFilePathSaveToTxt(%d-%d) %s",msgType,druation,data_full_name);
    ...
    if(pInfo != PNULL)
    {
        ...
        if(msgType == 0) //文本
        {
            pInfo->status_arr[pInfo->file_num].is_read = 0;
        }
        else if(msgType == 1) //语音
        {
            ...
            {
                pInfo->status_arr[pInfo->file_num].is_read = 1;
            }
        }
        else if(msgType == 7)//表情
        {
            if(YX_VOC_IsRcvFile(data_full_name))
            {
                pInfo->status_arr[pInfo->file_num].is_read = 0;
        ...
        pInfo->file_num++;
        YX_VocFileStatusWrite(pInfo->status_name.fullname,pInfo->status_arr);
    }
...
BOOLEAN YX_DB_Voc_Init(YX_APP_T* pMe)
{
    ...
    YX_VocFilePathSaveToTxt(full_path_name,"111",0,0);
    time_ms++;
    
    sprintf( full_path_name,"D:\\vChat\\common\\R2_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"222",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name,"200",1,13);
    time_ms++;
    
    sprintf( full_path_name,"D:\\vChat\\common\\S1_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"111",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name,"111",1,14);
    time_ms++;
    
    sprintf( full_path_name,"D:\\vChat\\common\\S2_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"57287A9762374E0A90025F53653E653E",32,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name,"222",0,0);
    time_ms++;
     sprintf( full_path_name,"D:\\vChat\\222\\S3_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"333",3,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name,"333",1,6);
    time_ms++;
    sprintf( full_path_name,"D:\\vChat\\common\\S4_%ld.amr",time_ms);
    ZDT_File_Remove(full_path_name);
    ZDT_File_Save(full_path_name,"1",1,FALSE);
    YX_VocFilePathSaveToTxt(full_path_name,"444",7,0);//表情
    time_ms++;
    return TRUE;
}
#endif
...
extern void YX_VocFileStatusWrite(uint8 * full_name ,YX_VOC_STATUS_DATA_T *ui_line_status)
{
    ...
    tmp_len = GUI_GBToWstr(full_path, (const uint8*)full_name, SCI_STRLEN(full_name));
    full_path_len = tmp_len;    
    ...
    if(ZDT_File_Write(file_handle, ui_line_status, (sizeof(YX_VOC_STATUS_DATA_T) * (MAX_YX_VOC_SAVE_SIZE+1))))
    {
        ZDT_LOG("YX_VocFileStatusWrite success msg_type=%d,is_read=%d",ui_line_status->msg_type,ui_line_status->is_read);
    }
    else
    {
    ...
    ZDT_LOG("YX_VocFileStatusWrite ui_line_status=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
        ui_line_status[0].msg_type,ui_line_status[1].msg_type,ui_line_status[2].msg_type,ui_line_status[3].msg_type,ui_line_status[4].msg_type,ui_line_status[5].msg_type,ui_line_status[6].msg_type,ui_line_status[7].msg_type,ui_line_status[8].msg_type,ui_line_status[9].msg_type,ui_line_status[10].msg_type);
}
...
PUBLIC uint32 YX_VocReadStatusCheck(uint8 * utf8_group_id)
{
    ...
    if(buffer && SCI_STRLEN(buffer) > 0)
    {
        ...
        if(line_index > 0)
        {
            ...
            if(ZDT_File_Read((const uint8*)cur_filestatus , cur_line_status, (sizeof(YX_VOC_STATUS_DATA_T) * MAX_YX_VOC_SAVE_SIZE+1) , &read_len))
            {
                for(i = 0 ; i < line_index ; i++)
                {
                    if(cur_line_status[i].is_read != 1)
                    {
                        unread_sum++;
                    }
...
uint8 YX_Net_GetVoiceDration(YX_APP_T *pMe)
{
    uint8 dration = (YX_VOICE_RECORD_TIME_OUT-pMe->m_yx_voc_send_times+1);
    if(dration > 10)
    {
        dration = 10;
    }
...
static BOOLEAN YX_VOC_Send_Link_Add(YX_APP_T *pMe, uint8 * filename, uint32 name_len, uint8 msg_type)
{
...
uint16 YX_VOC_Get_Next_Index(void)
{
    s_yx_voc_rcv_idx ++;
    ...
    return s_yx_voc_rcv_idx;
}

uint16 YX_VOC_Get_Cur_Index(void)
{
    return s_yx_voc_rcv_idx;
}

int  YX_VOC_TxtFile_Save(YX_APP_T *pMe,uint8 * msg, uint16 msg_len,uint8 * friend_id)
{
    int res = AEE_SUCCESS;
    MMIFILE_HANDLE file_handle;
    uint8 pack_sum = 0;
    uint16 i = 0;
    uint16 cur_index = YX_VOC_Get_Next_Index();
    
    YX_VCHAT_RCV_GetTxtFileName(pMe,cur_index,friend_id);
    ZDT_File_Remove(pMe->m_yx_pFileRcvVocName);
    file_handle = ZDT_File_Open(pMe->m_yx_pFileRcvVocName,FALSE);
    if(file_handle == SFS_INVALID_HANDLE)
    {
    ...
    if(ZDT_File_Write(file_handle, msg, msg_len)==0)
    {
        ZDT_File_Close(file_handle);
        return AEE_ENOMEMORY;
    }
...
{        
    ZDT_LOG("YX_Voice_RecordStop_CallBack err_id =%d",err_id);
    if(err_id != YX_VOCRCDSTOP_SUCCESS)
    {
        MMIZDT_UpdateVchatSendWait_Win(1);
    }
    return TRUE;
}
...
{
    ZDT_LOG("YX_Voice_Record_CallBack err_code =%d",err_code);
    if(YX_VOCRECORD_SUCCESS != err_code)
    {
        MMIZDT_TinyChatRecordCallBack(FALSE,err_code);
    }
    return TRUE;
}
...
{
    ZDT_LOG("YX_Voice_Send_CallBack err_code =%d",err_code);
    if(YX_VOCSEND_SUCCESS == err_code)
    {
        YX_VocFilePathSaveToTxt(pMe->m_yx_pFileSendVocName,PNULL,1,YX_Net_GetVoiceDration(pMe));
        MMIZDT_UpdateVchatSendWait_Win(0);
    }
    else
    {
        MMIZDT_UpdateVchatSendWait_Win(2);
    }
    return TRUE;
}
...
PUBLIC void YX_Voice_Record_HandleEnd(BOOLEAN is_succeed)
{
    ...
    if(is_succeed)
    {
        ...
        { 
            if(YX_VOC_Send_Link_Add(pMe,(uint8 *)pMe->m_yx_pFileSendVocName,MAX_YX_VOC_FILENAME_SIZE, 0))
            {
                YX_API_Record_Send();
...
PUBLIC void YX_Voice_Record_HandleCancel()
{
    YX_APP_T *   pMe = (YX_APP_T *)&g_yx_app;   
    ZDT_LOG("YX_Voice_Record_HandleEnd canel");
    YX_Voice_RecordStop_TimerStop(pMe);
    if (PNULL != pMe->m_yx_pRecordMedia)
    {
        MMIRECORDSRV_StopRecord(pMe->m_yx_pRecordMedia);
						  ...
						  MMIRECORD_SRV_RESULT_E result)
{
	...
	{			
            ...
            if(g_yx_app.m_yx_eRecorderStatus == YX_RECOREDER_CANCEL) //取消
            {
                MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_RECORD_CANCEL,NULL,0);
                return;
            }
...
PUBLIC void YX_Net_Send_Emoji_Success()
{
    ...
    {
        sprintf(data,"%d",cur_filedata.emoji_index);
        ZDT_File_Remove((char *)cur_filedata.pFilename);
        ZDT_File_Save(cur_filedata.pFilename,data,strlen(data),FALSE);
    }
    if(YX_VOC_Send_Link_Get(pMe,&cur_filedata))
    {
        YX_VOC_Send_Link_DelOne(pMe);
    }
    YX_VocFilePathSaveToTxt(pMe->m_yx_pFileSendVocName,PNULL,7,0);
}
...
static int YX_Net_Send_TK_VocFile_CB(void *pUser,uint8 * pRcv,uint32 Rcv_len)
{
    ...
    if(cont_len > 0)
    {
        ...
        {
            ZDT_LOG("YX_Net_Send_TK_VocFile_CB Content_len:%d content:%s",Content_len, pContent);
            if(Content_len == 0) //表情回复 --感觉回复数据定义不合理
            {
                MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_EMOJI_SEND_SUCCESS,NULL,0);
            ...
            if(rsp_val == -10)
            {
                ...
                ret_val = YX_Func_GetNextPara(&pContent, &Content_len,temp_str,500);
                if(ret_val > 0 && YX_VOC_Send_Link_Get(pMe,&cur_filedata))
                {
                    ZDT_File_Remove((char *)cur_filedata.pFilename);
        ...
        else if(ret == 3 &&  strncmp( (char *)buf, "TK2", ret ) == 0)
        {
            ...
            {
                MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_EMOJI_SEND_SUCCESS,NULL,0);
                ZDT_LOG("YX_Net_Send_TK2_VocFile_CB emoji Success");
                return AEE_SUCCESS;
            }
        ...
        else if(ret == 3 &&  strncmp( (char *)buf, "STK", ret ) == 0)
        {
            if(Content_len == 0) //表情回复 --感觉回复数据定义不合理
            {
                //YX_Net_Send_Emoji_Success();            
                MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_EMOJI_SEND_SUCCESS,NULL,0);
                ZDT_LOG("YX_Net_Send_STK_VocFile_CB emoji Success");
                return AEE_SUCCESS;
            }
            ret_val = YX_Func_GetNextPara(&pContent, &Content_len,buf,20);
            ret_val = YX_Func_GetNextPara(&pContent, &Content_len,val_buf,10);
            ...
            {
                ZDT_LOG("YX_Net_Send_STK_VocFile_CB Fail");
                return AEE_EFAILED;
            }
        } 
    }
...
int32 YX_Net_Send_TK_VocFile_Start(YX_APP_T *pMe)
{
    ...
    BOOLEAN is_appuser_send = FALSE;
    uint8 drution_buf[20] = {0};
    uint32 drution_buf_len = 0;
    ...
    {
        YX_GROUP_INFO_DATA_T * yx_group_info = YX_VCHAT_GetGroupInfoByID(id_str);
        if(yx_group_info->user_type == YX_GROUP_USER_BIND)
        {
            is_appuser_send = TRUE;
        }
    }
    ...
    sprintf(drution_buf,"%d,",YX_Net_GetVoiceDration(pMe));
    drution_buf_len = SCI_STRLEN(drution_buf);
    
    if(is_common_send)
    {
        content_len = content_len + drution_buf_len + 3;
    }
    else
    {
        content_len = content_len + drution_buf_len + id_len+5;
    }
#if FEATURE_YX_SEND_MD5
    ...
    if(is_common_send)
    {
        j = len+8+drution_buf_len;
    }
    else
    {
        j = len+drution_buf_len+10+id_len;
    }
    for(i = 0; i < cur_len; i++)
    {
        if(tmp_buf[i] == 0x7D)
    ...
    if(is_common_send)
    {
        ...
        cur_data->str[len++] = 'K';
        cur_data->str[len++] = ',';
        SCI_MEMCPY(cur_data->str+len,drution_buf,drution_buf_len);
        len += drution_buf_len;
    }
    else
    {
...
int32 YX_Net_Send_TK_Emoji(YX_APP_T *pMe)
{
    ZDT_TCP_LINK_DATA_T * cur_data = NULL;
    uint32 len = 0;
    uint32 msg_len = 0;
    ...
    if(FALSE == YX_VOC_Send_Link_Get(pMe,&cur_filedata))
    {
        ZDT_LOG("YX_Net_Send_TK_Emoji error nor emoji send");
        return 0;
    }
    ...
    if(file_name == NULL)
    {
        ZDT_LOG("YX_Net_Send_TK_Emoji: ERR Name");
        return -1;
    }
    id_len = YX_VCHAT_GetGroupIDByFileName(file_name,id_str);
    if(id_len == SCI_STRLEN(YX_VCHAT_DEFAULT_GROUP_ID) && strncmp(id_str,YX_VCHAT_DEFAULT_GROUP_ID,id_len) == 0)
    ...
    if(cur_data == NULL)
    {
        ZDT_LOG("YX_Net_Send_TK_Emoji: ERR Malloc 2");
        if(YX_VOC_Send_Link_Get(pMe,&cur_filedata))
        {
            YX_VOC_Send_Link_DelOne(pMe);
        }
    ...
    {
        content_len = content_len + id_len;
    }
#if FEATURE_YX_SEND_MD5
    buf_len = content_len+g_yx_imei_len+43;
#else
    ...
    if(cur_data->str == NULL)
    {
        ZDT_LOG("YX_Net_Send_TK_Emoji: ERR Malloc buf_len=%d",buf_len);
        SCI_FREE(cur_data);
        if(YX_VOC_Send_Link_Get(pMe,&cur_filedata))
        {
            YX_VOC_Send_Link_DelOne(pMe);
    ...
    cur_data->str[len++] = '4';
    cur_data->str[len++] = ',';
    sprintf(data_str,"%d",cur_filedata.emoji_index);
    SCI_MEMCPY(cur_data->str+len,data_str,strlen(data_str));
    len += strlen(data_str);
	file_len = strlen(data_str);

    //包长度
    sprintf(len_str,"%04X",len-msg_len);
    SCI_MEMCPY(cur_data->str+(msg_len-5),len_str,4);
    ...
    ZDT_API_TCP_SendDataNow(pMe->m_yx_tcp_reg_interface,cur_data);
    //pMe->m_yx_voc_is_sending = TRUE;
    ZDT_LOG("YX_Net_Send_TK_Emoji Start num=%d/%d,data_idx=%d,seek=%d,file_len=%d",\
        pMe->m_yx_voc_pack_idx,pMe->m_yx_voc_pack_num,pMe->m_yx_voc_data_idx,pMe->m_yx_voc_file_seek,file_len);
    return cur_len;
}
...
int YX_Net_Receive_TK_VocFile(YX_APP_T *pMe,uint8 * pFiledata,uint16 DataLen,uint8 * friend_id,int msgType)
{
    int result = 0;
    
    if(pFiledata != NULL && DataLen > 0)
    {
        uint8 pack_num;
        uint8 pack_idx;
        uint16 voc_idx;
        ...
        {            
            ...
            {
                result = 1;
                YX_VocFilePathSaveToTxt(pMe->m_yx_pFileRcvVocName,friend_id,msg_type,druation);
...
int YX_Net_Receive_TK2_VocFile(YX_APP_T *pMe,uint8 * pFiledata,uint16 DataLen,uint8 * friend_id,int msgType)
{
    int result = 0;
    ...
    if(pFiledata != NULL && DataLen > 0)
    {
        ...
        ZDT_LOG("YX_Net_Receive_TK2_VocFile s_yx_voc_rcv_idx=%d",voc_idx);
...
int YX_Net_Receive_STK_VocFile(YX_APP_T *pMe,uint8 * pFiledata,uint16 DataLen,uint8 * friend_id,int msgType)
{
    int result = 0;
    ...
    if(pFiledata != NULL && DataLen > 0)
    {
        ...
        ZDT_LOG("YX_Net_Receive_STK_VocFile s_yx_voc_rcv_idx=%d",voc_idx);
        ...
        if(voc_rcv_data.voc_data == NULL)
        {
            ZDT_LOG("YX_Net_Receive_STK_VocFile Err SCI_ALLOC_APPZ Len=%d",DataLen);
            return 0;
        }
        ...
        if(FALSE == YX_VOC_RcvFile_Link_Add(pMe,&voc_rcv_data))
        {
            ZDT_LOG("YX_Net_Receive_STK_VocFile ERR LinkAdd");
            return 0;
        }
        ...
        {            
            ...
            else
            {
                ...
                ZDT_LOG("YX_Net_Receive_STK_VocFile ERR FileSave res=%d,voc_idx=%d",res,voc_idx);
            }
            YX_VOC_RcvFile_Link_DelByVocIdx(pMe,voc_idx);
        }
        YX_Voice_File_Rcv_TimerStart(pMe);
    }
    ZDT_LOG("YX_Net_Receive_STK_VocFile result=%d",result);
    return result;
}
...
BOOLEAN YX_Voice_Record_Cancel(YX_APP_T * pMe)
{
    ZDT_LOG("YX_Voice_Record_Cancel");
    if(pMe->m_yx_eRecorderStatus == YX_RECOREDER_RECORD)
    {
        pMe->m_yx_eRecorderStatus = YX_RECOREDER_CANCEL;
        YX_Voice_Record_TimerStop(pMe);
        if(pMe->m_yx_pRecordMedia)
        {
            YX_Voice_RecordStop_TimerStart(pMe);
...
int YX_API_Emoji_Send(uint8 emoji_index)
{
    ...
    if(FALSE == build_file)
    {
        ZDT_LOG("YX_API_Emoji_Send ERR FileName");
        return FALSE;
    }
    if(YX_VOC_Send_Link_Add(pMe,(uint8 *)pMe->m_yx_pFileSendVocName,MAX_YX_VOC_FILENAME_SIZE, 7))
    {
        //MMIZDT_UpdateVchatSendWait_Win(0);
        pMe->m_yx_voc_send_link_head->data.emoji_index = emoji_index;
        MMIZDT_SendSigTo_APP(ZDT_APP_SIGNAL_YX_EMOJI_SEND,NULL,0);
    }
    else
    {
        ZDT_LOG("YX_API_Emoji_Send ERR FileName:%s",pMe->m_yx_pFileSendVocName);
        return FALSE;
    }
    return AEE_SUCCESS;
}
...
BOOLEAN YX_API_Record_FileDelAll(YX_APP_T * pMe)
{
    ...
    for(i = 0; i < YX_DB_APPUSER_MAX_SUM; i++)
    {
        if(yx_DB_AppUser_Reclist[i].db_id != 0 && SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_id) > 0)
        {
            YX_Voice_GroupRemoveAll(pMe,yx_DB_AppUser_Reclist[i].appuser_id);
        }
    }
    for(i = 0; i < YX_DB_FRIEND_MAX_SUM; i++)
    {
