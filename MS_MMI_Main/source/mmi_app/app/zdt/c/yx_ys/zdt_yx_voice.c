...
int YX_Net_Receive_TK_VocFile(YX_APP_T *pMe,uint8 * pFiledata,uint16 DataLen)
{
    ...
    if(pFiledata != NULL && DataLen > 18)
    {
        ...
        {            
		   ...
		   //add by fys 2024/03/28
		   update_chat_unread_number(m_pCurRcvGroupInfo->group_id, TRUE, 0);//来新消息
		   //add end
            }
            else
            {
...
///////////////////////////////////////////////////////////////zmt add by fys//////////////////////////////////////////////////
PUBLIC void * file_data_read(char * file_path,uint32 * data_size)
{
    uint32 file_len = 0;
	uint8 * tmp_buf = PNULL;
	SCI_TRACE_LOW("%s:read file start %s\n", __FUNCTION__, file_path);

	file_len = ZDT_File_GetSize(file_path);
	...
	ZDT_File_Read(file_path, tmp_buf, file_len,data_size);
   
	SCI_TRACE_LOW("%s:read file end %s\n", __FUNCTION__, file_path);
	return tmp_buf;
}

PUBLIC void file_data_write(void * data, uint32 size, char * file_path)
{
	uint16  full_path_len = 0;
    	uint16  tmp_len = 0;
	wchar full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
	...
	tmp_len = GUI_GBToWstr(full_path, (const uint8*)ZMTCHAT_LIST_SAVE_DIR, SCI_STRLEN(ZMTCHAT_LIST_SAVE_DIR));
    	full_path_len = tmp_len;
    	if(!MMIFILE_IsFileExist(full_path, full_path_len))
    	{
    		MMIFILE_CreateDir(full_path, full_path_len);
    	}

	ZDT_File_Save(file_path,data,size,FALSE);

	SCI_TRACE_LOW("%s: write file end %s\n", __FUNCTION__, file_path);
}

PUBLIC ZMT_CHAT_DATA_UNREAD_T * zmtchat_unread_list=PNULL;
...
PUBLIC void load_unread_number(void)
{
	...
	SCI_TRACE_LOW("%s start\n", __FUNCTION__);
	if(zmtchat_unread_list==PNULL)
	{
		json_str=file_data_read(ZMTCHAT_LIST_SAVE_UNREAD_FILE,&file_size);
		SCI_TRACE_LOW("%s file_size %d\n", __FUNCTION__,file_size);

		if(json_str&&file_size>0){
			cJSON * root=cJSON_Parse(json_str);
			cJSON * unread_arr=cJSON_GetObjectItem(root,"unreads");

			if(unread_arr != NULL && unread_arr->type != cJSON_NULL)
			{
				cJSON *item=unread_arr->child;
				ZMT_CHAT_DATA_UNREAD_T * last_chat_unread;
				int count =0;
				int i = 0;
				while (item!=NULL&&count <30)
				{
					ZMT_CHAT_DATA_UNREAD_T * chat_unread;
					cJSON * group_id=cJSON_GetObjectItem(item,"a");
					cJSON * mark_num=cJSON_GetObjectItem(item,"b");
					cJSON * left_num=cJSON_GetObjectItem(item,"c");
					SCI_TRACE_LOW("%s item start\n", __FUNCTION__);
					chat_unread=(ZMT_CHAT_DATA_UNREAD_T*)SCI_ALLOC_APPZ(sizeof(ZMT_CHAT_DATA_UNREAD_T));

					if(group_id != NULL && group_id->type != cJSON_NULL&&strlen(group_id->valuestring)>0)
					{
						int size=strlen(group_id->valuestring)+1;
						memcpy(chat_unread->group_id, group_id->valuestring, size);
					}
					chat_unread->left_num = left_num->valueint;
					if(mark_num != NULL && mark_num->type != cJSON_NULL)
					{
						cJSON * mark_arr=cJSON_GetObjectItem(item,"mark_arr");
						chat_unread->mark_num=mark_num->valueint;
						for(i = 0;i < MAX_YX_VOC_SAVE_SIZE+1;i++)
						{
							cJSON * status = cJSON_GetArrayItem(mark_arr, i);
							if (cJSON_IsNumber(status)) {
								chat_unread->mark[i] = cJSON_GetNumberValue(status);
							}else{
								chat_unread->mark[i] = 0x31;//已读
							}
						}
					}
					else
					{
						chat_unread->mark_num = 0;
						for(i = 0;i < MAX_YX_VOC_SAVE_SIZE+1;i++)
						{
							chat_unread->mark[i] = 0x31;//已读
						}
					...
					if(zmtchat_unread_list==PNULL)
					{
						zmtchat_unread_list=chat_unread;
						last_chat_unread=chat_unread;
					}
					else
					{
						last_chat_unread->next_chat_unread=chat_unread;
						last_chat_unread=chat_unread;
					}
					item=item->next;
					count++;
			...
			SCI_TRACE_LOW("%s no record marks.json\n", __FUNCTION__);
		}
	}
	else
	{
		SCI_TRACE_LOW("%s dslchat_unread_list!=PNULL\n", __FUNCTION__);
	}
	SCI_TRACE_LOW("%s end\n", __FUNCTION__);
}

PUBLIC void save_unread_number(void)
{
	SCI_TRACE_LOW("%s start\n", __FUNCTION__);
	if(zmtchat_unread_list!=PNULL)
	{
		...
		cJSON * unread_arr= cJSON_CreateArray();
		ZMT_CHAT_DATA_UNREAD_T * chat_unread = zmtchat_unread_list;
		cJSON_AddItemToObject(root,"unreads",unread_arr);
		while(chat_unread&&count<30) {
			cJSON * contact_root=cJSON_CreateObject();
			cJSON * group_id= cJSON_CreateString(chat_unread->group_id);
			cJSON * mark_num= cJSON_CreateNumber(chat_unread->mark_num);
			cJSON * left_num= cJSON_CreateNumber(chat_unread->left_num);
			cJSON_AddItemToArray(unread_arr,contact_root);
			cJSON_AddItemToObject(contact_root,"a",group_id);
			cJSON_AddItemToObject(contact_root,"b",mark_num);
			cJSON_AddItemToObject(contact_root,"c",left_num);
			{
				cJSON * mark_arr= cJSON_CreateArray();
				cJSON_AddItemToObject(contact_root,"mark_arr",mark_arr);
				for(i = 0;i < MAX_YX_VOC_SAVE_SIZE+1;i++)
				{
					if(i < chat_unread->mark_num){
						cJSON *status = cJSON_CreateNumber(chat_unread->mark[i]);
						cJSON_AddItemToArray(mark_arr, status);
					}else{
						cJSON *status = cJSON_CreateNumber(0x31);
						cJSON_AddItemToArray(mark_arr, status);
					}
				}
			}
			chat_unread = chat_unread->next_chat_unread;
			count++;
		}
		cout=cJSON_Print(root);
		SCI_TRACE_LOW("%s path %s\n", __FUNCTION__,ZMTCHAT_LIST_SAVE_UNREAD_FILE);
		file_data_write(cout,strlen(cout),ZMTCHAT_LIST_SAVE_UNREAD_FILE);
		free(cout); 
		cJSON_Delete(root);
	}
	SCI_TRACE_LOW("%s end\n", __FUNCTION__);
...
PUBLIC void delete_chat_unread_one_list_left_item(char * group_id, uint8 msg_index)
{
	ZMT_CHAT_DATA_UNREAD_T * chat_unread = zmtchat_unread_list;
	int char_mark[MAX_YX_VOC_SAVE_SIZE+1] = {0x31};
	int count=0;
	int i,j = 0;
	SCI_TRACE_LOW("%s msg_index = %d", __FUNCTION__, msg_index);
	while(chat_unread&&count<30) {
		if(0 == strcmp(chat_unread->group_id, group_id))
		{
			if(get_chat_is_unread(group_id, msg_index)  != 0x31){
				chat_unread->mark_num--;
			}
			chat_unread->left_num--;
			for(i = 0;i < MAX_YX_VOC_SAVE_SIZE+1;i++){
				if(i != msg_index){
					char_mark[j]  = chat_unread->mark[i];
					j++;
				}
			}
			for(i = 0;i < MAX_YX_VOC_SAVE_SIZE+1;i++){
				chat_unread->mark[i] = char_mark[i];
			}
		}
		chat_unread=chat_unread->next_chat_unread;
		count++;
...
PUBLIC void delete_chat_unread_one_list(char * group_id)
{
	ZMT_CHAT_DATA_UNREAD_T * chat_unread = zmtchat_unread_list;
	ZMT_CHAT_DATA_UNREAD_T * new_unread = NULL;
	int count=0;
	SCI_TRACE_LOW("%s start\n", __FUNCTION__);
	while(chat_unread&&count<30) {
		if(0 != strcmp(chat_unread->group_id, m_pCurGroupInfo->group_id))
		{
			new_unread = chat_unread;
		}
		chat_unread=chat_unread->next_chat_unread;
		count++;
	}
	zmtchat_unread_list = new_unread;
	if(zmtchat_unread_list == NULL){
		ZDT_File_Remove(ZMTCHAT_LIST_SAVE_UNREAD_FILE);
	}else{
		save_unread_number();
	}
}
...
{
	ZMT_CHAT_DATA_UNREAD_T * chat_unread = zmtchat_unread_list;
	BOOLEAN is_new_chat=TRUE;
	int count=0;
	SCI_TRACE_LOW("%s is_new_msg = %d, msg_index = %d", __FUNCTION__, is_new_msg, msg_index);
	while(chat_unread&&count<30) {
		if(0 == strcmp(chat_unread->group_id, group_id))
		{
			if(is_new_msg){
				chat_unread->mark[chat_unread->left_num] = 0x00;//来新消息
				chat_unread->mark_num++;
				chat_unread->left_num++;
			}else{
				if(MAX_YX_VOC_SAVE_SIZE+1 > msg_index){
					chat_unread->mark[msg_index] = 0x31;//更新消息为已读
					if(chat_unread->mark_num > 0){
						chat_unread->mark_num--;
					}
				}
			}
	...
	if(is_new_chat){
		ZMT_CHAT_DATA_UNREAD_T * unread;
		unread=(ZMT_CHAT_DATA_UNREAD_T*)SCI_ALLOC_APPZ(sizeof(ZMT_CHAT_DATA_UNREAD_T));
		SCI_TRACE_LOW("%s no exit group_id,try add \n", __FUNCTION__);
		strcpy(unread->group_id,group_id);
		unread->mark[0] = 0x00;//新消息默认为未读
		unread->mark_num++;
		unread->left_num++;
		SCI_TRACE_LOW("%s add group_id:%s,mark_num:%d \n", __FUNCTION__,unread->group_id,unread->mark_num);
		if(zmtchat_unread_list==PNULL)
		{
			SCI_TRACE_LOW("%s no exit unread_list \n", __FUNCTION__);
			zmtchat_unread_list=unread;
		}
		else
		{
			SCI_TRACE_LOW("%s add to unread_list last\n", __FUNCTION__);
			chat_unread = zmtchat_unread_list;
			count++;
			while(chat_unread->next_chat_unread!=PNULL&&count<30)
			{
				chat_unread=chat_unread->next_chat_unread;
				count++;
			}
			chat_unread->next_chat_unread=unread;
		}
		save_unread_number();
	}
	SCI_TRACE_LOW("%s end\n", __FUNCTION__);
...
PUBLIC uint8 get_chat_unread_number(char * group_id)
{
	ZMT_CHAT_DATA_UNREAD_T * chat_unread = zmtchat_unread_list;
	int count=0;
	SCI_TRACE_LOW("%s start\n", __FUNCTION__);
	while(chat_unread!=PNULL&&count<30)
	{
		if(strcmp(group_id, chat_unread->group_id)==0)
		{
			SCI_TRACE_LOW("%s exit group_id:%s group_id:%s \n", __FUNCTION__,group_id,chat_unread->group_id);
			SCI_TRACE_LOW("%s exit group_id mark_num %d\n", __FUNCTION__,chat_unread->mark_num);
			SCI_TRACE_LOW("%s end\n", __FUNCTION__);
			return chat_unread->mark_num;
		}
		chat_unread=chat_unread->next_chat_unread;
		count++;
	}
	SCI_TRACE_LOW("DSLCHAT_WIN %s end\n", __FUNCTION__);
	return 0;
}
...
PUBLIC uint16 get_chat_is_unread(char * group_id, uint8 msg_index)
{
	...
	while(chat_unread!=PNULL&&count<30)
	{
		if(strcmp(group_id, chat_unread->group_id)==0 && MAX_YX_VOC_SAVE_SIZE+1 > msg_index)
		{
			SCI_TRACE_LOW("%s chat_unread->mark[%d] = %d", __FUNCTION__, msg_index, chat_unread->mark[msg_index]);
			return chat_unread->mark[msg_index];
		}
		chat_unread=chat_unread->next_chat_unread;
		count++;
	}
