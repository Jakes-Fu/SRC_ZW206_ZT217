...
//向管理员发送sos短信
int YX_API_SMS_Send_Admin_SOS(void)
{
    uint16 num_len = 0;
    // //的设备向您发出紧急求助报警！7684 8BBE 5907 5411 60A8 53D1 51FA 7D27 6025 6C42 52A9 62A5 8B66 FF01 
    uint16 sms_tail[50] = {0x7684, 0x8BBE, 0x5907, 0x5411, 0x60A8, 0x53D1, 0x51FA, 0x7D27, 0x6025, 0x6C42, 0x52A9, 0x62A5, 0x8B66, 0xFF01, 0x0};
    ...
    MMIAPICOM_Wstrcat(sms_buf,sms_tail);
    send_len = MMIAPICOM_Wstrlen(sms_buf);
    if(yx_DB_White_Reclist[0].db_id != 0)
    {
        num_len = SCI_STRLEN(yx_DB_White_Reclist[0].white_num);
        if(num_len > 0)
        {
            ZDT_LOG("[%s] sms send number:%s",__FUNCTION__,yx_DB_White_Reclist[0].white_num);
            MMIZDT_SMS_AppSend(MN_DUAL_SYS_1,(uint8 *)sms_buf,send_len*2,yx_DB_White_Reclist[0].white_num,num_len,2,3);
        }
    }
#endif
    return AEE_SUCCESS;
