...
#define TINYCHAT_BOTTOM_HEIGHT   DP2PX_VALUE(64)    //52  //xiongkai 58//120
...
void MMIZDT_TinyShowBottom(MMI_WIN_ID_T  win_id)
{
    ...
    GUI_FONT_T f_big = DP_FONT_26;//DP_FONT_22;//DP_FONT_19;//DP_FONT_34;
    GUI_POINT_T      point = {0};
...
LOCAL void MMIZDT_DisplayTinyChatRecordAnim(MMI_WIN_ID_T win_id)
{
    ...
    GUISTR_STYLE_T      text_style      = {0};  
    GUISTR_STATE_T      state =         GUISTR_STATE_ALIGN|GUISTR_STATE_WORDBREAK|GUISTR_STATE_SINGLE_LINE; 
    GUI_FONT_T f_big = DP_FONT_30;//DP_FONT_28;//DP_FONT_34;
    ...
    text_style.font = DP_FONT_26;//DP_FONT_22;
    text_style.font_color = MMI_WHITE_COLOR;
...
void MMIZDT_ChatGroup_Win_Wating(MMI_WIN_ID_T win_id)
{
    ...
    {
        title_rect.top =  DP2PX_VALUE(20);
        title_rect.bottom =  DP2PX_VALUE(50);
        cur_rect.top =  DP2PX_VALUE(60);
        cur_rect.bottom =  DP2PX_VALUE(180);
                                            ...
                                            )
{
	...
	GUI_RECT_T rect = {MMI_MAINSCREEN_WIDTH/3,0, (MMI_MAINSCREEN_WIDTH -1), (MMI_MAINSCREEN_HEIGHT -1)};
	MMI_STRING_T cur_str_t	 = {0};
	GUI_RECT_T	  cur_rect			= {0};
	GUI_FONT_T f_big =DP_FONT_42;
       text_style.align = ALIGN_HVMIDDLE;
       text_style.font = f_big;
...
void  MMIZDT_ChatGroup_ShowList(MMI_WIN_ID_T win_id,uint8 * p_friend_id)
{
    ...
    #else
        item_t.item_style    = GUIITEM_STYLE_1ICON_1STR_1LINE_WITH_UNREAD_NUM ;//GUIITEM_STYLE_TWO_LINE_ANIM_TEXT_AND_TEXT_ANIM;
    #endif
        item_t.item_data_ptr = &item_data;
...
{
    WATCHCOM_DisplayTitleWithBg(win_id,TXT_WECHART);
}
...
LOCAL void Chat_Contact_List_Title_Back_Click(MMI_WIN_ID_T win_id, DPARAM param)
{
    ...
    if((point.x > 0 && point.x < DP2PX_VALUE(50))&&(point.y >= 0 &&  point.y < MMI_SPECIAL_TITLE_HEIGHT))
    {
        MMK_CloseWin(win_id);
    }
    if((point.x > DP2PX_VALUE( 190) && point.x <  DP2PX_VALUE(240))&&(point.y >= 0 &&  point.y < MMI_SPECIAL_TITLE_HEIGHT))
    {
        #if 0
...
#define QRCODE_DISP_X    DP2PX_VALUE(40)
#define QRCODE_DISP_Y    DP2PX_VALUE_Y(40)
#define QRCODE_DISP_WIDTH    DP2PX_VALUE(160)
#define QRCODE_DISP_HEIGHT    DP2PX_VALUE(160)
                                            ...
                                            )
{
    ...
    switch(msg_id) {
        case MSG_OPEN_WINDOW:
            ...
            {
                uint16 img_w = DP2PX_VALUE(180);
                dec_src.file_name_ptr = QRCODE_DOWNLOAD_FILE_NAME;
                //yangyu add, not generate
                dis_point.x= (MMI_MAINSCREEN_WIDTH - img_w)/2+5;
                dis_point.y= (MMI_MAINSCREEN_HEIGHT- img_w)/2+2; 
                                            ...
                                            )
{
    ...
    case MSG_FULL_PAINT:
        ...
        text_style.font = DP_FONT_25;
        text_style.align= ALIGN_HVMIDDLE;
        text_style.line_space = 0;
        text_style.region_num = 0;
        text_style.region_ptr = PNULL;        
        text_display_rect.left = 0; // 30;
        text_display_rect.top = DP2PX_VALUE(208);
        text_display_rect.right=(MMI_MAINSCREEN_WIDTH -1);
        text_display_rect.bottom=DP2PX_VALUE(240);
        temp_ptr = (char*)g_zdt_phone_imei_1;
        temp_len = SCI_STRLEN((char*)temp_ptr);
        MMIAPICOM_StrcatFromStrToUCS2(uint16_str, &uint16_str_len, (uint8*)temp_ptr, temp_len);
        imei_string.wstr_ptr = uint16_str;
     ...
     //if(index != m_yx_whitelist_pagenum)
     {
        MMIZDT_PB_DrawBg();
         ...
         for(i=(index)*4;i<(index)*4+4;i++)
         {
            ...
            {
                cur_rect.top = ZDT_PB_ICON_TOP_START+3;
            }
...
#ifndef MAINLCD_DEV_SIZE_128X128 //屏宽度太小 不要电话的图标
                                            ...
                                            )
{
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
            ...
            list_init.both_rect.v_rect.right = (MMI_MAINSCREEN_WIDTH -1);
            list_init.both_rect.v_rect.top = MMI_SPECIAL_TITLE_HEIGHT;
            ...
            list_init.both_rect.h_rect.top = MMI_SPECIAL_TITLE_HEIGHT;
        ...
        case MSG_FULL_PAINT:
                //MMIZDT_PB_Show(win_id);
            {
                WATCHCOM_DisplayTitleWithBg(win_id,TXT_CONTACT);
...
LOCAL void DisplayWeather(MMI_WIN_ID_T win_id)
{
    ...
    LCD_DrawThickLine(&lcd_dev_info,0,DP2PX_VALUE(40),MMI_MAINSCREEN_WIDTH,DP2PX_VALUE(40),MMI_DARK_GRAY2_COLOR,1);
    LCD_DrawThickLine(&lcd_dev_info,0,DP2PX_VALUE(160),MMI_MAINSCREEN_WIDTH,DP2PX_VALUE(160),MMI_DARK_GRAY2_COLOR,1);

  /*  point.x =DP2PX_VALUE( 17);
    point.y = DP2PX_VALUE(8) ;
        ...
        case MSG_TP_PRESS_UP:
            {
                ...
                point.x = MMK_GET_TP_X(param);
                point.y = MMK_GET_TP_Y(param);
                if(wt_tp_down_y - point.y  > DP2PX_VALUE(90))
                {
                    MMK_SendMsg(MMK_ConvertIdToHandle(win_id), MSG_APP_CANCEL, PNULL);//exit to clock screen
                    break;
                }
                else if(ABS(wt_tp_down_x - point.x) > DP2PX_VALUE(40))
                {
                    break;
                }
            }
...
LOCAL MMI_RESULT_E  HandleZDT_NoSimOrDataWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    switch(msg_id) 
    {
        case MSG_OPEN_WINDOW:
            break;
        case MSG_FULL_PAINT:
            GUIRES_DisplayImg(PNULL,&full_rect,PNULL,win_id,res_bg_star,&lcd_dev_info);
...
void MMIZDT_Schedule_ShowList(MMI_WIN_ID_T win_id,uint8 current_day)
{
    ...
    GUI_RECT_T content0_rect = DP2PX_RECT(10,  10,  130 , height);// 上午 12:30
    GUI_RECT_T content1_rect = DP2PX_RECT( 5,  10,  130 , height);// 1
    GUI_RECT_T content2_rect = DP2PX_RECT(130,  10,  240 , height);// 语文
	...
	content_style.content[0].font= content_style.content[0].font_focus = DP_FONT_22;
	content_style.content[0].state = 0;
	content_style.content[1].rect = content_style.content[1].rect_focus = content1_rect;
	content_style.content[1].font= content_style.content[1].font_focus = DP_FONT_22;
	...
	content_style.content[2].font= content_style.content[2].font_focus = DP_FONT_22;
	...
	none_style.content[1].font= none_style.content[1].font_focus = DP_FONT_20;
...
void HandleZDT_FriendPP_ShowTxt(uint16 * w_str1, uint16 * w_str2,uint16 * w_str3)
{
    ...
    if(w_str1 != NULL)
    {
        ...
        cur_rect.top    = 0; 
        cur_rect.bottom = DP2PX_VALUE(46);
    ...
    if(w_str2 != NULL)
    {
        ...
        cur_rect.top    =DP2PX_VALUE( 170); 
        cur_rect.bottom = DP2PX_VALUE(200);
        LCD_FillRect(&lcd_dev_info, cur_rect, MMI_BLACK_COLOR);
        GUISTR_DrawTextToLCDInRect( 
            (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
            (const GUI_RECT_T      *)&cur_rect,       
    ...
    if(w_str3 != NULL)
    {
        ...
        cur_rect.top    = DP2PX_VALUE(201); 
        cur_rect.bottom = DP2PX_VALUE(239);
        LCD_FillRect(&lcd_dev_info, cur_rect, MMI_BLACK_COLOR);
        GUISTR_DrawTextToLCDInRect( 
            (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
            (const GUI_RECT_T      *)&cur_rect,       
