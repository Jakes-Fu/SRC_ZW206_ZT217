...
int YX_RestorFactory(void)
{
    ...
    YX_VOC_RcvFile_Link_DelAll(pMe);
#endif
    YX_DB_DelAll(pMe);  //必须在 YX_API_Record_FileDelAll 后面消除
...
int YX_RestorFactoryByShip(void)
{
    YX_APP_T * pMe = &g_yx_app;
    ZDT_LOG("YX_RestorFactory");
#ifdef ZDT_PLAT_YX_SUPPORT_VOICE
...
{
    BOOLEAN res = FALSE;
    #if defined(ZDT_NO_LOC_LBS_WIFI_OR_GPS_WHEN_STILLNESS)&&defined(ZDT_GSENSOR_SUPPORT)
     // 静止时停止启动周期定位数据上报
    #else
    return TRUE;
    #endif
    ...
    {
        SCI_TraceLow("wuxx, YX_LocReport_Judge,  g_yx_is_jp_change || g_yx_is_lbs_change");
        res = TRUE;
    }
    if(res == FALSE)
    {
        if(g_yx_is_move)
        {
            SCI_TraceLow("wuxx, YX_LocReport_Judge,  g_yx_is_move");
            res = TRUE;
        }
    }
    SCI_TraceLow("wuxx, YX_LocReport_Judge,  03, res=%d", res);
    return res;
}
    ...
    switch (msg_id)
    {
...
#if defined(ZDT_GPS_SUPPORT) || defined(ZDT_WIFI_SUPPORT) ||defined(ZDT_W217_FACTORY_GPS)
            SCI_TraceLow("wuxx, YX_Handle_AppMsg,  YX_LOCTION_TYPE_NET, AAAA");
       ...
       case ZDT_APP_SIGNAL_YX_WT:
#if defined(ZDT_GPS_SUPPORT) || defined(ZDT_WIFI_SUPPORT)  ||defined(ZDT_W217_FACTORY_GPS)// wuxx 20231129 GPS_NEW_STYLE_20231204
                YX_LOCTION_Start(pMe,YX_LOCTION_TYPE_WT,NULL,0);
#else
        ...
        case ZDT_APP_SIGNAL_YX_RECORD_FAIL:
                ...
                break;
        case ZDT_APP_SIGNAL_YX_RECORD_CANCEL:
            if(pMe->m_yx_eRecorderStatus == YX_RECOREDER_CANCEL)
            {
                YX_Voice_Record_HandleCancel();
            }
            else
            {
                YX_Voice_Record_Cancel(pMe);
            }
                break;      
        ...
        case ZDT_APP_SIGNAL_YX_RECORD_STOP:
            ...
            break;
        case ZDT_APP_SIGNAL_YX_EMOJI_SEND:
            YX_Net_Send_TK_Emoji(pMe);
            break;
        case ZDT_APP_SIGNAL_YX_EMOJI_SEND_SUCCESS:
