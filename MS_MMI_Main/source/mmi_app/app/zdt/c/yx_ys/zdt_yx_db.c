...
#elif defined(ZDT_PLAT_YX_SUPPORT_ZTE)// ZTE NUBIA
// ZTE 努比亚后台
#define YX_DB_IP_DOMAIN_URL  "care.nubia.cn" //"watch-tcp-test.nubia.com" //测试服务器
...
BOOLEAN YX_DB_SET_Default(void)
{
    ...
    yx_DB_Set_Rec.band_status = 0;
    #if defined(ZDT_PLAT_YX_SUPPORT_ZTE)// wuxx_20231219 FOR ZTE 
    yx_DB_Set_Rec.time_interval = 1200;
    #else
    yx_DB_Set_Rec.time_interval = 3600;
    #endif
    yx_DB_Set_Rec.net_open = 1;
    yx_DB_Set_Rec.alert_sos_sms = 1;
    yx_DB_Set_Rec.alert_low_bat = 1;
    yx_DB_Set_Rec.alert_jp = 1;
	yx_DB_Set_Rec.is_land = 1;
    yx_DB_Set_Rec.height = 120; //120cm
    yx_DB_Set_Rec.weight = 35; //35kg
    SCI_MEMSET(&yx_DB_Set_Rec.app_on, 1, sizeof(uint8)*MAX_YX_APP_MAX_SIZE);//不合理应该是0显示 1关闭
    return TRUE;
}
...
#ifdef YX_DB_NEED_BIND_STATUS
static BOOLEAN DB_yx_statues_is_init = FALSE;
YX_STATUES_DATA_Type struct_yx_statues_data= {0};

/*****************************************链表数据操作*****************************************************************/
BOOLEAN DB_Yx_Statues_Data_ListInit(void)
{
    uint32 read_byte = 0;
    BOOLEAN is_exsit = 0;
    BOOLEAN res = FALSE;
    uint32 file_size = 0;
    uint32 buf_size = sizeof(YX_STATUES_DATA_Type);
    uint32 buf_array_size = (buf_size);
    
    if(DB_yx_statues_is_init == TRUE)
    {
        return TRUE;
    }
    
    is_exsit = ZDT_DBFile_Exist(YX_DB_DIR,YX_STATUES_DATA_FS_FILE);
    if (is_exsit)
    {
        file_size = ZDT_DBFile_GetSize(YX_DB_DIR,YX_STATUES_DATA_FS_FILE);
        if(file_size != buf_array_size)
        {
            SCI_TRACE_LOW("DB_Yx_Statues_Data_ListInit Filesize = 0x%x, BufSize=0x%x",file_size,buf_array_size);
            ZDT_DBFile_RemoveFile(YX_DB_DIR,YX_STATUES_DATA_FS_FILE);
        }
        else
        {
            is_exsit = TRUE;
    ...
    if (is_exsit == FALSE)
    {
        SCI_MEMSET(&struct_yx_statues_data, 0, buf_array_size);
        res = ZDT_DBFile_Write(YX_DB_DIR,YX_STATUES_DATA_FS_FILE,(uint8 *)&struct_yx_statues_data,buf_array_size);
        if (res == FALSE)
        {
            SCI_TRACE_LOW("DB_Yx_Statues_Data_ListInit ERR DB Write");
        }
    }
    else
    {
        res = ZDT_DBFile_Read(YX_DB_DIR,YX_STATUES_DATA_FS_FILE,(void *)&struct_yx_statues_data,buf_array_size,&read_byte);
        if (res == FALSE)
        {
            SCI_TRACE_LOW("DB_Yx_Statues_Data_ListInit ERR DB Open");
        }
    }
    if(res)
    {
        DB_yx_statues_is_init = TRUE;
    }
    return res;
}

BOOLEAN DB_Yx_Statues_Data_ListDelAll(void)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_STATUES_DATA_Type);
    uint32 buf_array_size = (buf_size);
    
    SCI_MEMSET(&struct_yx_statues_data,0x00,sizeof(YX_STATUES_DATA_Type));
    
    if(DB_yx_statues_is_init == FALSE)
    {
        SCI_TRACE_LOW("DB_Yx_Statues_Data_ListDelAll ERR NoInit");
        return FALSE;
    }
    ...
    if (res == FALSE)
    {
        SCI_TRACE_LOW("DB_Yx_Statues_Data_ListDelAll DB Write");
        return FALSE;
    }		
    return TRUE;
}

BOOLEAN DB_Yx_statues_Data_ListSaveBuf(void)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_STATUES_DATA_Type);
    uint32 buf_array_size = (buf_size);
    ...
    if(DB_yx_statues_is_init == FALSE)
    {
        SCI_TRACE_LOW("DB_Yx_statues_Data_ListSaveBuf ERR NoInit");
        return FALSE;
    }
    ...
    if (res == FALSE)
    {
        SCI_TRACE_LOW("DB_Yx_statues_Data_ListSaveBuf DB Write");
        return FALSE;
    }		
    return TRUE;
}

uint8 YX_MMI_Get_Bingd_Statues(void)
{
	return struct_yx_statues_data.bingd_statues;
}

void YX_MMI_Set_Bingd_Statues(uint8 statues)
{
	struct_yx_statues_data.bingd_statues=statues;
	DB_Yx_statues_Data_ListSaveBuf();
	return;
}
#endif
...
int YX_DB_SOS_IsValidNumber(char * num)
{
    ...
    ZDT_LOG("YX_DB_SOS_IsValidNumber num=%s",num);
    r_len = ZDT_DeleteStrHead(num,num_len,r_num);
    ZDT_LOG("YX_DB_SOS_IsValidNumber r_num=%s",r_num);
    YX_DB_SOS_ListInit();
    for(i = 0; i < YX_DB_SOS_MAX_SUM; i++)
    {
		...
		ZDT_LOG("YX_DB_SOS_IsValidNumber sos_num=%d,db_num_len=%d,r_len=%d,",sos_num,db_num_len,r_len);	
    ...
    for(i = 0; i < YX_DB_WHITE_MAX_SUM; i++)
    {
        if(yx_DB_White_Reclist[i].db_id != 0)
        {
            ...
            if(db_num_len > 0 && r_len == db_num_len)
            {
                ...
                {
                    return yx_DB_White_Reclist[i].db_id;
                }
            }
        }
    }
...
BOOLEAN YX_DB_WHITE_GetNameByNumber(uint8* num, uint16* name)
{
    uint16 num_len = 0;
    uint16 i = 0;
    uint16 db_num_len = 0;
        ...
        if(yx_DB_White_Reclist[i].db_id != 0)
        {
            ...
            if(db_num_len > 0 && r_len == db_num_len)
            {
                if(strncmp(white_num,r_num,r_len) == 0)
                {
                    ...
                    if( name_len > 0)
                    {
                        ZDT_UCS_Str16_to_uint16((uint8*)yx_DB_White_Reclist[i].white_name, name_len ,name, 100);
                        return TRUE;
                    }
                }
            }
...
#ifdef YX_DB_NEED_APPUSER
static BOOLEAN yx_DB_AppUser_is_init = FALSE;
YX_DB_APPUSER_Type yx_DB_AppUser_Reclist[YX_DB_APPUSER_MAX_SUM] = {0};

/*****************************************链表数据操作*****************************************************************/
BOOLEAN YX_DB_APPUSER_ListInit(void)
{
    uint32 read_byte = 0;
    BOOLEAN is_exsit = 0;
    BOOLEAN res = FALSE;
    uint32 file_size = 0;
    uint32 buf_size = sizeof(YX_DB_APPUSER_Type);
    uint32 buf_array_size = (buf_size * YX_DB_APPUSER_MAX_SUM);
    
    if(yx_DB_AppUser_is_init == TRUE)
    {
        return TRUE;
    }
    
    is_exsit = ZDT_DBFile_Exist(YX_DB_DIR,YX_DB_APPUSER_FS_FILE);
    if (is_exsit)
    {
        file_size = ZDT_DBFile_GetSize(YX_DB_DIR,YX_DB_APPUSER_FS_FILE);
        if(file_size != buf_array_size)
        {
            ZDT_LOG("YX_DB_APPUSER_ListInit Filesize = 0x%x, BufSize=0x%x",file_size,buf_array_size);
            ZDT_DBFile_RemoveFile(YX_DB_DIR,YX_DB_APPUSER_FS_FILE);
        }
        else
        {
            is_exsit = TRUE;
    ...
    if (is_exsit == FALSE)
    {
        SCI_MEMSET(yx_DB_AppUser_Reclist, 0, buf_array_size);
        res = ZDT_DBFile_Write(YX_DB_DIR,YX_DB_APPUSER_FS_FILE,(uint8 *)yx_DB_AppUser_Reclist,buf_array_size);
        if (res == FALSE)
        {
            ZDT_LOG("YX_DB_APPUSER_ListInit ERR DB Write");
        }
    }
    else
    {
        res = ZDT_DBFile_Read(YX_DB_DIR,YX_DB_APPUSER_FS_FILE,(void *)yx_DB_AppUser_Reclist,buf_array_size,&read_byte);
        if (res == FALSE)
        {
            ZDT_LOG("YX_DB_APPUSER_ListInit ERR DB Open");
        }
    }
    if(res)
    {
        yx_DB_AppUser_is_init = TRUE;
    }
    return res;
}

BOOLEAN YX_DB_APPUSER_ListModify(uint8 appuser_idx,uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_DB_APPUSER_Type);
    uint32 buf_array_size = (buf_size * YX_DB_APPUSER_MAX_SUM);
    
    if (appuser_idx >= YX_DB_APPUSER_MAX_SUM)
    {
        ZDT_LOG("YX_DB_APPUSER_ListModify(%d) ERR idx>=%d",appuser_idx,YX_DB_APPUSER_MAX_SUM);
        return FALSE;
    }
    	
    yx_DB_AppUser_Reclist[appuser_idx].db_id = appuser_idx+1;
    if(id_len > YX_DB_APPUSER_MAX_ID_SIZE)
    {
        id_len = YX_DB_APPUSER_MAX_ID_SIZE;
    }
    SCI_MEMSET(yx_DB_AppUser_Reclist[appuser_idx].appuser_id,0x00,YX_DB_APPUSER_MAX_ID_SIZE+1);
    SCI_MEMCPY(yx_DB_AppUser_Reclist[appuser_idx].appuser_id,appuser_id,id_len);
    
    if(num_len > YX_DB_APPUSER_MAX_NUMBER_SIZE)
    {
        num_len = YX_DB_APPUSER_MAX_NUMBER_SIZE;
    }
    SCI_MEMSET(yx_DB_AppUser_Reclist[appuser_idx].appuser_num,0x00,YX_DB_APPUSER_MAX_NUMBER_SIZE+1);
    SCI_MEMCPY(yx_DB_AppUser_Reclist[appuser_idx].appuser_num,appuser_num,num_len);
    
    if(name_len > YX_DB_APPUSER_MAX_NAME_SIZE)
    {
        name_len = YX_DB_APPUSER_MAX_NAME_SIZE;
    }
    SCI_MEMSET(yx_DB_AppUser_Reclist[appuser_idx].appuser_name,0x00,YX_DB_APPUSER_MAX_NAME_SIZE+1);
    SCI_MEMCPY(yx_DB_AppUser_Reclist[appuser_idx].appuser_name,appuser_name,name_len);
    
    if(yx_DB_AppUser_is_init == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListModify(%d) ERR NoInit",appuser_idx);
        return FALSE;
    }
    res = ZDT_DBFile_Write(YX_DB_DIR,YX_DB_APPUSER_FS_FILE,(uint8 *)yx_DB_AppUser_Reclist,buf_array_size);
    if (res == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListModify(%d) ERR DB Write",appuser_idx);
        return FALSE;
    }		
    return TRUE;
}

BOOLEAN YX_DB_APPUSER_ListModifyBuf(uint8 appuser_idx,uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len)
{    
    if (appuser_idx >= YX_DB_APPUSER_MAX_SUM)
    {
        ZDT_LOG("YX_DB_APPUSER_ListModifyBuf(%d) ERR idx>=%d",appuser_idx,YX_DB_APPUSER_MAX_SUM);
        return FALSE;
    }
    //ZDT_LOG("YX_DB_APPUSER_ListModifyBuf(%d) %s,%s",appuser_idx,appuser_num,appuser_name);
    yx_DB_AppUser_Reclist[appuser_idx].db_id = appuser_idx+1;
...
BOOLEAN YX_DB_APPUSER_ListDel(uint8  appuser_idx)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_DB_APPUSER_Type);
    uint32 buf_array_size = (buf_size * YX_DB_APPUSER_MAX_SUM);
    ...
    if (appuser_idx >= YX_DB_APPUSER_MAX_SUM)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDel(%d) ERR idx>=%d",appuser_idx,YX_DB_APPUSER_MAX_SUM);
        return FALSE;
    }
    
    SCI_MEMSET(&yx_DB_AppUser_Reclist[appuser_idx],0x00,sizeof(YX_DB_APPUSER_Type));
    
    if(yx_DB_AppUser_is_init == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDel(%d) ERR NoInit",appuser_idx);
        return FALSE;
    }
    ...
    if (res == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDel(%d) ERR DB Write",appuser_idx);
        return FALSE;
    }		
    return TRUE;
}

BOOLEAN YX_DB_APPUSER_ListDelBuf(uint8  appuser_idx)
{
    if (appuser_idx >= YX_DB_APPUSER_MAX_SUM)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDel(%d) ERR idx>=%d",appuser_idx,YX_DB_APPUSER_MAX_SUM);
...
BOOLEAN YX_DB_APPUSER_ListDelAll(void)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_DB_APPUSER_Type);
    uint32 buf_array_size = (buf_size * YX_DB_APPUSER_MAX_SUM);
    
    SCI_MEMSET(yx_DB_AppUser_Reclist,0x00,sizeof(YX_DB_APPUSER_Type)* YX_DB_APPUSER_MAX_SUM);
    
    if(yx_DB_AppUser_is_init == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDelAll ERR NoInit");
        return FALSE;
    }
    ...
    if (res == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListDelAllERR DB Write");
        return FALSE;
    }		
    return TRUE;
}

BOOLEAN YX_DB_APPUSER_ListDelAllBuf(void)
{
    SCI_MEMSET(yx_DB_AppUser_Reclist,0x00,sizeof(YX_DB_APPUSER_Type)* YX_DB_APPUSER_MAX_SUM);
    return TRUE;
}

BOOLEAN YX_DB_APPUSER_ListSaveBuf(void)
{
    BOOLEAN res = FALSE;
    uint32 buf_size = sizeof(YX_DB_APPUSER_Type);
    uint32 buf_array_size = (buf_size * YX_DB_APPUSER_MAX_SUM);
    ...
    if(yx_DB_AppUser_is_init == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListSaveBuf ERR NoInit");
        return FALSE;
    }
    ...
    if (res == FALSE)
    {
        ZDT_LOG("YX_DB_APPUSER_ListSaveBuf DB Write");
        return FALSE;
    }		
    return TRUE;
}

int YX_DB_APPUSER_IsValid_ID(char * cur_id)
{
    uint16 id_len = 0;
    uint16 i = 0;
    uint8  appuser_id[YX_DB_APPUSER_MAX_ID_SIZE+1];
    ...
    id_len = SCI_STRLEN(cur_id);
    if(id_len == 0)
    {
        return -1;
    }
    ZDT_LOG("YX_DB_APPUSER_IsValid_ID cur_id:%s",cur_id);
    YX_DB_APPUSER_ListInit();
    for(i = 0; i < YX_DB_APPUSER_MAX_SUM; i++)
    {
        ZDT_LOG("YX_DB_APPUSER_IsValid_ID db_id:%d appuser_id:%s",yx_DB_AppUser_Reclist[i].db_id,yx_DB_AppUser_Reclist[i].appuser_id);
        if(yx_DB_AppUser_Reclist[i].db_id != 0)
        {
            ZDT_LOG("YX_DB_APPUSER_IsValid_ID appuser_id:%s",yx_DB_AppUser_Reclist[i].appuser_id);
            if(strncmp(yx_DB_AppUser_Reclist[i].appuser_id,cur_id,id_len) == 0)
            {
                return yx_DB_AppUser_Reclist[i].db_id;
            }
        }
    }
    return -1;
...
BOOLEAN YX_DB_APPUSER_GetNameByNumber(uint8* num, uint16* name)
{
    uint16 num_len = 0;
    uint16 i = 0;
    uint16 db_num_len = 0;
    uint16 name_len = 0;
    uint8  appuser_num[YX_DB_APPUSER_MAX_NUMBER_SIZE+1];
    uint8  r_num[101] = {0};
    uint16 r_len = 0;
    ...
    for(i = 0; i < YX_DB_APPUSER_MAX_SUM; i++)
    {
        if(yx_DB_AppUser_Reclist[i].db_id != 0)
        {
            SCI_MEMSET(appuser_num,0,sizeof(appuser_num));
            db_num_len = ZDT_DeleteStrHead(yx_DB_AppUser_Reclist[i].appuser_num,SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_num),appuser_num);
            if(db_num_len > 0 && r_len == db_num_len)
            {
                if(strncmp(appuser_num,r_num,r_len) == 0)
                {
                    name_len = SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_name);
                    if( name_len > 0)
                    {
                        ZDT_UCS_Str16_to_uint16((uint8*)yx_DB_AppUser_Reclist[i].appuser_name, name_len ,name, 100);
                        return TRUE;
                    }
                }
            }
...
BOOLEAN YX_DB_AppUser_have_num(void)
{
    uint8 i = 0;	
    uint8 idx = 0;	
...
uint8 YX_DB_AppUser_GetSum(void)
{
    uint8 i = 0;	
    uint8 idx = 0;	
...
uint8  YX_DB_AppUser_Add(uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len)
{
    uint8 i = 0;	
    uint8 idx = 0;	
    ...
    for(i = 0; i < YX_DB_APPUSER_MAX_SUM; i++)
    {
        ...
        {
            idx = i+1;
            YX_DB_APPUSER_ListModify(i,appuser_id,id_len,appuser_num,num_len,appuser_name,name_len);
            break;
        }
    }
    return idx;
...
uint8 YX_DB_AppUser_DeleteByFrinedID(uint8 * appuser_id)
{
    uint8 i = 0;	
    uint8 idx = 0;	
    ...
    {
        if(yx_DB_AppUser_Reclist[i].db_id != 0 
            && SCI_STRLEN(yx_DB_AppUser_Reclist[i].appuser_id) > 0
            && strcmp(yx_DB_AppUser_Reclist[i].appuser_id,appuser_id) == 0)
        {
            YX_DB_APPUSER_ListDel(i);
            break;
        }
    }
    return idx;
...
int YX_DB_FRIEND_IsValid_ID(char * cur_id)
{
    uint16 id_len = 0;
    uint16 i = 0;
    ...
    for(i = 0; i < YX_DB_FRIEND_MAX_SUM; i++)
    {
        ...
        {
            if(strncmp(yx_DB_Friend_Reclist[i].friend_id,cur_id,id_len) == 0)
            {
...
BOOLEAN YX_DB_Init(YX_APP_T* pMe)
{
    ...
    YX_DB_APPUSER_ListInit();
#ifdef WIN32
    YX_DB_APPUSER_ListModifyBuf(0,"200",3,"3",1,"5F204E09",8);
...
#ifdef YX_DB_NEED_BIND_STATUS
    //DB_Yx_Statues_Data_ListDelAll();
#endif
#ifdef YX_DB_NEED_WHITE
    YX_DB_WHITE_ListDelAll();
#endif
#ifdef YX_DB_NEED_APPUSER
    YX_DB_APPUSER_ListDelAll();
#endif
#ifdef YX_DB_NEED_FRIEND
