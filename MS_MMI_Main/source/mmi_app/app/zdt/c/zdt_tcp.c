...
static BOOLEAN  ZDT_TCP_Link_CheckPri(ZDT_TCP_INTERFACE_T *pMe,uint8 priority)
{
    ZDT_TCP_LINK_NODE_T  * p1 = NULL;
    BOOLEAN res = FALSE;
    ...
    if(pMe->m_tcp_is_sending)
    {
        if(pMe->m_tcp_cur_data != NULL)
        {
            if(pMe->m_tcp_cur_data->priority == priority)
            {
                return TRUE;
            }
        }
...
static BOOLEAN  ZDT_TCP_SocWriteEx(ZDT_TCP_INTERFACE_T *pMe)
{
    ...
    uint8 * out_ptr = pMe->m_tcp_buf_get;
...
#ifdef ZDT_PLAT_YX_SUPPORT_YS_AES
    aes_ptr = YX_Func_EncryptByIV(pMe->m_tcp_buf_get,len,&aes_len,g_yx_iv_str);
    if(aes_ptr == PNULL)
    {
        out_ptr = pMe->m_tcp_buf_get;
        len =  pMe->m_tcp_get_len;
        send_len = pMe->m_tcp_get_len;
    }
    else
    {
        out_ptr = aes_ptr;
    ...
    {
        sent_len = sci_sock_send(pMe->m_tcp_pISocket, (out_ptr + total_sent), send_len, 0 );
        ...
        {  // 发送成功，累计发送长度
            ZDT_TCPSendTrace((out_ptr + total_sent), send_len);
            total_sent += sent_len;
            if(total_sent >= len)
            {
                //发送完成
...
#if defined(BAIDU_VIDEOCHAT_SUPPORT)// wuxx add. priority--111--99
// TCP 任务优先级改为99. 因为百度视频TASK 优先级为100 , 须使ZDT_TCP_TaskCreate 优先级高于百度视频的100
// 问题是: 手表视频呼叫APP , APP 不接听去挂断,此时挂断指令下发到手表
// 但是手表却没有把此HANGUP 消息给到ZDT_TCP_TaskCreate 的READ 中去, 导致手表没有退出呼叫界面
// 优化此问题，ZDT_TCP_TaskEntry优先级太低，导致socket接收缓存快满了，就不会再继续产生SOCKET_READ_EVENT_IND消息了
static BLOCK_ID ZDT_TCP_TaskCreate(ZDT_TCP_INTERFACE_T *pMe)
{
    ...
    if(pMe->m_tcp_task_id == SCI_INVALID_BLOCK_ID)
    {
        pMe->m_tcp_task_id = SCI_CreateThread("ZDT_TCP_THREAD_NAME",
                                                                ...
                                                                1024*10,
                                                                50,
                                                                99,//TX_SYSTEM_NORMALE,	// TX_SYSTEM_HIGH, // wuxx TX_SYSTEM_NORMALE--99 for baidu video
                                                                SCI_PREEMPT,
                                                                SCI_AUTO_START
                                                                );
    }
...
static  int ZdtTCP_HandleSend(DPARAM param)
{
    ...
    if(pData != NULL)
    {
        ...
        if(pData->priority == 1 || pData->priority == 2 || pData->priority == 3)
        {
            //BUG 如果是微聊请求，不能连续发两条
            if(ZDT_TCP_Link_CheckPri(pMe,pData->priority))
            {
                if(pData->str != NULL)
                {
                    SCI_FREE(pData->str);
