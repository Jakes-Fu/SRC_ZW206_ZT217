...
#define DROPDOWN_BRIGHTNESS_RECT DP2PX_RECT(12,121, 236, 179)
#define DROPDOWN_VOLUME_RECT DP2PX_RECT(12,180, 236, 228)
#define DP_BRIGHT_ICON_Y	121//129
#define DP_VOLUME_ICON_Y	180//188
#else
#define DROPDOWN_BRIGHTNESS_RECT DP2PX_RECT(12,100, 236, 153)
#define DROPDOWN_VOLUME_RECT DP2PX_RECT(12,155, 236, 203)
#define DP_BRIGHT_ICON_Y	100//129
#define DP_VOLUME_ICON_Y	155//188
#endif
...
LOCAL int8 tp_down_in_button_index;
...
LOCAL void Set_Brightness_To_Max()
{
    last_brightness = MMIAPISET_GetCurrentContrast();
...
LOCAL void Restore_Brightness()
{
	MMIAPISET_SetCurrentContrast(last_brightness);
	MMIAPISET_UpdateLCDContrast(GUI_MAIN_LCD_ID, last_brightness);
	MMIAPISET_SetMainLCDContrast();
...
#if 0
LOCAL MMI_IMAGE_ID_T Shortcut_GetSimInfo()
{
    MN_PHONE_PLMN_STATUS_E plmn_status = PLMN_NO_SERVICE;
    ...
    else if(MMIAPIPHONE_GetSimExistedStatus(dual_sys))
    {
        ...
        case MMI_GMMREG_RAT_GPRS:
             signal_start_id = service_image[NET_SERVICE_TYPE_G][MMIPDP_NOTIFY_EVENT_ACTIVED];
        break;
        case MMI_GMMREG_RAT_3G:
            signal_start_id = service_image[NET_SERVICE_TYPE_3G][MMIPDP_NOTIFY_EVENT_ACTIVED];
        break;    
        case MMI_GMMREG_RAT_LTE:
        case UAL_TELE_RADIO_CURRENT_NETWORK_TYPE_4G:
            service_type = NET_SERVICE_TYPE_4G;
            signal_start_id = service_image[service_type][MMIPDP_NOTIFY_EVENT_ACTIVED];
        break; 
        case MMI_GMMREG_RAT_UNKNOWN:
        case UAL_TELE_RADIO_CURRENT_NETWORK_TYPE_UNKNOWN:
        ...
        {
            signal_start_id = service_image[NET_SERVICE_TYPE_E][MMIPDP_NOTIFY_EVENT_ACTIVED];
        }
         plmn_status = MMIAPIPHONE_GetPlmnStatus(dual_sys);
        if (PLMN_NO_SERVICE == plmn_status
            || PLMN_EMERGENCY_ONLY == plmn_status 
...
PUBLIC void ZDT_DisplayBattery(MMI_WIN_ID_T win_id)
{
...
#if 0   
    {
        ...
        wchar battery_wchar[5] = {0};
        CHGMNG_STATE_INFO_T* p_chgmng_info = CHGMNG_GetModuleState();
    	battery = p_chgmng_info->bat_remain_cap;
        if(battery == 100 && is_charge)
        {
            MMIRES_GetText(text_softkey_ok_mk, win_id, &battery_string);
        }
        else
        {
            sprintf(battery_char, "%d", battery);
            GUI_GBToWstr(battery_wchar, battery_char, strlen(battery_char));
            battery_string.wstr_ptr = battery_wchar;
...
LOCAL void DisplayLoginIcon(MMI_WIN_ID_T win_id)
{
...
LOCAL int8 get_rect_index(GUI_POINT_T point)
{
    uint8 i=0;
    GUI_RECT_T btn_img_rect = {DROPDOWN_TOP_TOOLS_IMG_MARGIN_LEFT, DROPDOWN_TOP_TOOLS_IMG_MARGIN_TOP
        , DROPDOWN_TOP_TOOLS_IMG_MARGIN_LEFT+DROPDOWN_TOP_TOOLS_IMG_WIDTH, DROPDOWN_TOP_TOOLS_IMG_MARGIN_TOP+DROPDOWN_TOP_TOOLS_BG_HEIGHT};
...
LOCAL void HandleTpPressDown(MMI_WIN_ID_T win_id, GUI_POINT_T point)
{
    ...
    {
        tp_down_in_button_index = get_rect_index(point);
    }
}

LOCAL void HandleTpPressMove(MMI_WIN_ID_T win_id, DPARAM param)
{
    GUI_POINT_T   point = {0};
    GUI_RECT_T progressrect = DROPDOWN_BRIGHTNESS_RECT;
	GUI_RECT_T progress_volume_rect = DROPDOWN_VOLUME_RECT;
	...
	{
        Set_Brightness(win_id, point);
	}
	else if (DROPDOWN_PROGBAR_SEL_VOLUME == progress_down_flag && GUI_PointIsInRect(point, progress_volume_rect))
	{
		Set_RingVolume(win_id,point);				
	}
	else
    {
...
LOCAL void Draw_Brightness_And_Volume(MMI_WIN_ID_T win_id,GUI_LCD_DEV_INFO lcd_dev_info)
{
    GUI_RECT_T progressrect = DROPDOWN_BRIGHTNESS_RECT;
	GUI_RECT_T progress_volume_rect = DROPDOWN_VOLUME_RECT;
	uint8 cur_value = 0;
    ...
    int32  volume_item_total = MMISET_VOL_MAX+1;
    WATCHCOM_ProgressBarByIndex(win_id, progressrect, cur_bright_img_id,MMISET_CONTRAST_SEVEN,res_progress_bg, res_progress_fg, res_progress_hand);
	...
	GUIRES_DisplayImg(&display_point,PNULL,PNULL,win_id,statusbar_brightness,&lcd_dev_info);
	WATCHCOM_ProgressBarByIndex(win_id, progress_volume_rect, cur_ring_volume_img_id,volume_item_total,res_progress_bg, res_progress_fg, res_progress_hand);
	...
	GUIRES_DisplayImg(&display_point,PNULL,PNULL,win_id,statusbar_volume,&lcd_dev_info);
}
...
LOCAL void HandleTpPressUp(MMI_WIN_ID_T win_id, GUI_POINT_T point)
{
    GUI_RECT_T progressrect = DROPDOWN_BRIGHTNESS_RECT;
	GUI_RECT_T progress_volume_rect = DROPDOWN_VOLUME_RECT;
    if (GUI_PointIsInRect(point, progressrect) && progress_down_flag == DROPDOWN_PROGBAR_SEL_BRIGHT)
	{
        Set_Brightness(win_id,point);
	}
	else if(GUI_PointIsInRect(point,progress_volume_rect) && progress_down_flag == DROPDOWN_PROGBAR_SEL_VOLUME)
	{
        Set_RingVolume(win_id,point);
	}
    else
    {
        int8 i = get_rect_index(point);
        if(i >= 0 && i == tp_down_in_button_index)
        {
            switch(i)
            {
                case 0:
                    //MMIROPDOWN_CreateFlasglightWin();
                    goto_flasg_light = 1; //为了手电筒退出可以留在下拉栏增加的
                                        ...
                                        )
{
    ...
    switch (msg_id)
    {
        ...
        #if !DROPDOWN_USE_ZDT_SUPPORT 
                Draw_Brightness_And_Volume(win_id,lcd_dev_info);
        #endif
            }
            break;
        }
	    ...
	    case MSG_TP_PRESS_DOWN:	
            ...
            if(goto_flasg_light == 0)
            {
                HandleTpPressDown(win_id, down_point);
            }
	        break;	
	    ...
	    case MSG_TP_PRESS_MOVE:
        {
            ...
            point.x = MMK_GET_TP_X(param);
            point.y = MMK_GET_TP_Y(param);
            if(abs(point.x - down_point.x) > abs(point.y - down_point.y))
            {
                HandleTpPressMove(win_id, param);
            }
        }
			break;
#endif
	    case MSG_TP_PRESS_UP:
		{
            ...
            if(goto_flasg_light == 1)
            {
                if((point.x - down_point.x) > MMI_MAINSCREEN_WIDTH/3)//手电筒右滑退出
                {
                    goto_flasg_light = 0;
                    Restore_Brightness();
                    MMK_SendMsg(win_id,MSG_FULL_PAINT,PNULL);
            ...
            {
                if((down_point.y - point.y) > MMI_MAINSCREEN_HEIGHT/3)//上滑退出
                {
                    MMK_CloseWin(win_id);
                    break;
                }
