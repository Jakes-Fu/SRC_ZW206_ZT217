...
static void MMIZDT_Net_ResetExt(void)
{
    ...
    if(offset_time_ms > 600 && 0 == MNPHONE_GetAttState(MMIZDT_Net_GetActiveSys()))
    {
#ifdef ZDT_NET_SUPPORT_FLYMODE_RESET
        if(FALSE == MMIAPI_ZDT_FlyMode_IsReset() && MMIAPI_ZDT_Set_FlyMode(TRUE))
        {
            MMIAPI_ZDT_FlyMode_SetReset(TRUE);
        }
        else
#endif
        {
...
static void MMIZDT_Net_Reset_CheckMode(void)
{
    ...
    {
        //其它每两个小时一次
        zdt_net_reset_interval_time = 7200;
    }
    return;
}
...
#ifdef ZDT_NET_SUPPORT_FLYMODE_RESET
    if(FALSE == MMIZDT_Net_GPRS_IsOn() || MMIAPISET_GetFlyMode() || s_zdt_net_is_init == FALSE || MMIAPI_ZDT_FlyMode_IsReset())
#else
