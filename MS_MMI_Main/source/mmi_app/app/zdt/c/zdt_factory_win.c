...
typedef enum
{
    ...
    EM_TEST_WL,
#endif
#if defined(SDCARD_SUPPORT) || defined(MSDC_CARD_SUPPORT)
    EM_TEST_TF_CARD,
#endif
    EM_TEST_RESTORE,
#ifdef ZDT_FOTA_SUPPORT
...
static BOOLEAN g_test_ok = FALSE;
static BOOLEAN g_test_pause = FALSE;
#ifdef ZDT_SHIPPING_MODE_CMD
static BOOLEAN g_test_shipmode = FALSE;
#endif
...
static EM_TEST_IS_OK_E em_test_is_ok[EM_TEST_END-1] = {FAIL};

#if defined(W217_AGING_TEST_CUSTOM)//wuxx add.
uint8 g_facotry_aging_timer_id = 0;
...
#if 1
#define DELAT_TIMES_TP 	  2000
LOCAL uint8 delay_time_tp = 0;
LOCAL uint8 delay_time_tp_id = 0;
void ZdtWatch_delay_TimeStop(void);
void ZdtWatch_delay_TimeStart(uint32 time_ms,MMI_TIMER_FUNC  func);
#endif
...
#ifdef ZDT_SHIPPING_MODE_CMD
extern void ZdtWatch_Factory_SHIPPING_MODE_Exit(void);
extern void ZdtWatch_Factory_SHIPPING_MODE_Entry(void);
#endif 

#if defined(SDCARD_SUPPORT) || defined(MSDC_CARD_SUPPORT)
extern void ZdtWatch_Factory_SdCard_Entry(void);
extern void ZdtWatch_Factory_SdCard_Exit(void);
#endif
...
#endif
#if defined(SDCARD_SUPPORT) || defined(MSDC_CARD_SUPPORT)
    ZdtWatch_Factory_SdCard_Entry, ZdtWatch_Factory_SdCard_Exit,
#endif
...
#endif
#ifdef ZDT_SHIPPING_MODE_CMD
    ZdtWatch_Factory_SHIPPING_MODE_Entry,ZdtWatch_Factory_SHIPPING_MODE_Exit,
#endif 
	NULL,NULL,
};
#endif
...
// wuxx add 20231020 from ZW95
void MMI_Factory_ShowSoftkey(void)
{
    uint16 softkey_left[] = {0x004F, 0x004B,0};
    uint16 softkey_bk[] = {0x004E,0x0047, 0}; //í?3?9000 51FA 
    uint16 softkey_fail[] = {0x0046,0x0041,0x0049,0x004c,0}; //í?3?9000 51FA 
    ...
    rect.left   = 0;
    rect.top    = 0;
    rect.right  = UI_DEVICE_WIDTH;
    rect.bottom = UI_DEVICE_HEIGHT;
	
    cur_rect = rect;
    cur_rect.top  = MMI_MAINSCREEN_HEIGHT-50 + 10; 
    
    LCD_FillRect(&lcd_dev_info, cur_rect, MMI_BLACK_COLOR);

#ifdef W217_AGING_TEST_CUSTOM //new_aginglist
    if(agingtext_timer_id != 0)
    {
        return;
    }
#endif
        ...
        cur_str_t.wstr_len = MMIAPICOM_Wstrlen(softkey_left);
        cur_str_t.wstr_ptr = softkey_left;
...
#ifdef ZDT_SHIPPING_MODE_CMD
        if(g_test_shipmode == FALSE)
#endif
        {
            if(g_test_pause)
            {
                   cur_str_t.wstr_len = MMIAPICOM_Wstrlen(softkey_fail);
                   cur_str_t.wstr_ptr = softkey_fail;
            }
            else
            {
                   cur_str_t.wstr_len = MMIAPICOM_Wstrlen(softkey_bk);
                   cur_str_t.wstr_ptr = softkey_bk;
            }
            cur_rect.left = UI_DEVICE_WIDTH/2;
            cur_rect.right = UI_DEVICE_WIDTH- 25 -10 ;
...
void MMI_Factory_ShowSoftkeyEX(void)
{
    uint16 softkey_left[] = {0x004F, 0x004B,0};
    uint16 softkey_bk[] = {0x004E,0x0047, 0}; //í?3?9000 51FA 
    uint16 softkey_fail[] = {0x0046,0x0041,0x0049,0x004c,0}; //í?3?9000 51FA 
...
void ZdtWatch_delay_TimeStart(uint32 time_ms,MMI_TIMER_FUNC  func)
{
    if(0 != delay_time_tp_id)
    {
        MMK_StopTimer(delay_time_tp_id);
        delay_time_tp_id = 0;
    }
    delay_time_tp = TRUE;
    delay_time_tp_id = MMK_CreateTimerCallback(time_ms, 
                                                                        func, 
                                                                        (uint32)PNULL, 
                                                                        FALSE);
}
...
#if defined(W217_AGING_TEST_CUSTOM)
void ZdtWatch_Aging_TimeStart(uint32 time_ms,MMI_TIMER_FUNC  func)
{
    if(0 != g_facotry_aging_timer_id)
    {
        MMK_StopTimer(g_facotry_aging_timer_id);
        g_facotry_aging_timer_id = 0;
    }
    g_facotry_aging_timer_id = MMK_CreateTimerCallback(time_ms, 
                                                                        func, 
                                                                        (uint32)PNULL, 
                                                                        FALSE);
}

void ZdtWatch_Aging_TimeStop(void)
{
    if(0 != g_facotry_aging_timer_id)
    {
        MMK_StopTimer(g_facotry_aging_timer_id);
...
void ZdtWatch_Factory_SWVER_ShowData(void)
{
    ...
    cur_rect.bottom = cur_rect.top + WATCH_FACTORY_ITEM_HEIGHT -5;
...

{
    ...
    SCI_MEMSET(disp_str,0,sizeof(disp_str));
    SCI_MEMSET(disp_wstr,0,sizeof(disp_wstr));
    temp_len = zdt_get_dev_sn1(temp_buf);
    SCI_TRACE_LOW("zdt_get_dev_sn1 temp_len : %d temp_buf=%s",temp_len,temp_buf);
    cur_rect.top    = cur_rect.bottom+2;//10*SCALE; 
    cur_rect.bottom = cur_rect.top + WATCH_FACTORY_ITEM_HEIGHT -5;

    cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "SN1:%s",temp_buf);
    cur_str_t.wstr_ptr = disp_wstr;
    SCI_TRACE_LOW("zdt_get_dev_sn1 wstr_len= %d disp_str=%s",cur_str_t.wstr_len,disp_str);
    MMI_STRNTOWSTR(cur_str_t.wstr_ptr, cur_str_t.wstr_len, (uint8*)disp_str, cur_str_t.wstr_len, cur_str_t.wstr_len);
    GUISTR_DrawTextToLCDInRect( 
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
        (const GUI_RECT_T      *)&cur_rect,       
    ...
    temp_len = zdt_get_dev_sn2(temp_buf);
    SCI_TRACE_LOW("zdt_get_dev_sn2 temp_len : %d temp_buf=%s",temp_len,temp_buf);
    
    cur_rect.top    = cur_rect.bottom+2;//10*SCALE; 
    cur_rect.bottom = cur_rect.top + WATCH_FACTORY_ITEM_HEIGHT -5;
    cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "SN2:%s",temp_buf);
    cur_str_t.wstr_ptr = disp_wstr;
    MMI_STRNTOWSTR(cur_str_t.wstr_ptr, cur_str_t.wstr_len, (uint8*)disp_str, cur_str_t.wstr_len, cur_str_t.wstr_len);
    GUISTR_DrawTextToLCDInRect( 
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
    ...
    SCI_MEMSET(disp_wstr,0,sizeof(disp_wstr));
    SCI_MEMSET(temp_buf,0,sizeof(temp_buf));
    temp_len = MMIZDT_Get_Aging_Times();
    //ls_hal_get_dev_mac(temp_buf,&temp_len);
    cur_rect.top    = cur_rect.bottom+2;//10*SCALE; 
    cur_rect.bottom = cur_rect.top + WATCH_FACTORY_ITEM_HEIGHT -5;
    cur_str_t.wstr_len = sprintf((char*)disp_str, "Aging times: %.2lu:%.2lu:%.2lu", temp_len / 60 / 60, temp_len / 60 % 60, temp_len % 60);      
    cur_str_t.wstr_ptr = disp_wstr;
    MMI_STRNTOWSTR(cur_str_t.wstr_ptr, cur_str_t.wstr_len, (uint8*)disp_str, cur_str_t.wstr_len, cur_str_t.wstr_len);
    GUISTR_DrawTextToLCDInRect(
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
    ...
    cur_rect.top    = cur_rect.bottom+2;//26*SCALE; 
    cur_rect.bottom = cur_rect.top + WATCH_FACTORY_ITEM_HEIGHT -5;
    GUISTR_DrawTextToLCDInRect( 
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
        (const GUI_RECT_T      *)&cur_rect,       
	...
	MMI_Factory_ShowSoftkeyEX();// wuxx add 20231020 from ZW95
    return;
}
...
#ifdef WIN32
uint32 g_zdt_temp_adc_val;
uint32 g_zdt_temp_val;
int32 g_zdt_temp_du;
#else
extern uint32 g_zdt_temp_adc_val;
extern uint32 g_zdt_temp_val;
extern int32 g_zdt_temp_du;
#endif
#endif
void ZdtWatch_Factory_SIM_ShowData(void)
{
    ...
    uint16 charge_txt[100] = {0x5145, 0x7535, 0x4E2D, 0x0020,0x0020,0x0}; //充电中5145 7535 4E2D 
    uint16 charge_out_t[100] = {0x672A, 0x5145, 0x7535, 0x0020,0x0020,0x0}; //未充电672A 5145 7535 
    GUI_FONT_T f_big =FACTORY_BIG_FONT;
    ...
    disp_wstr[2] = 0x003A;
    disp_wstr[3] = 0x0;
    cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) " %d (%d-%d-%d)", (p_chgmng_info->charging_temperature-1000),g_zdt_temp_adc_val,g_zdt_temp_val,g_zdt_temp_du);
    cur_str_t.wstr_ptr = disp_wstr;
    ...
    if(is_charge)
    {
        MMIAPICOM_Wstrcat(charge_txt, disp_wstr);
        cur_str_t.wstr_len = MMIAPICOM_Wstrlen(charge_txt);
        cur_str_t.wstr_ptr = charge_txt;
    }
    else
    {
        MMIAPICOM_Wstrcat(charge_out_t, disp_wstr);
...
void ZdtWatch_Factory_Speaker_Entry(void)
{
    ...
    if (g_factory_aging_test_flag == TRUE)
    {
        ...
        #else
            MMIAPISET_PlayRing(MN_DUAL_SYS_1,FALSE,1,10000,MMISET_RING_TYPE_CALL,PNULL);
        #endif
    }
    else
    {
...
#if defined(W217_AGING_TEST_CUSTOM)&&defined(ZTE_WATCH)// ZTE SRS for ZW202 ZW206
    if (g_factory_aging_test_flag == TRUE)
	{
         m_DspStat = DISPLAY_WHITE_STATE; // wuxx add , 客户要一直白屏 20231007 // W217_AGING_TEST_CUSTOM for ZW202 ZW206
	}
#endif
...
// 老化测试 ALL
void ZdtWatch_Factory_Camera_AGING_TEST_Entry(void)
{
    g_factory_aging_test_flag = TRUE;

    g_em_test_idx = EM_TEST_CAMERA_MAIN;
...
#if defined(W217_AGING_TEST_CUSTOM)
void ZdtWatch_Factory_VIB_AgingHandleTimer(
                                                                        uint8 timer_id,
                                                                        uint32 param
                                                                        );

void ZdtWatch_Factory_VIB_Aging_ShowData(void)
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    char disp_str[100] = {0};
    wchar disp_wstr[100] = {0};
    ...
    GUISTR_STYLE_T      text_style      = {0};  
    GUISTR_STATE_T      state =         GUISTR_STATE_ALIGN|GUISTR_STATE_WORDBREAK|GUISTR_STATE_SINGLE_LINE; 
    uint16 tittle[10] = {0x8001, 0x5316, 0x6D4B, 0x8BD5,0}; //老化测试8001 5316 6D4B 8BD5
    uint16 testing[10] = {0x6D4B, 0x8BD5, 0x4E2D, 0x002E, 0x002E, 0x002E, 0};
    GUI_FONT_T f_big =FACTORY_BIG_FONT;
    GUI_FONT_T f_mid =FACTORY_NORMAL_FONT;
...
void ZdtWatch_Factory_VIB_AgingTimeOut(void)
{
    ZdtWatch_Factory_VIB_Req(1);
	ZdtWatch_Factory_VIB_Aging_ShowData();
    ZdtWatch_Display_TimeStart(3000, ZdtWatch_Factory_VIB_AgingHandleTimer);
}
                                                                        ...
                                                                        )
{
    ...
    ZdtWatch_Display_TimeStart(2000, ZdtWatch_Factory_VIB_AgingTimeOut);
}

void ZdtWatch_Factory_VIB_AGING(void)
{
    ZdtWatch_Factory_VIB_Req(1);
    ZdtWatch_Display_TimeStart(3000, ZdtWatch_Factory_VIB_AgingHandleTimer);
    return;
...
void ZdtWatch_Factory_GSENSOR_ShowData(void)
{
    ...
    else if (gsenser_type == GSENSOR_TYPE_QMA6981)
    {
        cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "QMA6981");
    }
    else if (gsenser_type == GSENSOR_TYPE_QMA7981)
    {
        cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "QMA7981");
    }
    else if (gsenser_type == GSENSOR_TYPE_QMA6100)
    {
        cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "QMA6100");
    }
    #if 0// wuxx del
    else if(gsenser_type == GSENSOR_TYPE_QMA7981)
    {
...
#endif
    cur_str_t.wstr_len = sprintf((char*)disp_str, (char*) "x=%d,y=%d,z=%d",x,y,z);
    cur_str_t.wstr_ptr = disp_wstr;
    MMI_STRNTOWSTR(cur_str_t.wstr_ptr, cur_str_t.wstr_len, (uint8*)disp_str, cur_str_t.wstr_len, cur_str_t.wstr_len);
    GUISTR_DrawTextToLCDInRect( 
    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
...
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    uint8 pause_ms = 180;//90; //GPS NO SIM MAX SEARCH to 180
    uint16 i = 0;
    uint16 j = 0;
    ...
    if(((MMIAPICOM_GetFfsTime()-entry_gps_ms)/1000>pause_ms)&&(0 == gps_data.Latitude)&&(0 == gps_data.Longitude)) // 90--180 pause_ms wuxx
    {
        g_test_pause=TRUE;
...
#if defined(W217_AGING_TEST_CUSTOM)
void ZdtWatch_Factory_Aging_HandleTimer(
                                                                        uint8 timer_id,
                                                                        uint32 param
                                                                        )
{
...
void ZdtWatch_Factory_CameraMain_Entry(void)
{
    ...
    if (g_factory_aging_test_flag == TRUE)
    {
        ZdtWatch_Factory_VIB_AGING();
            ...
            MMIAPISET_PlayRing(MN_DUAL_SYS_1,FALSE,1,10000,MMISET_RING_TYPE_CALL,PNULL);
        #endif
        ZdtWatch_Aging_TimeStart(4*3600*1000,ZdtWatch_Factory_Aging_HandleTimer);
    }
#endif
    return;
}
...
void ZdtWatch_Factory_RESTORE_ShowData(void)
{
    ...
    uint16 content[30] = {  0x6309 , 0x004f , 0x004b , 0x952E , 0xFF0C ,     //按OK键，
                            0x6062 , 0x590D , 0x51FA , 0x5382 , 0x8BBE , 0x7F6E , 0x3002  ,0} ;//恢复出厂设置。

    //uint8 info = MMIZDT_Get_InfoReset();
                                                 ...
                                                 )
{
    ...
    case MSG_SET_RESET_FACTORY_OVER_IND:
        ...
        if(s_waitting_num == 0)
        {
            ...
            if( info == 1 ) //非第一次恢复出厂设置 关机进入船运模式
            {
#if defined(ZDT_SHIPPING_MODE_CMD)//shipping mode
#ifndef WIN32     
                GPIO_SetValue(8, 1); // modified by bao for ship mode enable 拉高后4秒进入船运模式，关机动画不要超过3秒
#endif
#endif
                MMIPHONE_PowerOff();
            }
...
void ZdtWatch_Factory_SdCard_Exit(void)
{
    return;
}

void ZdtWatch_Factory_SdCard_Entry(void)
{
    MMI_STRING_T text = {0};  
...
#if 1
void ZdtWatch_Factory_SHIPPING_MODE_ShowData(void)
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    char disp_str[100] = {0};
    wchar disp_wstr[100] = {0};
    ...
    GUISTR_STATE_T      state =         GUISTR_STATE_ALIGN|GUISTR_STATE_WORDBREAK|GUISTR_STATE_SINGLE_LINE; 
    GUISTR_STATE_T      state2 =         GUISTR_STATE_ALIGN|GUISTR_STATE_WORDBREAK; 
    uint16 tittle[10] = { 0x8239 , 0x8fd0 , 0x6a21 , 0x5f0f ,0}; //船运模式.
    uint16 remove_charge[20] = { 0x8BF7, 0x5148, 0x79FB, 0x9664, 0x5145, 0x7535, 0x5668, 0xFF01, 0x0}; //请先移除充电器！
   
    uint16 content[30] = {   0x6309 , 0x004f , 0x004b , 0x952e , 0xFF0C ,    //按OK键，
                            0x8fdb , 0x5165 , 0x8239 , 0x8fd0 , 0x6a21 , 0x5f0f , 0x3002 , 0 }; //进入船运模式.
    ...
    if(ZDT_GetIsCharge())
    {
        //add by bao 充电中不能进入船运模式
        cur_rect.top = 60;
        cur_rect.bottom = cur_rect.top+60;

        cur_str_t.wstr_len = MMIAPICOM_Wstrlen(remove_charge);
        cur_str_t.wstr_ptr = remove_charge;
        text_style.font_color = MMI_DARK_RED_COLOR;
        GUISTR_DrawTextToLCDInRect( 
            (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
            (const GUI_RECT_T      *)&cur_rect,       
    ...
    cur_rect.bottom = MMI_MAINSCREEN_HEIGHT - 60;
...
void ZdtWatch_Factory_SHIPPING_MODE_Exit(void)
{
    ZdtWatch_Display_TimeStop();
    return;
}
...
void ZdtWatch_Factory_SHIPPING_MODE_HandleTimer(
                                                                        uint8 timer_id,
                                                                        uint32 param
                                                                        )
{
    if(g_em_test_idx == EM_SHIPPING_MODE)
    {
        ZdtWatch_Factory_SHIPPING_MODE_ShowData();
        ZdtWatch_Display_TimeStart(1000, ZdtWatch_Factory_SHIPPING_MODE_HandleTimer);
    }
}

void ZdtWatch_Factory_SHIPPING_MODE_Entry(void)
{
#ifdef ZDT_PLAY_RING_CUSTOM
    uint32 cal_info = 0;
    BOOLEAN is_calibration = FALSE;
...
LOCAL BOOLEAN Show_Softkey(void)
{
...
#ifdef FACTORY_KEY_TEST
 else if(g_em_test_idx == EM_TEST_KEY)
 {
 }
 #endif
 //else if(g_em_test_idx == EM_TEST_MAINL)
//  {
//  }
 //else if(g_em_test_idx == EM_TEST_CAMERA_MAIN)
 //{
 //}
else if(g_em_test_idx == EM_TEST_RESTORE)
{
#if defined(MAINLCD_DEV_SIZE_240X284)
    if( (pt.x <= (UI_DEVICE_WIDTH/2) )&&(pt.y> (UI_DEVICE_HEIGHT-40) ))
#elif defined(MAINLCD_DEV_SIZE_240X240)
   if( (pt.x <= (UI_DEVICE_WIDTH/2) )&&(pt.y> (UI_DEVICE_HEIGHT-40)  )) // wuxx modify 240--(240-60)
#else
   if( (pt.x <= (UI_DEVICE_WIDTH/2) )&&(pt.y> (UI_DEVICE_HEIGHT-40) )) //
#endif
   {
   ...
   else if((pt.x > (UI_DEVICE_WIDTH/2) )&&(pt.y> (UI_DEVICE_HEIGHT-40) ))
   {
#ifdef ZDT_SHIPPING_MODE_CMD
     ZdtWatch_Factory_StartNext();
#else
     MMIZDT_CloseWatchFactoryWin();
#endif 
   }
}
#ifdef ZDT_SHIPPING_MODE_CMD
     else if(g_em_test_idx == EM_SHIPPING_MODE)
     {
        if((pt.x  <= (UI_DEVICE_WIDTH/2)  )&&(pt.y> (UI_DEVICE_HEIGHT-40) ))
        {
            if(ZDT_GetIsCharge() == FALSE) //add by bao 充电中不能进入船运模式
            {
 ...
 else
 {
    if((pt.x  <= (UI_DEVICE_WIDTH/2)  )&&(pt.y>(UI_DEVICE_HEIGHT-40)))
    {
        ...
        em_test_is_ok[g_em_test_idx] = PASS;
        ZdtWatch_Factory_StartNext();
        if(g_test_pause == TRUE)
        {
            g_test_pause = FALSE;
            if(TRUE==Show_Softkey())
            {
                MMI_Factory_ShowSoftkey();
            }
        }
...
BOOLEAN ZdtWatch_Factory_StartNext(void)
{
    ...
    g_em_test_idx++;
    ZdtWatch_delay_TimeStart(DELAT_TIMES_TP,ZdtWatch_delay_TimeStop);
...
#if defined(W217_AGING_TEST_CUSTOM)// wuxx 20210805
    // 退出界面停止播放RING 语音
    MMIAPISET_StopAllRing(TRUE);
...
WINDOW_TABLE( MMIZDT_WATCH_FACTORY_WIN_TAB ) = 
{
    ...
    WIN_HIDE_STATUS,
    WIN_STYLE(WS_DISABLE_FLING_CLOSE_WIN || WS_DISABLE_RETURN_WIN), // wuxx 20231228 for FACTORY TEST //添加了右滑不能退
    END_WIN
};
                                            ...
                                            )
{
	...
	SCI_TRACE_LOW("FACTORY HandleZDT_WatchFactoryWinMsg, msg_id=0x%x",msg_id);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        {
...
#endif 
    	{
    		if(g_em_test_idx == EM_TEST_CAMERA_MAIN)
    		{
                g_test_ok=TRUE;
                em_test_is_ok[g_em_test_idx] = PASS;
                ZdtWatch_Factory_StartNext();
    ...
    case MSG_KEYUP_CANCEL:
        ...
        if(!IsWatchListFactoryWinOpen())//yangyu add
        {
            watch_factory_key_enter_hdlr();
        }
        else
...
#ifdef ZDT_SHIPPING_MODE_CMD
BOOLEAN MMIZDT_OpenWatchFactoryWin_ShipMode(void)
{
    uint8 * p_idx = SCI_ALLOC_APP(sizeof(uint8));
    g_test_shipmode = TRUE;
    *p_idx = EM_SHIPPING_MODE;
    MMK_CreateWin((uint32*)MMIZDT_WATCH_FACTORY_WIN_TAB,(ADD_DATA)p_idx);
    return TRUE;
}
#endif
...
LOCAL void Watch_FactoryListInit()
{
    ...
    item_t.item_style =  GUIITEM_STYLE_ONE_LINE_TEXT_FACTORY;//GUIITEM_STYLE_TWO_LINE_TWO_ICON_TEXT_AND_TEXT;//GUIITEM_STYLE_ONE_LINE_BIGICON_TEXT_WITH_MASK_MS;//GUIITEM_STYLE_ONE_LINE_ICON_TEXT_MS;//GUIITEM_STYLE_ONE_LINE_ICON_TEXT;//GUIITEM_STYLE_ONE_LINE_TEXT; 
    ...
    for(i = 0 ;i < EM_TEST_END-1 ; i++)
    {
        ...
        item_data.item_content[0].custom_font_color = em_test_is_ok[i+1]?MMI_GREEN_COLOR:MMI_RED_COLOR;
        item_data.item_content[0].custom_font_color_focus = em_test_is_ok[i+1]?MMI_GREEN_COLOR:MMI_RED_COLOR;
                                            ...
                                            )
{
    ...
    case MSG_CLOSE_WINDOW:
            //Watch_Factory_Exit();
            ZdtWatch_BackLight(FALSE);
            SCI_MEMSET(em_test_is_ok,0,(EM_TEST_END-1)*sizeof(EM_TEST_IS_OK_E));
            if(p_index != PNULL)
            {
                SCI_FREE(p_index);
            }
...
#ifdef W217_AGING_TEST_CUSTOM //new_aginglist
//add_agingtest
BOOLEAN is_in_Agingfactorywin = FALSE;

PUBLIC BOOLEAN MMIeng_IsInAgingTestWin()
{
    BOOLEAN ret = FALSE;
    
    MMI_WIN_ID_T win_id = MMK_GetFocusWinId();
    //if(win_id == MMIENG_AGINGTEST_MENU_WIN_ID)
    if(MMK_IsOpenWin(MMIENG_AGINGTEST_MENU_WIN_ID))
    {
        ret = TRUE;
    }
    return ret;
...
extern void ZdtWatch_Factory_Camera_Start(void);
extern void ZdtWatch_Factory_Camera_Stop(void);

extern void ZdtWatch_Display_TimeStart(uint32 time_ms,MMI_TIMER_FUNC  func);
extern void ZdtWatch_Display_TimeStop(void);

void ZdtWatch_Factory_Aging_HandleTimerEX(
                                                                        uint8 timer_id,
                                                                        uint32 param
                                                                        )
{
		MMK_SendMsg(MMIENG_AGINGTEST_MENU_WIN_ID, MSG_FULL_PAINT, NULL);
        ZdtWatch_Display_TimeStart(1000, ZdtWatch_Factory_Aging_HandleTimerEX);
}
...
LOCAL void MMIZDT_StartAgingTextTimer()
{
    if(agingtext_timer_id != 0)
    {
        MMK_StopTimer(agingtext_timer_id);
        agingtext_timer_id = 0;
    }
    agingtext_timer_id =  MMK_CreateWinTimer(MMIENG_AGINGTEST_WIN_ID, 1000,TRUE);

}

LOCAL void MMIZDT_StopAgingTextTimer()
{
    if(agingtext_timer_id != 0)
    {
        MMK_StopTimer(agingtext_timer_id);
...
LOCAL void MMIZDT_StopAgingTimer()
{

    if(aging_timer_id != 0)
    {
		SCI_TRACE_LOW("aging MMIZDT_StopAgingTimer");
        MMK_StopTimer(aging_timer_id);
        aging_timer_id = 0;
    }
...
LOCAL void AgingFunc(void)
{
	ZdtWatch_Factory_Camera_Start();
    MMIAPISET_PlayCallRingByVol(8, 1,AUD_PLAY_FOREVER, MMISET_RING_TYPE_CALL, PNULL);
    MMIDEFAULT_AllowTurnOffBackLight(FALSE);

}

LOCAL void AgingFuncExit(void)
{
	MMK_SendMsg(MMIENG_AGINGTEST_WIN_ID, MSG_KEYDOWN_CANCEL, PNULL);
}

LOCAL void AgingStartFunc(uint32 param)
{
	uint32 time = param;
	SCI_TRACE_LOW("aging AgingStartFunc time=%d",time);
	if( time == 0)
	{
		AgingFunc();
	}else{
		AgingFunc();
		aging_timer_id =  MMK_CreateTimerCallback(time*60*60*1000, AgingFuncExit, 0, FALSE);
	}
}

LOCAL MMI_RESULT_E HandleZDT_AgingWinMsg(MMI_WIN_ID_T win_id, 
										  MMI_MESSAGE_ID_E msg_id, 
										  DPARAM param
										  )
{
    ...
    uint8 * p_index = PNULL;
	uint32 idx = 0;
	LOCAL uint32 get_time = 0;
	LOCAL uint32 save_time = 0;
   ...
   SCI_TRACE_LOW("aging HandleZDT_AgingWinMsg msg_id=0x%x",msg_id);
	switch (msg_id) 
	{
	case MSG_OPEN_WINDOW:
		{
			is_in_Agingfactorywin=1;
			MMIZDT_StopAgingTimer();
			MMIZDT_StartAgingTextTimer();
            if(p_index != PNULL)
            {
                idx = *p_index;
				switch(idx)  //函数内延时  现在是延时调用函数
				{
					case 0:		
							get_time = 0;
							time_index=0;
	...
	case MSG_FULL_PAINT:
		...
		LCD_FillRect(MMITHEME_GetDefaultLcdDev(), client_rect, MMI_BLACK_COLOR);
		lab_rect.top = 2;
		lab_rect.bottom = lab_rect.top+30;
		lab_rect.left = 0;
		lab_rect.right= lab_rect.left + 80;
		GUILABEL_SetRect(MMIENG_UITEST_LABEL_CTRL_ID, &lab_rect, FALSE);
        GUILABEL_SetFont(MMIENG_UITEST_LABEL_CTRL_ID,MMI_DEFAULT_SMALL_FONT,MMI_WHITE_COLOR);
		MMI_GetLabelTextByLang(TXT_ENG_AGING_TEST, &text );
		...
		SCI_TRACE_LOW("aging HandleZDT_AgingWinMsg time_index=%d",time_index);
       memset(data_buf,0,sizeof(data_buf));


			sprintf((char*)data_buf, "%.2lu:%.2lu:%.2lu", time_index / 60 / 60, time_index / 60 % 60, time_index % 60); 	 
			str = (char*)data_buf;
	        text1.wstr_len = strlen(str);
	        text1.wstr_ptr = wstr;

		   MMIAPICOM_StrcatFromStrToUCS2(content_text_1, &uint16_str_len, (uint8*)str, strlen(str));
		   ...
		   GUISTR_DrawTextToLCDInRect( 
					   ...
					   (const GUI_RECT_T	  *)&lab_rect,		 //the fixed display area
					   (const GUI_RECT_T	  *)&lab_rect,		 //用户要剪切的实际区域
    ...
    case MSG_TIMER:
    {
        if (*(uint8*)param == agingtext_timer_id)
        {
        	if(get_time == 0)
        	{
				time_index ++;
            ...
            save_time++;
            ZdtWatch_Factory_VIB_Req( time_index%2 );
	...
	case MSG_CLOSE_WINDOW:
            save_time += MMIZDT_Get_Aging_Times();
            MMIZDT_NV_Set_Aging_Times(save_time);
...
WINDOW_TABLE( MMIZDT_WATCH_AGING_WIN_TAB ) = 
{
    WIN_HIDE_STATUS,
    WIN_FUNC((uint32) HandleZDT_AgingWinMsg),    
    WIN_ID( MMIENG_AGINGTEST_WIN_ID ),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, MMIENG_UITEST_LABEL_CTRL_ID),
    END_WIN
};

BOOLEAN MMIZDT_OpenAgingTestWinIndex(uint8 index)
{
    uint8 * p_idx = SCI_ALLOC_APP(sizeof(uint8));
    SCI_TRACE_LOW("aging MMIZDT_OpenAgingTestWinIndex index=%d",index);
    *p_idx = index;
    MMK_CreateWin((uint32*)MMIZDT_WATCH_AGING_WIN_TAB,(ADD_DATA)p_idx);
    return TRUE;
}

LOCAL void Watch_AgingTestListInit()
{


    MMI_TEXT_ID_T text_id_list[6] =   
    {
        TXT_ENG_AGING_T00,
        TXT_ENG_AGING_T02,
        TXT_ENG_AGING_T04,
    ...
    GUILIST_RemoveAllItems(MMIENG_UITEST_TIMEOUT_CTRL_ID);
    ...
    for(i = 0 ;i < 6 ; i++)
    {
        ...
        ret = GUILIST_AppendItem (MMIENG_UITEST_TIMEOUT_CTRL_ID, &item_t);
    }

}


LOCAL MMI_RESULT_E AgingTestMenuWinHandleMsg(MMI_WIN_ID_T win_id, 
										  MMI_MESSAGE_ID_E msg_id, 
										  DPARAM param
										  )
{
		MMI_RESULT_E			result	= MMI_RESULT_TRUE;
		MMI_CTRL_ID_T			 ctrl_id = MMIENG_UITEST_TIMEOUT_CTRL_ID;
		uint16					  node_id = 0;
		static BOOLEAN			  s_need_update   = FALSE;

		GUI_RECT_T full_rect = MMITHEME_GetFullScreenRect();
#if 1	//xiongkai ADD_TITLE_IN_ALL_SETTING_MENU	
			GUI_RECT_T			content_rect={0,MMI_SPECIAL_TITLE_HEIGHT,240,MMI_MAINSCREEN_HEIGHT}; 
			GUI_LCD_DEV_INFO	lcd_dev_info = {GUI_MAIN_LCD_ID,GUI_BLOCK_MAIN};
	...
	case MSG_OPEN_WINDOW:
	          ...
	          GUILIST_SetRect(ctrl_id,&content_rect);
	          ...
	          ///////draw title
	          text_style.align = ALIGN_LVMIDDLE;
	          text_style.font = FACTORY_BIG_FONT;
	          text_style.font_color = MMI_WHITE_COLOR;
	          text_style.char_space = 0;
							
	          MMI_GetLabelTextByLang(TXT_ENG_AGING_TEST, &string);
	          GUISTR_DrawTextToLCDInRect( 
	                      (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
...
#endif
		MMIZDT_StopAgingTimer();
            //full_rect.top = MMI_SPECIAL_TITLE_HEIGHT;
			GUILIST_SetMaxItem(ctrl_id, 7, FALSE);
			//GUILIST_SetRect(ctrl_id, &full_rect);
			CTRLLIST_SetTextFont(ctrl_id, SONG_FONT_26, MMI_WHITE_COLOR);	

			Watch_AgingTestListInit();
    ...
    case MSG_CTL_MIDSK:  
		{
			...
			MMIZDT_OpenAgingTestWinIndex(index);
		}
		break;
...
WINDOW_TABLE(MMIENG_AGINGTEST_MENU_WIN_TAB) =
{
    ...
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E,MMIENG_UITEST_TIMEOUT_CTRL_ID),
    WIN_HIDE_STATUS,
    END_WIN
};
