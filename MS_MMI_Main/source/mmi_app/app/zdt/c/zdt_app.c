...
static void  MMIGPS_HanldePosOK(DPARAM param)
{
    ...
    if(g_gps_app_loc_once_start)
    {
        ...
        {
            SCI_TraceLow("wuxx, MMIGPS_HanldePosOK, 02 ");
            MMIZDTGPS_LocOnceCallback(1,&pos_data);
        }
        else
        {
            SCI_TraceLow("wuxx, MMIGPS_HanldePosOK, 03 ");
...
static void  MMIGPS_HanldePosFail(DPARAM param)
{
    ...
    SCI_TraceLow("wuxx, MMIGPS_HanldePosFail, 01 ");
    if(p_getdata->str != PNULL)
    {
        SCI_FREE(p_getdata->str);
    }
...
#endif
   SCI_TraceLow("wuxx, MMIGPS_HanldePosOver, 02 is_ok=%d", is_ok);
    if(g_gps_app_loc_once_start)
    {
...
LOCAL void UITestGPS_AppendMutilLINEItemContent_for_w217(MMI_CTRL_ID_T ctrl_id,ual_gnss_read_info_cnf_t*gpsSatelliteInfo)
{
    ...
    uint8 gps_max_snr = 0;
    uint8 gps_min_snr = 0;
    uint8 beidou_max_snr = 0;
    uint8 beidou_min_snr = 0;
    ...
    for(i=0;i<gpsSatelliteInfo->satellite_list_info.satellite_num;i++)
    {
        ...
        //if (s_rmc_data.is_valid == 1)// 已定位OK
        {
           ...
           {
                s_gpgsv_data.rsv[i_gps].is_valid = TRUE;//gpsSatelliteInfo->satellite_list_info.satellite_info[i].is_used;//TRUE;//gpsSatelliteInfo.satellite_list_info.satellite_info[i]. // always is FALSE. MUST MODIFY TO TRUE
                ...
                SCI_TraceLow("[W217] [GPS]:gpsSatelliteInfo->satellite_list_info.satellite_info[%d].is_used=%d", i,gpsSatelliteInfo->satellite_list_info.satellite_info[i].is_used);

                SCI_TraceLow("[W217] [GPS]:s_gpgsv_data.rsv[%d].is_valid=%d", i_gps,s_gpgsv_data.rsv[i_gps].is_valid);
           ...
           {
               s_bdgsv_data.rsv[i_bd].is_valid =  TRUE;//gpsSatelliteInfo->satellite_list_info.satellite_info[i].is_used;//TRUE;//gpsSatelliteInfo.satellite_list_info.satellite_info[i].// always is FALSE. MUST MODIFY TO TRUE
               ...
               SCI_TraceLow("[W217] [GPS]:s_bdgsv_data.rsv[%d].is_valid=%d", i_bd,s_bdgsv_data.rsv[i_bd].is_valid);
    ...
    #endif
    s_gpgsv_data.sates_in_view = i_gps;
    SCI_TraceLow("[W217] [GPS]:s_gpgsv_data.sates_in_view=%d", s_gpgsv_data.sates_in_view);

    //GPS SNR 最大最小值
    for (i=0; i<s_gpgsv_data.sates_in_view; i++)
    {
        if (i == 0)
        {
            gps_max_snr = s_gpgsv_data.rsv[0].snr;
            gps_min_snr = s_gpgsv_data.rsv[0].snr;
        }
        if (s_gpgsv_data.rsv[i].snr > gps_max_snr)
        {
             gps_max_snr = s_gpgsv_data.rsv[i].snr;
        }
        if (s_gpgsv_data.rsv[i].snr < gps_min_snr)
        {
             gps_min_snr = s_gpgsv_data.rsv[i].snr;
        }
    }
    s_gpgsv_data.max_snr = gps_max_snr;
    s_gpgsv_data.min_snr = gps_min_snr;
    ...
    #endif
    s_bdgsv_data.sates_in_view = i_bd;
    SCI_TraceLow("[W217] [GPS]:s_bdgsv_data.sates_in_view=%d", s_bdgsv_data.sates_in_view);

    //BEIDOU SNR 最大最小值
    for (i=0; i<s_bdgsv_data.sates_in_view; i++)
    {
        if (i == 0)
        {
            beidou_max_snr = s_bdgsv_data.rsv[0].snr;
            beidou_min_snr = s_bdgsv_data.rsv[0].snr;
        }
        if (s_bdgsv_data.rsv[i].snr > beidou_max_snr)
        {
             beidou_max_snr = s_bdgsv_data.rsv[i].snr;
        }
        if (s_bdgsv_data.rsv[i].snr < beidou_min_snr)
        {
             beidou_min_snr = s_bdgsv_data.rsv[i].snr;
        }
    }
    s_bdgsv_data.max_snr = beidou_max_snr;
    s_bdgsv_data.min_snr = beidou_min_snr;
                                ...
                                )
{
    ...
    MMIAPI_ZDT_FlyMode_SetReset(FALSE);
...
static BOOLEAN MMIZDT_HandleCheckTimer(void)
{
    ...
    {
        if(FALSE == MMIAPI_ZDT_FlyMode_IsReset())
        {
            MMIZDT_FlyReset_Start();
        }
    }
#endif
...
#ifdef ZDT_GSENSOR_SUPPORT
LOCAL STEP_NV_T s_current_step_nv = {0};
PUBLIC void ZDT_NV_SetTodaySteps(uint32 steps)
{
    ...
    s_current_step_nv = step_nv;
    MMINV_WRITE(MMI_ZDT_STEPS, &step_nv);
    MMI_Pedometer_WriteInfoNvm();
}

PUBLIC uint32 ZDT_NV_GetTodaySteps()
{
    return s_current_step_nv.steps;
}

PUBLIC STEP_NV_T ZDT_NV_GetTodayNvStepInfo()
{
    STEP_NV_T step_nv = {0};
    MMI_ReadNVItem(MMI_ZDT_STEPS,&step_nv);
    return step_nv;
}
...
PUBLIC BOOLEAN Low_Battery_Model_Check()
{
   ...
   SCI_TRACE_LOW("Low_Battery_Model_Check, mode:%d battery:%d bingd:%d auto_low_battery_status:%d",mode,battery,struct_yx_statues_data.bingd_statues,auto_low_battery_status);
   return ((mode == 1 || (battery <= MMIPHONE_CHARGE_WARNING_CAP_LOW_POWER && auto_low_battery_status == 1)) 
            && struct_yx_statues_data.bingd_statues != 0);
}
