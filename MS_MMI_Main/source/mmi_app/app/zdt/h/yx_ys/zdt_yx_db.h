...
#define YX_DB_NEED_BIND_STATUS
#define YX_DB_NEED_APPUSER
...
#define MAX_YX_APP_MAX_SIZE 24    //应用最大个数 用做应用管控
...
typedef struct _YX_DB_SET_Type
{
    ...
    uint16 weight;
    uint8 sms_to_app;
    uint8 allow_family_call;
    uint8 app_on[MAX_YX_APP_MAX_SIZE]; //app 管控
    uint8 is_land;
}YX_DB_SET_Type;
...
#define YX_STATUES_DATA_FS_FILE "YXBindStatus_DBFile.db"

typedef struct _DB_STATUES_DATA_Type
{
    uint8 db_id;
    uint8 bingd_statues;
}YX_STATUES_DATA_Type;

extern YX_STATUES_DATA_Type struct_yx_statues_data;

extern BOOLEAN DB_Yx_Statues_Data_ListInit(void);
extern BOOLEAN DB_Yx_Statues_Data_ListDelAll(void);
extern BOOLEAN DB_Yx_statues_Data_ListSaveBuf(void);
#endif

#ifdef YX_DB_NEED_SOS
#define YX_DB_SOS_MAX_SUM                     3
#ifdef ZTE_WATCH
#define YX_SOS_CALL_RETRY                     2
#else
#define YX_SOS_CALL_RETRY                     3
#endif
...
{
    uint8 db_id;
    uint8 relation_id;   // 1 爸爸 2 妈妈 3 爷爷4 奶奶 5外公 6外婆 7 叔叔  8 阿姨 9 自定义
...
#define YX_DB_APPUSER_MAX_SUM                     10
#define YX_DB_APPUSER_MAX_ID_SIZE             40
#define YX_DB_APPUSER_MAX_NUMBER_SIZE             20
#define YX_DB_APPUSER_MAX_NAME_SIZE             40

#define YX_DB_APPUSER_FS_FILE "yxappuser_DBFile.db"

typedef struct _YX_DB_APPUSER_Type
{
    uint8 db_id;
    uint8  appuser_id[YX_DB_APPUSER_MAX_ID_SIZE+1];
    uint8  appuser_num[YX_DB_APPUSER_MAX_NUMBER_SIZE+1];
    uint8  appuser_name[YX_DB_APPUSER_MAX_NAME_SIZE+1];
}YX_DB_APPUSER_Type;

extern YX_DB_APPUSER_Type yx_DB_AppUser_Reclist[YX_DB_APPUSER_MAX_SUM];

extern BOOLEAN YX_DB_APPUSER_ListInit(void);
extern BOOLEAN YX_DB_APPUSER_ListModify(uint8 appuser_idx,uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len);
extern BOOLEAN YX_DB_APPUSER_ListDel(uint8  appuser_idx);
extern BOOLEAN YX_DB_APPUSER_ListDelAll(void);
extern BOOLEAN YX_DB_APPUSER_ListModifyBuf(uint8 appuser_idx,uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len);
extern BOOLEAN YX_DB_APPUSER_ListDelBuf(uint8  appuser_idx);
extern BOOLEAN YX_DB_APPUSER_ListDelAllBuf(void);
extern BOOLEAN YX_DB_APPUSER_ListSaveBuf(void);
extern int YX_DB_APPUSER_IsValid_ID(char * num);
extern uint8  YX_DB_AppUser_Add(uint8 * appuser_id,uint16 id_len,uint8 * appuser_num,uint16 num_len,uint8 * appuser_name,uint16 name_len);
extern uint8 YX_DB_AppUser_GetSum(void);
#endif

#ifdef YX_DB_NEED_FRIEND
#define YX_DB_FRIEND_MAX_SUM                     20
#define YX_DB_FRIEND_MAX_ID_SIZE             40
...
extern int YX_DB_FRIEND_IsValid_ID(char * cur_id);
