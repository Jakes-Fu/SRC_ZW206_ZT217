...
LOCAL AICHAT_STATUS_T ai_chat_status = AI_CHAT_TIPS;
                           ...
                           )
{
    if (UILAYER_IsMultiLayerEnable())
    {
        ...
        create_info.offset_y = DP2PX_VALUE_Y(240-52);//130;
        create_info.width = DP2PX_VALUE(52);
        create_info.height = DP2PX_VALUE(52);
...
PUBLIC void AppendAIListContent(uint8* content, BOOLEAN isLeft)
{
    ...
    const uint16 img_height = DP2PX_VALUE(40); // xiongkai 50;
    const uint16 img_width = DP2PX_VALUE(145); // xiongkai  170;//150;
...
LOCAL void StartTipsTimer()
{
    if(ai_tips_timer_id != 0)
    {
        MMK_StopTimer(ai_tips_timer_id);
    }
    ai_tips_timer_id =  MMK_CreateWinTimer(MMIAI_CHAT_WIN_ID, AI_CHAT_TIPS_DELAY,TRUE);

}

LOCAL void StopTipsTimer()
{
    if(ai_tips_timer_id != 0)
    {
        MMK_StopTimer(ai_tips_timer_id);
...
LOCAL void AI_ShowTips(MMI_WIN_ID_T win_id)
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    GUI_POINT_T point = {0};
    GUI_POINT_T tips_icon_point = AI_CHAT_TIPS_ICON_POINT;
    MMI_STRING_T text = {0};
    GUI_RECT_T tips_text_rect = AI_CHAT_TIPS_TEXT_RECT;
    GUISTR_STYLE_T text_style = {0};
    GUI_RECT_T rect = {0,0,0,0};
    GUILIST_SetRect(MMIAI_CHAT_LIST_CTRL_ID,&rect); //隐藏列表

    GUIRES_DisplayImg(&point,PNULL,PNULL,win_id,res_bg_star,&lcd_dev_info);
    GUIRES_DisplayImg(&tips_icon_point,PNULL,PNULL,win_id,res_icon_big,&lcd_dev_info);

    text_style.align = ALIGN_HVMIDDLE;
    text_style.font = DP_FONT_26;
    text_style.font_color = MMI_WHITE_COLOR;
    MMI_GetLabelTextByLang(TXT_AICHAT_TIPS, &text);

    GUISTR_DrawTextToLCDInRect( 
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
        (const GUI_RECT_T      *)&tips_text_rect,       
        (const GUI_RECT_T      *)&tips_text_rect,     
...
LOCAL void AI_ShowUsageTips(MMI_WIN_ID_T win_id)
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    GUI_POINT_T point = {0};
    MMI_STRING_T text = {0};
    GUI_RECT_T tips_text_rect = AI_CHAT_USAGE_TIPS_TEXT_RECT;
    GUISTR_STYLE_T text_style = {0};  
    ...
    text_style.font = DP_FONT_26;
    text_style.font_color = MMI_WHITE_COLOR;
    MMI_GetLabelTextByLang(TXT_AICHAT_USAGE_TIPS, &text);
...
LOCAL void AI_ShowBottom(MMI_WIN_ID_T  win_id)
{
    if(ai_chat_status == AI_CHAT_USAGE_TIPS || ai_chat_status == AI_CHAT_CHAT)
    {
        GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
        MMI_STRING_T    cur_str_t   = {0};
                                            ...
                                            )
{
...
#else
        CTRLLIST_SetTextFont(ctrl_id, DP_FONT_28, MMI_CYAN_COLOR);
#endif
    ...
    case MSG_FULL_PAINT:
        //AI_ShowBottom(win_id);//new ui
            ...
            ai_chat_status = AI_CHAT_TIPS;
        #endif
        if(ai_chat_status == AI_CHAT_TIPS)
        {
            AI_ShowTips(win_id);
        }
        break;
    ...
    case MSG_TP_PRESS_LONG:    
    {
        ...
        if(point.y > AI_LIST_BOTTON_POSITON &&  point.y < MMI_MAINSCREEN_HEIGHT )
        {
            ...
            if(ai_chat_status == AI_CHAT_USAGE_TIPS)
            {
                GUI_RECT_T rect = {0,0,MMI_MAINSCREEN_WIDTH,MMI_MAINSCREEN_HEIGHT-AI_CHAT_RECORD_IMG_MARGIN_BOTTOM};
                GUILIST_SetRect(MMIAI_CHAT_LIST_CTRL_ID,&rect); //显示列表
            }
            #ifndef WIN32
            AI_API_Record_Start();
            #endif
  ...
  //      {
	...
	case MSG_TIMER:
        ...
        {
            ai_chat_status = AI_CHAT_USAGE_TIPS;
            AI_ShowUsageTips(win_id);
            AI_ShowBottom(win_id);
            StopTipsTimer();
