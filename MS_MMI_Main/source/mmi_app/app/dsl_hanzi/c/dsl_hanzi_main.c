...
#include "cjson.h"
#include "dal_time.h"
#include "dsl_hanzi_id.h"
#include "dsl_hanzi_main.h"
#include "dsl_hanzi_text.h"
#include "dsl_hanzi_image.h"
#include "gps_drv.h"
...
LOCAL GUI_RECT_T hanzi_pinyin_rect = {5, 2*HANZI_CARD_LINE_HIGHT, MMI_MAINSCREEN_WIDTH, 3*HANZI_CARD_LINE_HIGHT};//音标
LOCAL GUI_RECT_T hanzi_text_rect = {5,3*HANZI_CARD_LINE_HIGHT,MMI_MAINSCREEN_WIDTH-5,MMI_MAINSCREEN_HEIGHT-HANZI_CARD_LINE_HIGHT};//释义
...
extern DSL_HANZI_BOOK_NAME hanzi_book_name_str[];
...
LOCAL void Hanzi_DisplayBookList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
    ...
    for(index = 0;index < hanzi_book_info.book_total; index++)
    {
        ...
        #else
        GUI_UTF8ToWstr(name_wchar, length, hanzi_book_name_str[index].name, length);
        #endif
        ...
        case MSG_OPEN_WINDOW:
            {
                ...
                GUILABEL_SetAlign(MMI_DSL_HANZI_MAIN_LABEL_NUM_CTRL_ID, GUILABEL_ALIGN_RIGHT);
        ...
        case MSG_FULL_PAINT:
            {
                ...
                GUI_FillRect(&lcd_dev_info,hanzi_win_rect,MMI_WHITE_COLOR);

                GUIBUTTON_SetTextId(MMI_DSL_HANZI_MAIN_LABEL_BACK_CTRL_ID, HANZI_CARD);              
        ...
        case MSG_LIST_NEXT_PAGE:
            {
                ...
                if(hanzi_book_info.cur_book_page + 1 < total_page)
                {
                    hanzi_book_info.cur_book_page++;
                    MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
                }        
...
WINDOW_TABLE(MMI_HANZI_WIN_TAB) = {
    ...
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_MAIN_LABEL_BACK_CTRL_ID),
    //CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMI_DSL_HANZI_MAIN_LIST_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_HANZI_MAIN_LABEL_NUM_CTRL_ID),
    WIN_HIDE_STATUS,
    END_WIN
};
...
LOCAL void Hanzi_DisplayChapterList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
    ...
    for(i = 0;i < hanzi_chapter_count;i++)
    {
        for(j = 0; j < hanzi_chapter_children_count[i]; j++)
        {
            ...
            //chapter name
            //#ifdef WIN32
            //GUI_GBToWstr(name_wchar, hanzi_chapter.info[index].chapter_name, length);
            //#else
            ...
            if(hanzi_book_info.cur_section_idx == i && hanzi_book_info.cur_section_children_idx == j){
                item_data.item_content[1].item_data.image_id = IMG_DSL_SELECTED;
            }else{
                item_data.item_content[1].item_data.image_id = IMG_DSL_UNSELECTED;
            }
    ...
    switch (msg_id) 
    {
        case MSG_OPEN_WINDOW:
            {
                ...
                GUIBUTTON_SetFont(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID, &font);
                GUIBUTTON_SetTextAlign(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID,ALIGN_HVMIDDLE);
                GUIBUTTON_SetTextId(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID, HANZI_PACTISE);
                GUIBUTTON_SetBorder(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID, &btn_border, FALSE);
                GUIBUTTON_SetVisible(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID,FALSE,FALSE);

                GUIBUTTON_SetRect(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID, &hanzi_right_rect);
                GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID, Hanzi_OpenNewHanzi);
                GUIBUTTON_SetFont(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID, &font);
                GUIBUTTON_SetTextAlign(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID,ALIGN_HVMIDDLE);
                GUIBUTTON_SetTextId(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID, HANZI_NEW_WORD);
                GUIBUTTON_SetBorder(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID, &btn_border, FALSE);
                GUIBUTTON_SetVisible(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID,FALSE,FALSE);

                GUILABEL_SetRect(MMI_DSL_HANZI_CHAPTER_LABEL_NUM_CTRL_ID, &hanzi_dir_rect, FALSE);
                GUILABEL_SetFont(MMI_DSL_HANZI_CHAPTER_LABEL_NUM_CTRL_ID, SONG_FONT_16,MMI_BLACK_COLOR);
                GUILABEL_SetAlign(MMI_DSL_HANZI_CHAPTER_LABEL_NUM_CTRL_ID, GUILABEL_ALIGN_RIGHT);
        ...
        case MSG_FULL_PAINT:
            {
                ...
                GUIBUTTON_SetText(MMI_DSL_HANZI_CHAPTER_LABEL_BACK_CTRL_ID, text_string.wstr_ptr, text_string.wstr_len);              
                ...
                else
                {
                    ...
                    int total_page = 0;
                    for(i = 0;i < hanzi_chapter_count;i++){
                        count += hanzi_chapter_children_count[i];
                    }
                    total_page = count / HANZI_BOOK_LIST_MAX_SHOW;
                    if(count % HANZI_BOOK_LIST_MAX_SHOW != 0){
                        total_page += 1;
                    }
                    GUIBUTTON_SetVisible(MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID,TRUE,FALSE);
                    GUIBUTTON_SetVisible(MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID,TRUE,FALSE);
                    //Hanzi_SetDiretionText(MMI_DSL_HANZI_CHAPTER_LABEL_NUM_CTRL_ID, hanzi_book_info.cur_section_page, total_page);
                    Hanzi_DisplayChapterList(win_id, MMI_DSL_HANZI_CHAPTER_LIST_CTRL_ID);
                }
            }
            break;
        /*case MSG_LIST_PRE_PAGE:
            {
                if(hanzi_book_info.cur_section_page != 0)
                {
                    hanzi_book_info.cur_section_page--;
                    MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
                }
            }
            break;
        case MSG_LIST_NEXT_PAGE:
            {
                ...
                if(hanzi_book_info.cur_section_page + 1 < total_page)
                {
                    hanzi_book_info.cur_section_page++;
                    MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
                }        
        ...
        case MSG_CTL_PENOK:
            { 
                ...
                uint8 cur_idx = GUILIST_GetCurItemIndex(MMI_DSL_HANZI_CHAPTER_LIST_CTRL_ID);
...
WINDOW_TABLE(MMI_HANZI_CHAPTER_WIN_TAB) = {
    ...
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_CHAPTER_LABEL_BACK_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_HANZI_CHAPTER_LABEL_NUM_CTRL_ID),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_CHAPTER_LEFT_CTRL_ID),
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_CHAPTER_RIGHT_CTRL_ID),
    WIN_HIDE_STATUS,
    END_WIN
};
...
LOCAL void HanziDetail_DisplayDtailInfo(MMI_WIN_ID_T win_id)
{
    ...
    GUILABEL_SetRect(MMI_DSL_HANZI_DETAIL_LABEL_WORD_CTRL_ID, &hanzi_word_rect, FALSE);
    ...
    GUILABEL_SetText(MMI_DSL_HANZI_DETAIL_LABEL_WORD_CTRL_ID, &text_word, FALSE);
    ...
    text_width_piex = GUI_CalculateStringPiexlNum(text_pinyin.wstr_ptr, text_pinyin.wstr_len, SONG_FONT_16, 1);
    pinyin_rect = hanzi_pinyin_rect;
    pinyin_rect.right = pinyin_rect.left + text_width_piex + 10;
    GUILABEL_SetRect(MMI_DSL_HANZI_DETAIL_LABEL_PINYIN_CTRL_ID, &pinyin_rect, FALSE);
    GUILABEL_SetText(MMI_DSL_HANZI_DETAIL_LABEL_PINYIN_CTRL_ID, &text_pinyin, FALSE);
    
    audio_rect = hanzi_pinyin_rect;
    audio_rect.left = pinyin_rect.right;
    GUIBUTTON_SetRect(MMI_DSL_HANZI_DETAIL_BUTTON_AUDIO_CTRL_ID, &audio_rect);

    //LCD_DrawHLine(&lcd_dev_info, hanzi_pinyin_rect.left, hanzi_pinyin_rect.bottom, hanzi_pinyin_rect.right, MMI_BLACK_COLOR);
    ...
    GUITEXT_SetString(MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID, text_remark.wstr_ptr,text_remark.wstr_len, TRUE);
        ...
        case MSG_OPEN_WINDOW:
            {
                ...
                GUITEXT_IsDisplayPrg(FALSE, MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID);
                GUITEXT_SetHandleTpMsg(FALSE, MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID);
                ...
                if(is_open_new_hanzi){
                    ...
                    GUIBUTTON_SetRect(MMI_DSL_HANZI_DETAIL_DELETE_CTRL_ID, &hanzi_del_rect);                    
                }else{
                    GUIBUTTON_SetRect(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, &hanzi_left_rect);
                    GUIBUTTON_SetRect(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, &hanzi_right_rect);
                                  
                    GUIBUTTON_SetFont(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, &font);
                    GUIBUTTON_SetTextAlign(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID,ALIGN_HVMIDDLE);
                    GUIBUTTON_SetBorder(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, &btn_border, FALSE);
                  
                    GUIBUTTON_SetFont(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, &font);
                    GUIBUTTON_SetTextAlign(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID,ALIGN_HVMIDDLE);
                    GUIBUTTON_SetBorder(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, &btn_border, FALSE);                    
                }
                GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_DELETE_CTRL_ID,FALSE,FALSE);
                GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID,FALSE,FALSE);
                GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID,FALSE,FALSE);

                GUILABEL_SetRect(MMI_DSL_HANZI_DETAIL_LABEL_NUM_CTRL_ID, &hanzi_dir_rect, FALSE);
                GUILABEL_SetFont(MMI_DSL_HANZI_DETAIL_LABEL_NUM_CTRL_ID, SONG_FONT_16,MMI_BLACK_COLOR);
                GUILABEL_SetAlign(MMI_DSL_HANZI_DETAIL_LABEL_NUM_CTRL_ID, GUILABEL_ALIGN_RIGHT);
        ...
        case MSG_FULL_PAINT:
            {
                ...
                GUIBUTTON_SetText(MMI_DSL_HANZI_DETAIL_LABEL_BACK_CTRL_ID, text_string.wstr_ptr, text_string.wstr_len);
                ...
                {
                    MMIRES_GetText(WORD_LOADING, win_id, &text_string);
                    GUITEXT_SetAlign(MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID, ALIGN_HMIDDLE);
                    GUITEXT_SetString(MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID, text_string.wstr_ptr,text_string.wstr_len, TRUE);
                }
                ...
                {                       
                    GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID,TRUE,FALSE);
                    GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID,TRUE,FALSE);

                    SCI_TRACE_LOW("%s: hanzi_detail_cur_idx = %d, hanzi_detail_count = %d",
                        __FUNCTION__, hanzi_detail_cur_idx, hanzi_detail_count);
                    if(hanzi_detail_cur_idx + 1 <= hanzi_detail_count)
                    {
                        GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_BUTTON_AUDIO_CTRL_ID,TRUE,FALSE);
                        GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_BUTTON_AUDIO_CTRL_ID, HanziDetail_PlayPinyinAudio);
                        if(is_open_new_hanzi)
                        {
                            GUI_BG_T but_bg = {0};
                            but_bg.bg_type = GUI_BG_IMG;
                            if(hanzi_detail_cur_idx > 0){
                                but_bg.img_id = HANZI_PRE_ICON;
                                GUIBUTTON_SetBg(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, &but_bg);
                                GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, HanziDetail_LeftDetail);
                            }else{
                                GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID,FALSE,FALSE);
                            }
                            if(hanzi_detail_cur_idx + 1 < hanzi_detail_count){
                                but_bg.img_id = HANZI_NEXT_ICON;
                                GUIBUTTON_SetBg(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, &but_bg);                          
                                GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, HanziDetail_RightDetail);
                            }else{
                                GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID,FALSE,FALSE);
                            }
                            GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_DELETE_CTRL_ID,TRUE,FALSE);
                            GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_DELETE_CTRL_ID, HanziDetail_DeleteNewWord);
                        }
                        else
                        {
                            GUIBUTTON_SetTextId(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, HANZI_MASTERED);
                            GUIBUTTON_SetTextId(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, HANZI_UNMASTERED);
                            GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, HanziDetail_LeftDetail);
                            GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, HanziDetail_RightDetail);
                        }
                        Hanzi_SetDiretionText(MMI_DSL_HANZI_DETAIL_LABEL_NUM_CTRL_ID, hanzi_detail_cur_idx, hanzi_detail_count);                   
                        HanziDetail_DisplayDtailInfo(win_id);
                    }
                    else
                    {
                         if(!is_open_new_hanzi){
                            GUIBUTTON_SetVisible(MMI_DSL_HANZI_DETAIL_BUTTON_AUDIO_CTRL_ID,FALSE,FALSE);
                            GUIBUTTON_SetTextId(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, HANZI_BACK_CHAPTER);
                            GUIBUTTON_SetTextId(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, HANZI_NEXT_CHAPTER);
                            GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID, MMI_CloseHanziDetailWin);
                            GUIBUTTON_SetCallBackFunc(MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID, HanziDetail_NextChapterInfo);
                        }
                    }
                }
...
WINDOW_TABLE(MMI_HANZI_DETAIL_WIN_TAB) = {
    ...
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_DETAIL_LABEL_BACK_CTRL_ID),//单元名称
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DSL_HANZI_DETAIL_LABEL_NUM_CTRL_ID),//导航页
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DSL_HANZI_DETAIL_LABEL_WORD_CTRL_ID),//汉字
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DSL_HANZI_DETAIL_LABEL_PINYIN_CTRL_ID),//拼音
    CREATE_BUTTON_CTRL(IMG_DIS_AUTO_PLAY, MMI_DSL_HANZI_DETAIL_BUTTON_AUDIO_CTRL_ID),//音频图标
    CREATE_TEXT_CTRL(MMI_DSL_HANZI_DETAIL_TEXT_INFO_CTRL_ID),//释义
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_DETAIL_LEFT_CTRL_ID),//已掌握/上一个
    CREATE_BUTTON_CTRL(PNULL, MMI_DSL_HANZI_DETAIL_RIGHT_CTRL_ID),//未掌握/下一个
    CREATE_BUTTON_CTRL(HANZI_DELETE_ICON, MMI_DSL_HANZI_DETAIL_DELETE_CTRL_ID),//删除
    WIN_HIDE_STATUS,
    END_WIN
};
