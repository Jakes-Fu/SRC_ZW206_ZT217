...
#ifdef DSL_MAIN_SUPPORT
#include "dsl_main_main.h"
#include "dsl_main_nv.h"
#include "dsl_main_image.h"
#endif
#include "dsl_listening_export.h"
#include "dsl_listening_id.h"
#include "dsl_listening_image.h"
#include "dsl_listening_text.h"
...
LOCAL void ListeningAudioWin_DisplayAudioList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
	...
	index = AUDIO_LIST_SHOW_ITEM_MAX * listening_info->item_cur_page;
	num = AUDIO_LIST_SHOW_ITEM_MAX * (listening_info->item_cur_page + 1);
	...
	for( ; index < num; index++)
	{
		...
		SCI_TRACE_LOW("%s: aduio_ready = %d, index = %d", __FUNCTION__, aduio_ready, index);
		...
		{
			item_data.item_content[1].item_data.image_id = DSL_LISTEN_DOWNLOADING;
		}else
		{
			...
			{
				item_data.item_content[1].item_data.image_id = DSL_LISTEN_DOWNLOAD;
			}
	...
	)
{
	...
	switch (msg_id)
	{
		...
		case MSG_FULL_PAINT:
			{
				...
				{
					MMIRES_GetText(READBOY_TXT_LOADING_FAIL, win_id, &text_string);
					GUISTR_DrawTextToLCDInRect(
						(const GUI_LCD_DEV_INFO *)&lcd_dev_info,
						&listen_win_rect,
						&listen_win_rect,
				...
				//SCI_TRACE_LOW("%s: listening_info->item_total_num = %d", __FUNCTION__, listening_info->item_total_num);
				total_page = listening_info->item_total_num / AUDIO_LIST_SHOW_ITEM_MAX;
				if(listening_info->item_total_num % AUDIO_LIST_SHOW_ITEM_MAX != 0)
				{
					total_page++;
				}
				sprintf(num_buf, "%d/%d", listening_info->item_cur_page + 1, total_page);
		...
		case MSG_CTL_PENOK:
			{
				...
				if(Listening_IsMp3FileExist(index, album_info))
				{
						...
						)
					{
						...
						MMIREADBOY_CreateListeningPlayerWin(player_info);
					}
				}
				else
				{
					...
					{
						MMIREADBOY_CreateListeningTipWin(PALYER_PLAY_DOWNLOADING_TIP);
						break;
					}
		...
		case MSG_LIST_PRE_PAGE:
			{
				if(listening_info->item_cur_page != 0)
				{
					listening_info->item_cur_page--;
		...
		case MSG_LIST_NEXT_PAGE:
			{	
				if(listening_info->item_total_num % AUDIO_LIST_SHOW_ITEM_MAX == 0)
				{
					if(listening_info->item_cur_page + 1 < listening_info->item_total_num / AUDIO_LIST_SHOW_ITEM_MAX)
					{
						listening_info->item_cur_page++;
						MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
					}
				...
				{
					if(listening_info->item_cur_page < listening_info->item_total_num / AUDIO_LIST_SHOW_ITEM_MAX)
					{
						listening_info->item_cur_page++;
						MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
					}
...
PUBLIC void MMIREADBOY_CreateListeningAudioWin(uint8 index)
{
...
PUBLIC BOOLEAN MMIREADBOY_IsListeningAudioWinOpen(void)
{
	return MMK_IsOpenWin(LISTENING_AUDIO_LIST_WIN_ID);
}

PUBLIC void MMIREADBOY_TestToOpenPlayerWin(void)
{
	LISTENING_PLAYER_INFO * player_info = NULL;
	player_info = (LISTENING_PLAYER_INFO *)SCI_ALLOC_APPZ(sizeof(LISTENING_PLAYER_INFO));
...
LOCAL void ListeningWin_DisplayAlbumList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
	...
	num = ALBUM_LIST_SHOW_ITEM_MAX * (listening_info->album_cur_page + 1);
	...
	for( ; index < num; index++)
	{
		...
		item_data.item_content[0].item_data.image_id = DSL_LISTEN_ALBUM;
...
PUBLIC void ListeningWin_DisplaySecletedClass(MMI_WIN_ID_T win_id, GUI_LCD_DEV_INFO lcd_dev_info)
{
	...
	GUISTR_STYLE_T text_style = {0};
	MMI_STRING_T text_string = {0};
	GUI_COLOR_T color4 = MMI_BLACK_COLOR;
	GUI_COLOR_T color1 = MMI_BLACK_COLOR;
	GUI_COLOR_T color2 = MMI_BLACK_COLOR;
...
PUBLIC MMI_RESULT_E ButtonPreClass_CallbackFunc(void)
{
	...
	else
	{
		...
		MMIREADBOY_CreateListeningLocalWin();
	}
	return TRUE;
}
		...
		case MSG_FULL_PAINT:
			{
				...
				text_style.font = SONG_FONT_16;
				text_style.font_color = MMI_BLACK_COLOR;
				MMIRES_GetText(READBOY_TXT_LISTENING, win_id, &text_string);
				GUISTR_DrawTextToLCDInRect(
					(const GUI_LCD_DEV_INFO *)&lcd_dev_info,
					&listen_title_rect,
					&listen_title_rect,
				...
				if(listening_info->select_cur_class != SELECT_MODULE_LOCAL)
				{
					...
					total_page = listening_info->album_total_num / ALBUM_LIST_SHOW_ITEM_MAX;
					if(listening_info->album_total_num % ALBUM_LIST_SHOW_ITEM_MAX != 0)
					{
						total_page++;
					}
					sprintf(num_buf, "%d/%d", listening_info->album_cur_page + 1, total_page);
					GUI_GBToWstr(num_str, num_buf, strlen(num_buf));
					text_string.wstr_ptr = num_str;
					text_string.wstr_len = MMIAPICOM_Wstrlen(text_string.wstr_ptr);
					GUISTR_DrawTextToLCDInRect(
		...
		case MSG_CTL_PENOK:
			{
				...
				if(listening_load_win == 1)
				{
					...
					MMIREADBOY_CreateListeningAudioWin(index);
...
PUBLIC void MMIREADBOY_CreateListeningWin(void)
{
		...
		case MSG_FULL_PAINT:
			{
				...
				{
					MMIRES_GetText(READBOY_TXT_TF_NO_SPACE, win_id, &text_string);
				}
				else if(type == PALYER_PLAY_NO_PRE_TIP)
				{
					MMIRES_GetText(READBOY_TXT_NO_PRE, win_id, &text_string);
				}
				else if(type == PALYER_PLAY_NO_NEXT_TIP)
				{
					MMIRES_GetText(READBOY_TXT_NO_NEXT, win_id, &text_string);
				}
				else if(type == PALYER_PLAY_DOWNLOADING_TIP)
				{
					MMIRES_GetText(READBOY_TXT_DOWNLOAD_WAITTING, win_id, &text_string);
				}
				else if(type == PALYER_PLAY_DOWNLOAD_FAIL_TIP)
				{
					MMIRES_GetText(READBOY_TXT_DOWNLOAD_FAIL, win_id, &text_string);
				}
				GUISTR_DrawTextToLCDInRect(
					(const GUI_LCD_DEV_INFO *)&lcd_dev_info,
					&win_rect,
				...
				MMIRES_GetText(READBOY_TXT_DOWNLOAD_OK, win_id, &text_string);
				GUISTR_DrawTextToLCDInRect(
					(const GUI_LCD_DEV_INFO *)&lcd_dev_info,
					&ok_rect,
					&ok_rect,
...
PUBLIC void MMIREADBOY_CreateListeningTipWin(LISTENING_TIPS_TYPE_E type)
{
...
PUBLIC void MMIREADBOY_CloseListeningTipWin()
{
