		...
		if(local_info->module_info[i].module_id == module_id)
		{
			for(j = 0;j < local_info->module_info[i].album_count;j++)
			{
				...
				if(local_info->module_info[i].module_id == module_id && delete_module)
				{
					...
					dsl_directory_delete(file_path);
				}
			}
		}
	}
...
PUBLIC void Listening_ParseAudioDownload(BOOLEAN success, uint32 idx) 
{
    ...
    //if(request_http_listening_idx == idx)
    {
		...
		MMIREADBOY_CreateListeningTipWin(PALYER_PLAY_DOWNLOAD_FAIL_TIP);
        }
	 if(listening_downloading_audio_path != NULL){
		SCI_FREE(listening_downloading_audio_path);
		listening_downloading_audio_path = NULL;
	...
	if(request_http_listening_idx == idx)
	{
		...
		if(success)
		{
			...
			if(dsl_file_exist(file_str))
			{
				/*uint8 * data_buf = NULL;
				int data_size = 0;
				data_buf = dsl_file_data_read(file_str,&data_size);
				Listening_Encryption(data_buf, data_size);
				dsl_file_delete(file_str);
				dsl_file_data_write(data_buf, data_size, file_str);
				SCI_FREE(data_buf);*/
				{
