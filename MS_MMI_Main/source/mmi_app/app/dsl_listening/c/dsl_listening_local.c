...
LOCAL void ListeningLocalAudioWin_DisplayLocalAudioList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id, uint8 id_index)
{
	...
	for( ; index < num && index < local_info->module_info[id_index].album_info[0].audio_count; index++)
	{
		...
		else
		{
			...
			{
				item_data.item_content[1].item_data.image_id = DSL_LISTEN_UNSELECT;
			}
			else
			{
				item_data.item_content[1].item_data.image_id = DSL_LISTEN_SELECT;
			}
		}
...
//#endif
	...
	if(delete_info.is_select_delete)
	{
		GUIRES_DisplayImg(PNULL, &listen_del_all_rect, PNULL, win_id, DSL_LISTEN_SECLET_ALL, &lcd_dev_info);
		GUIRES_DisplayImg(PNULL, &listen_del_rect, PNULL, win_id, DSL_LISTEN_DELECT, &lcd_dev_info);
		GUIRES_DisplayImg(PNULL, &listen_del_back_rect, PNULL, win_id, DSL_LISTEN_BACK, &lcd_dev_info);
	}
	else
	{
		GUIRES_DisplayImg(PNULL, &listen_del_all_rect, PNULL, win_id, DSL_LISTEN_DELECT, &lcd_dev_info);
	}
}
...
PUBLIC void MMIREADBOY_CreateListeningLocalAudioWin(uint8 index)
{
...
PUBLIC void ListeningLocalWin_DisplayLocalAlbumList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
	...
	for( ; index < num && listening_info->local_album_total; index++)
	{
		...
		else
		{
			if(delete_info.select_info[index].is_select == 0)
			{
				item_data.item_content[2].item_data.image_id = DSL_LISTEN_UNSELECT;
			}
			else
			{
				item_data.item_content[2].item_data.image_id = DSL_LISTEN_SELECT;
			}
		}
		...
		case MSG_CTL_PENOK:
			{
				...
				{
					listening_info->local_audio_cur = 0;
					MMIREADBOY_CreateListeningLocalAudioWin(index);
				}
				else
				{
					if(delete_info.select_info[index].is_select)
		...
		case MSG_CLOSE_WINDOW:
			{
				...
				Listening_FreeLocalDataInfo();
				//SetLSTimeChooseType(GetTimeLSChooseType());
				//READBOY_SetDefaultBackLightInfo();
				memset(&delete_info, 0 , sizeof(LISTEING_LOCAL_DELETE_INFO));
			}
			break;
		default:
...
PUBLIC void MMIREADBOY_CreateListeningLocalWin(void)
{
	...
	//READBOY_SetListeningBackLightInfo();
