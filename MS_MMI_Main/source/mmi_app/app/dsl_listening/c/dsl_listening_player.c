...
LOCAL void ListeningPlayerWin_DisplayPlayerButton(MMI_WIN_ID_T win_id, GUI_LCD_DEV_INFO lcd_dev_info)
{
	...
	{
		play_bg.img_id = DSL_LISTEN_PAUSE;
	}
	else if(player_play_info.play_status == 1)
	{
		play_bg.img_id = DSL_LISTEN_PLAY;
	}
	else
	{
		play_bg.img_id = DSL_LISTEN_DOWNLOADING;
	}
...
LOCAL void ListeningPlayerWin_DisplayPlayChoose(MMI_WIN_ID_T win_id, GUI_LCD_DEV_INFO lcd_dev_info)
{
	...
	{
		GUIRES_DisplayImg(PNULL, &listening_play_style_rect, PNULL, win_id, DSL_LISTEN_LOOP, &lcd_dev_info);
	}
	else if(player_play_info.play_style == PALYER_PLAY_STYLE_RANDOM)
	{
		GUIRES_DisplayImg(PNULL, &listening_play_style_rect, PNULL, win_id, DSL_LISTEN_RANDOM, &lcd_dev_info);
	}
	else if(player_play_info.play_style == PALYER_PLAY_STYLE_SINGLE)
	{
		GUIRES_DisplayImg(PNULL, &listening_play_style_rect, PNULL, win_id, DSL_LISTEN_SINGLE, &lcd_dev_info);
	}
	GUIRES_DisplayImg(PNULL, &listening_volume_rect, PNULL, win_id, DSL_LISTEN_VOLUME, &lcd_dev_info);

	text_style.align = ALIGN_HVMIDDLE;
	text_style.font = SONG_FONT_16;
	text_style.font_color = MMI_BLACK_COLOR;
	MMIRES_GetText(READBOY_TXT_LYRICS, win_id, &text_string);
	GUISTR_DrawTextToLCDInRect(
		(const GUI_LCD_DEV_INFO *)&lcd_dev_info,
		&listening_lrc_rect,
		&listening_lrc_rect,
...
LOCAL void ListeningPlayerWin_DisplayPlayBottom(MMI_WIN_ID_T win_id, GUI_LCD_DEV_INFO lcd_dev_info)
{
	...
	if(player_play_info.bottom_type == 1)
	{
		...
		MMI_IMAGE_ID_T img_id[3] = {DSL_LISTEN_LOOP, DSL_LISTEN_RANDOM, DSL_LISTEN_SINGLE};
		MMI_TEXT_ID_T text_id[3] = {READBOY_TXT_LOOP_PLAY, READBOY_TXT_RANDOM_PLAY, READBOY_TXT_SINGLE_PLAY};
...
LOCAL void ListeningPlayer_ButtonPreCallback(void)
{
	...
	else
	{
		if(player_info->audio_index == 0) 
		{
			...
			Listening_StopPlayMp3();
			MMIREADBOY_CreateListeningTipWin(PALYER_PLAY_NO_PRE_TIP);
			SCI_FREE(player_infos);
			return;
		}
		player_info->audio_index--;
	...
	else
	{
		...
		}else
		{
			if(player_play_info.play_style == PALYER_PLAY_STYLE_LOOP)
			{
				...
				Listening_StopPlayMp3();
				MMIREADBOY_CreateListeningTipWin(PALYER_PLAY_NO_NEXT_TIP);
			}
			else
			{
				ListeningPlayer_ButtonNextCallback();
					...
					SCI_TRACE_LOW("%s: is_local_play = %d", __FUNCTION__, player_info->is_local_play);
					{
						...
						if(player_info->is_local_play)
						{
							...
							if(dsl_file_exist(file_str)){
								...
								MMIREADBOY_CreateListeningPlayerLrcWin(player_infos);
							}else{
...
WINDOW_TABLE(MMI_LIST_PLAYER_WIN_TAB) = 
{
	...
	CREATE_BUTTON_CTRL(DSL_LISTEN_NEXT, LISTENING_BUTTON_NEXT_CTRL_ID),
	CREATE_BUTTON_CTRL(PNULL, LISTENING_BUTTON_PLAY_CTRL_ID),
	CREATE_BUTTON_CTRL(DSL_LISTEN_PRE, LISTENING_BUTTON_PREV_CTRL_ID),
	WIN_HIDE_STATUS,
	END_WIN
};

PUBLIC void MMIREADBOY_CreateListeningPlayerWin(LISTENING_PLAYER_INFO * player_info)
{
...
LOCAL void display_lrc(MMI_CTRL_ID_T ctrl_id,char *path )
{
	...
	file_buf=dsl_file_data_read(path,&data_size);
...
PUBLIC void MMIREADBOY_CreateListeningPlayerLrcWin(LISTENING_PLAYER_INFO * player_info)
{
