...
PUBLIC uint32 dsl_tfcard_get_used_kb(void)
{
	MMIFILE_DEVICE_E sd_type             = MMI_DEVICE_SDCARD;
	uint32 space_low2 = 0, space_high2 = 0;
	uint32 account=0;
...
uint32  dsl_file_get_size(const char * file_name)
{
...
PUBLIC BOOLEAN dsl_file_save(const char * file_name,uint8  * data_ptr, uint32 data_len,BOOLEAN is_add)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
PUBLIC BOOLEAN dsl_file_read(const char * file_name,uint8  * data_ptr, uint32  data_len, uint32 * read_len)
{
    BOOLEAN ret = FALSE;
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
...
uint32  dsl_lrc_file_get_size(const char * file_name)
{
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
    uint16   tmp_len = 0;
...
PUBLIC BOOLEAN dsl_lrc_file_read(const char * file_name,uint8  * data_ptr, uint32  data_len, uint32 * read_len)
{
    BOOLEAN ret = FALSE;
    wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
    uint16  full_path_len = 0;
...
PUBLIC BOOLEAN dsl_file_exist(char * file_path)
{
	BOOLEAN exist = FALSE;
...
PUBLIC BOOLEAN dsl_file_delete(char * file_path)
{
    	wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
	uint16  full_path_len = 0;
...
PUBLIC BOOLEAN dsl_directory_delete(char * file_path)
{
	wchar   full_path[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
	uint16  full_path_len = 0;
	full_path_len = GUI_GBToWstr(full_path, file_path, strlen(file_path));
...
PUBLIC void dsl_file_data_add(void * data, uint32 size, char * file_path)
{
	SCI_TRACE_LOW("dsl_file %s: add file %s\n", __FUNCTION__, file_path);

	dsl_file_save(file_path,data,size,TRUE);

	SCI_TRACE_LOW("dsl_file %s: add file end %s\n", __FUNCTION__, file_path);
}

PUBLIC void dsl_file_data_write(void * data, uint32 size, char * file_path)
{
	SCI_TRACE_LOW("dsl_file %s: write file %s\n", __FUNCTION__, file_path);

	dsl_file_save(file_path,data,size,FALSE);

	SCI_TRACE_LOW("dsl_file %s: write file end %s\n", __FUNCTION__, file_path);
}

PUBLIC void * dsl_file_data_read(char * file_path,uint32 * data_size)
{
    uint32 file_len = 0;
	uint8 * tmp_buf = PNULL;
	SCI_TRACE_LOW("dsl_file %s:read file start %s\n", __FUNCTION__, file_path);

	file_len = dsl_file_get_size(file_path);
	...
	dsl_file_read(file_path, tmp_buf, file_len,data_size);

	SCI_TRACE_LOW("dsl_file %s:read file end %s\n", __FUNCTION__, file_path);
	return tmp_buf;
}

PUBLIC void * dsl_lrc_file_data_read(char * file_path,uint32 * data_size)
{
    uint32 file_len = 0;
	uint8 * tmp_buf = PNULL;
	SCI_TRACE_LOW("dsl_file %s:read file start %s\n", __FUNCTION__, file_path);

	file_len = dsl_lrc_file_get_size(file_path);
	...
	tmp_buf = (uint8 * )SCI_ALLOC_APPZ(file_len);
	dsl_lrc_file_read(file_path, tmp_buf, file_len,data_size);

	SCI_TRACE_LOW("dsl_file %s:read file end %s\n", __FUNCTION__, file_path);
	return tmp_buf;
}

PUBLIC BOOLEAN dsl_dir_exsit(char * file_path)
{
    BOOLEAN exist = FALSE;
    wchar   file_str[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
	uint16 file_length = 0;
...
PUBLIC BOOLEAN dsl_dir_creat(char * file_path)
{
    BOOLEAN err = FALSE;
    wchar   file_str[MMIFILE_FULL_PATH_MAX_LEN+1] = {0}; 
	GUI_GBToWstr( file_str, file_path, strlen(file_path));
