...
typedef struct
{
	...
	// don't clone this data
	void*   userData;
	void    (*userDataFree)(void* userData);
	column_list_t* columnList;
...
sds		lebao_srv_get_copyright_uri(void);

int 	lebao_srv_run_once(void);

int 	lebao_srv_get_chart_data(lebao_wnd_data_t* wndData);
int		lebao_srv_get_chart_res(lebao_wnd_data_t* wndData);
int 	lebao_srv_get_songlist_song(const int songlistId);
int		lebao_srv_get_default_content_brief_list(void);
...
int		lebao_srv_collect_get_list(void);
int		lebao_srv_collect_del_list(link_list_t* idList);
int     lebal_srv_collect_add(const char* id);
int     lebal_srv_collect_del(const char* id);

int		lebao_srv_get_mp3_uri_callback(const char* musicId, sds* url);
...
void	lebao_srv_clear_invalid_mp3(void);
void    lebao_srv_set_mp3_deleted_callback(int(*callback)(const int cmd, const int status, void* data));
