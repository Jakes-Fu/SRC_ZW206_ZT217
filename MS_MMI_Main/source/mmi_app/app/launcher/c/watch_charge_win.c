...
LOCAL void DrawChgTemp(MMI_WIN_ID_T win_id)
{
    GUI_RECT_T rect = LAUNCHER_CHARGE_TEMP_RECT;
    MMI_STRING_T textStr ={0};
    GUISTR_STYLE_T text_style = {0};
    GUISTR_STATE_T state = {0};
    GUI_LCD_DEV_INFO lcd_dev_info = {0};
    ...
    {
        MMI_GetLabelTextByLang(TXT_CHARGE_OVER_TEMP,&textStr);
    }
    else if(p_chgmng_info->charging_stop_reason == CHGMNG_LOWTEMP)
    {
        MMI_GetLabelTextByLang(TXT_CHARGE_LOW_TEMP,&textStr);
    }
...
LOCAL void DrawChargeInfo(MMI_WIN_ID_T win_id)
{
    ...
    {
        int cur_index = 0;
        cur_index = p_chgmng_info->bat_remain_cap/10-1;
        if(cur_index < 0)
        {
            cur_index = 0;
        }
...
LOCAL void CreateChgLabel(MMI_CTRL_ID_T ctrl_id, BOOLEAN b_init)
{
    ...
    if(p_chgmng_info->charging_stop_reason == CHGMNG_OVERTEMP || p_chgmng_info->charging_stop_reason == CHGMNG_HOTTEMP)
    {
        MMI_GetLabelTextByLang(TXT_CHARGE_OVER_TEMP,&text);
    }
    else if(p_chgmng_info->charging_stop_reason == CHGMNG_LOWTEMP)
    {
        MMI_GetLabelTextByLang(TXT_CHARGE_LOW_TEMP,&text);
    }
    else
#endif
    if (!isChargeFull())
...
#endif
    if (GetCurrentChgValue() >= BATTERY_CHARGE_FULL_CAP) //防止一直充不满
    {
        GUIANIM_SetVisible(ctrl_id, FALSE, FALSE);
        GUILABEL_SetVisible(ctrl_id2, TRUE, FALSE);
    }
...
{
#ifdef ZTE_WATCH
    uint8 bat_cap = GetCurrentChgValue();
