...
#if defined(ZDT_VIDEOCHAT_SUPPORT) && (defined(VIDEO_CALL_AGORA_SUPPORT)||defined(BAIDU_VIDEOCHAT_SUPPORT))//声网视频通话
                                      ...
                                      )
{
    ...
    switch (msg_id)
    {
        ...
        {
            int16 y = MMK_GET_TP_Y(param);
            TRACE_APP_LAUNCHER("MSG_TP_PRESS_MOVE");
            b_tp_moved = TRUE;
            if(abs(y - tp_point.y) > 20)
            {
                if(b_drop_from_top || (y - tp_point.y) > LAUNCHER_HEIGHT/2)
                {
                    MMIZDT_DropDown_EnterWin(SLIDEWIN_STARTUPMODE_AUTO);
                    b_drop_from_top = FALSE;
                }
                else if(b_drop_from_bottom || (tp_point.y - y) > LAUNCHER_HEIGHT/2)
                {
				#ifdef ZTE_WATCH
...
//存放没有被管控的菜单的序号即 菜单在g_app_list_info中的序号
LOCAL uint8 APP_MENU_SHOW[sizeof(g_app_list_info)/sizeof(APP_LIST_ITEM_T)] = {0};

//获取没有被管控的菜单总数
LOCAL uint8 Get_Show_Menu_Count()
{
    uint8 i = 0;
    uint8 j = 0;
    uint8 app_index = 0;
    memset(APP_MENU_SHOW,0,sizeof(APP_MENU_SHOW));
    for(i; i < APP_MENU_SIZE; i++)
    {
        app_index = g_app_list_info[i].app_index;
        if(app_index < MAX_YX_APP_MAX_SIZE)
        {
            if(yx_DB_Set_Rec.app_on[app_index] == 1)
            {
                APP_MENU_SHOW[j] = i;
                j++;
            }
...
{
    uint16  title_index = *((uint16 *)param);
    GUIICONLIST_DISP_T  icon_list = {0};
    uint8 menu_index = APP_MENU_SHOW[title_index];
    MMI_GetLabelTextByLang(g_app_list_info[menu_index].text_id,&icon_list.name_str);//title
    GUIICONLIST_AppendText(title_index,ctrl_id,&icon_list);
}
#endif
...
LOCAL void Append_App_Icon(MMI_CTRL_ID_T ctrl_id, DPARAM param)
{
    ...
    uint8 menu_index = APP_MENU_SHOW[icon_index];
    data_info.img_id = g_app_list_info[menu_index].img_id;
...
LOCAL void App_List_Start_App(uint8 index)
{
    ...
    {
        if(g_app_list_info[index].check_sim == 1 && !MMIAPIPHONE_IsSimOk(MN_DUAL_SYS_1))
        {
        #ifdef ZTE_WATCH
            MMIZDT_OpenNoSimOrDataWin();
        #else
...
LOCAL void Create_App_IconList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
    ...
    menu_count = Get_Show_Menu_Count();
    ...
    GUIICONLIST_SetTotalIcon(ctrl_id,menu_count);
...
LOCAL MMI_RESULT_E HandleAppIconListMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_CTL_PENOK:
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
        {
            ...
            uint8 menu_index = APP_MENU_SHOW[cur_index];
            App_List_Start_App(menu_index);
        }
        break;
#ifdef APP_ITEM_NAME //是否显示菜单名称
    ...
    case MSG_TP_PRESS_UP:
        {
        #ifdef ZTE_WATCH
            if(CTRLICONLIST_GetRelativeDislayTop(ctrl_id) < -30) //下拉关闭 奇葩需求
            {
                MMK_CloseWin(win_id);
            }
        #endif
                                      ...
                                      )
{
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
            ...
            break;
        case MSG_GET_FOCUS:
            if(yx_DB_Set_Rec.app_on[2] == 0)//拨号盘被管控
            {
                GUILABEL_SetVisible(LAUNCHER_DIALER_CTRL_ID, FALSE, FALSE);
            }
            else
            {
                GUILABEL_SetVisible(LAUNCHER_DIALER_CTRL_ID, TRUE, FALSE);
            }
            MMK_PostMsg(win_id,MSG_FULL_PAINT,PNULL,PNULL);
            break;
        case MSG_FULL_PAINT:
...
LOCAL void Init_WeChat_Page_Ctrl(MMI_WIN_ID_T win_id)
{
    GUI_RECT_T wechat_ctrl_rect = LAUNCHER_WECHAT_PAGE_LABEL_RECT;
    GUI_RECT_T wechat_title_ctrl_rect = LAUNCHER_WECHAT_PAGE_TITLE_LABEL_RECT;
    GUI_RECT_T new_message_ctrl_rect = LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_RECT;
    GUI_FONT_T font = SONG_FONT_28;

    wechat_ctrl_rect = MMI_ConvertWinRect(MMICOM_WINPOS_WIN2DISP, win_id, wechat_ctrl_rect);
    GUILABEL_SetRect(LAUNCHER_WECHAT_PAGE_CTRL_ID, &wechat_ctrl_rect, TRUE);
    GUILABEL_SetIcon(LAUNCHER_WECHAT_PAGE_CTRL_ID, res_wechat_page);

    wechat_title_ctrl_rect = MMI_ConvertWinRect(MMICOM_WINPOS_WIN2DISP, win_id, wechat_title_ctrl_rect);
    GUILABEL_SetRect(LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID, &wechat_title_ctrl_rect, TRUE);
    GUILABEL_SetTextById(LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID, TXT_WECHART, FALSE);
    GUILABEL_SetFont(LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID, font, MMI_WHITE_COLOR);

    new_message_ctrl_rect = MMI_ConvertWinRect(MMICOM_WINPOS_WIN2DISP, win_id, new_message_ctrl_rect);
    GUILABEL_SetRect(LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID, &new_message_ctrl_rect, TRUE);
    GUILABEL_SetIcon(LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID, res_new_messgae_tip);

}

LOCAL void Init_Low_Battery_Status_Exit_TipsCtrl()
{
    GUI_FONT_T font = SONG_FONT_24;//SONG_FONT_16;
    GUI_COLOR_T color = MMI_WHITE_COLOR;
    GUI_BG_T text_bg;
    GUI_RECT_T leftBtnRect = WATCH_SETOK_BTN_RECT;
    GUI_RECT_T content_rect = LAUNCHER_LOW_BATTERY_TIP_RECT;
    
    GUIBUTTON_SetRect(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID, &leftBtnRect);
    CTRLBUTTON_SetTextId(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,STXT_EXIT );
    CTRLBUTTON_SetFontSize(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,SONG_FONT_24 );
    CTRLBUTTON_SetFontColor(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,MMI_WHITE_COLOR );
    
    CTRLTEXT_SetAlign(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,ALIGN_HVMIDDLE);
    GUITEXT_IsDisplayPrg(FALSE,LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID);
    GUITEXT_SetRect(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,&content_rect);
    GUITEXT_SetFont(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID, &font, &color);
    text_bg.bg_type = GUI_BG_COLOR;
    text_bg.color = MMI_BLACK_COLOR;
    GUITEXT_SetBg(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID, &text_bg);
    GUITEXT_SetClipboardEnabled(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,FALSE);//bug 2087169
    GUITEXT_IsSlide(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,FALSE);
    CTRLTEXT_SetHandleTpMsg(FALSE,LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID);
}

LOCAL void Low_Battery_Status_Exit_SetTips()
{
    ...
    {
        CTRLBUTTON_SetVisible(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,FALSE,FALSE);
        MMIAPICOM_StrcatFromTextIDToUCS2(pDisplayWStr,&displayWStrLen,TXT_SET_LONGMODE_LBTIPS);
    }
    else
    {    
        CTRLBUTTON_SetVisible(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,TRUE,TRUE);        
        MMIAPICOM_StrcatFromTextIDToUCS2(pDisplayWStr,&displayWStrLen,TXT_SET_LONGMODE_TIPS3);
    }
    CTRLTEXT_SetRect(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,&content_rect);
    GUITEXT_SetString(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID, pDisplayWStr, displayWStrLen, FALSE );

    SCI_FREE( pDisplayWStr );
}

LOCAL void WeChat_Set_Ctrl_Visible()
{
    if(Low_Battery_Model_Check())
    {
        
        CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_CTRL_ID,FALSE,FALSE);
        CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID,FALSE,FALSE);
        CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID,FALSE,FALSE);
        //WATCHCOM_DisplayBackground(WATCH_LAUNCHER_WECHAT_PAGE_WIN_ID);
        Low_Battery_Status_Exit_SetTips();
    }
    else
    {
        GUI_RECT_T rect = {0,0,1,1};//隐藏TextBox
        CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_CTRL_ID,TRUE,FALSE);
        CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID,TRUE,FALSE);
        if(YX_VCHAT_GetAllGroupUnread() > 0 )
        {
            CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID,TRUE,FALSE);
        }
        else
        {
            CTRLLABEL_SetVisible(LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID,FALSE,FALSE);
        ...
        CTRLBUTTON_SetVisible(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID,FALSE,FALSE);   
        GUITEXT_SetRect(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,&rect);
        //CTRLLABEL_SetVisible(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID,FALSE,FALSE);
    }
}

LOCAL MMI_RESULT_E HandleLauncherWeChatPageWinMsg(
                                      MMI_WIN_ID_T        win_id,        //IN:
                                      MMI_MESSAGE_ID_E    msg_id,        //IN:
                                      DPARAM            param        //IN:
                                      )
{
    ...
    {
        case MSG_OPEN_WINDOW:
            Init_WeChat_Page_Ctrl(win_id);
            Init_Low_Battery_Status_Exit_TipsCtrl();
            WeChat_Set_Ctrl_Visible();
            break;
        case MSG_GET_FOCUS:
            WeChat_Set_Ctrl_Visible();
            MMK_PostMsg(win_id,MSG_FULL_PAINT,PNULL,PNULL);
        ...
        case MSG_APP_OK:
        {
            if(LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID == ((MMI_NOTIFY_T*)param)->src_id)
            {
                WatchSET_LongRangeModeTipsWin(win_id);
            }
            else if((LAUNCHER_WECHAT_PAGE_CTRL_ID == ((MMI_NOTIFY_T*)param)->src_id))
            {
...
WINDOW_TABLE(WATCH_LAUNCHER_WECHART_PAGE_WINTAB) =
{
    WIN_HIDE_STATUS,
    WIN_FUNC( (uint32)HandleLauncherWeChatPageWinMsg),
    WIN_ID(WATCH_LAUNCHER_WECHAT_PAGE_WIN_ID),
    WIN_STYLE(WS_DISPATCH_TO_CHILDWIN |WS_DISABLE_RETURN_WIN),
    WIN_MOVE_STYLE(MOVE_FORBIDDEN),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, LAUNCHER_WECHAT_PAGE_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, LAUNCHER_WECHAT_PAGE_TITLE_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, LAUNCHER_WECHAT_PAGE_NEW_MESSAGE_CTRL_ID),
    CREATE_BUTTON_CTRL(IMAGE_COMMON_ONEBTN_KBBG_IMAG, LAUNCHER_LOW_BATTERY_EXIT_BTN_CTRL_ID),  
    CREATE_TEXT_CTRL(LAUNCHER_LOW_BATTERY_TIPS_TEXT_CTRL_ID),
    END_WIN
};


PUBLIC void WatchLAUNCHER_WeChat_PageWin_Enter()
{
    MMI_WIN_ID_T win_id = WATCH_LAUNCHER_WECHAT_PAGE_WIN_ID;
    MMI_HANDLE_T win_handle = 0;
    GUI_RECT_T rect = LAUNCHER_CLIENT_RECT;
    if (MMK_IsOpenWin(win_id))
    {
    ...
    win_handle = MMK_CreateWin(WATCH_LAUNCHER_WECHART_PAGE_WINTAB, NULL);
    MMK_SetWinRect(win_handle, &rect);
}

LOCAL void Init_Call_Page_Ctrl(MMI_WIN_ID_T win_id)
{
    GUI_RECT_T call_ctrl_rect = LAUNCHER_CALL_PAGE_LABEL_RECT;
    GUI_RECT_T call_title_ctrl_rect = LAUNCHER_CALL_PAGE_TITLE_LABEL_RECT;
    GUI_FONT_T font = SONG_FONT_28;

    call_ctrl_rect = MMI_ConvertWinRect(MMICOM_WINPOS_WIN2DISP, win_id, call_ctrl_rect);
    GUILABEL_SetRect(LAUNCHER_CALL_PAGE_CTRL_ID, &call_ctrl_rect, TRUE);
    GUILABEL_SetIcon(LAUNCHER_CALL_PAGE_CTRL_ID, res_app_ic_dailer);

    call_title_ctrl_rect = MMI_ConvertWinRect(MMICOM_WINPOS_WIN2DISP, win_id, call_title_ctrl_rect);
    GUILABEL_SetRect(LAUNCHER_CALL_PAGE_TITLE_CTRL_ID, &call_title_ctrl_rect, TRUE);
    GUILABEL_SetTextById(LAUNCHER_CALL_PAGE_TITLE_CTRL_ID, TXT_TO_CALL, FALSE);
    GUILABEL_SetFont(LAUNCHER_CALL_PAGE_TITLE_CTRL_ID, font, MMI_WHITE_COLOR);

}

LOCAL MMI_RESULT_E HandleLauncherCallPageWinMsg(
                                      MMI_WIN_ID_T        win_id,        //IN:
                                      MMI_MESSAGE_ID_E    msg_id,        //IN:
                                      DPARAM            param        //IN:
                                      )
{
    ...
    switch (msg_id)
    {
        ...
        case MSG_APP_OK:
        {
            //WatchLAUNCHER_PhoneBook_And_DialerWin_Enter();
            MMIZDT_OpenPBWin();
            break;
        }
        case MSG_KEYDOWN_RED:
...
WINDOW_TABLE(WATCH_LAUNCHER_CALL_PAGE_WINTAB) =
{
    WIN_HIDE_STATUS,
    WIN_FUNC( (uint32)HandleLauncherCallPageWinMsg),
    WIN_ID(WATCH_LAUNCHER_CALL_PAGE_WIN_ID),
    WIN_STYLE(WS_DISPATCH_TO_CHILDWIN |WS_DISABLE_RETURN_WIN),
    WIN_MOVE_STYLE(MOVE_FORBIDDEN),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, LAUNCHER_CALL_PAGE_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, LAUNCHER_CALL_PAGE_TITLE_CTRL_ID),
    END_WIN
};


PUBLIC void WatchLAUNCHER_Call_PageWin_Enter()
{
    MMI_WIN_ID_T win_id = WATCH_LAUNCHER_CALL_PAGE_WIN_ID;
    MMI_HANDLE_T win_handle = 0;
    GUI_RECT_T rect = LAUNCHER_CLIENT_RECT;
    if (MMK_IsOpenWin(win_id))
    {
    ...
    win_handle = MMK_CreateWin(WATCH_LAUNCHER_CALL_PAGE_WINTAB, NULL);
    MMK_SetWinRect(win_handle, &rect);
}
...
LOCAL void WatchLAUNCHER_ZTE_Enter()
{
    ...
    //Check_Low_Battery_Status();//电量低于20%时自动进入长续航模式这里check 一下自动退出
    handle= WatchSLIDEPAGE_CreateHandle();  

    elem[i].fun_enter_win = WatchLAUNCHER_Call_PageWin_Enter;
    elem[i].win_id = WATCH_LAUNCHER_CALL_PAGE_WIN_ID;
    i++;
    ...
    //if(Low_Battery_Model_Check()) // 绑定的才允许进长续航模式 刚开机可能还没获取到绑定状态
    //{    
    //    elem[i].fun_enter_win = WatchSET_LongRangeModeExitTipsWin;
    //    elem[i].win_id = MMISET_LONGRANGEEIXTTIPS_WIN_ID;
    //    i++;
    //}
    //else
    //{
    //    elem[i].fun_enter_win = WatchLAUNCHER_WeChat_PageWin_Enter;
    //    elem[i].win_id = WATCH_LAUNCHER_WECHAT_PAGE_WIN_ID;
    //    i++;
    //}
    elem[i].fun_enter_win = WatchLAUNCHER_WeChat_PageWin_Enter;
    elem[i].win_id = WATCH_LAUNCHER_WECHAT_PAGE_WIN_ID;
...
extern uint8   need_ship_mode_alert;
#endif

#ifdef WIN32
uint8 need_ship_mode_alert = 0;
void REFPARAM_ResetShipModeRefParam(){}
#endif
...
#endif
#ifdef ZDT_SHIPPING_MODE_CMD
    if(need_ship_mode_alert != 0)
    {
        WatchOpen_IdleWin();
        MMIZDT_OpenWatchFactoryWin_ShipMode();
		REFPARAM_ResetShipModeRefParam();
        need_ship_mode_alert = 0;
    }
    else
#endif
    {
