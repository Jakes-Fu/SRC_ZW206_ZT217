...
LOCAL GUI_RECT_T poetry_title_rect = {0.5*POETRY_LINE_WIDTH, 0, 5.5*POETRY_LINE_WIDTH, 1.2*POETRY_LINE_HIGHT};
LOCAL GUI_RECT_T poetry_back_rect = {0.5*POETRY_LINE_WIDTH, 0, 2.5*POETRY_LINE_WIDTH, POETRY_LINE_HIGHT};
LOCAL GUI_RECT_T poetry_list_rect = {20, 1.6*POETRY_LINE_HIGHT, MMI_MAINSCREEN_WIDTH - 20, MMI_MAINSCREEN_HEIGHT - 15};
...
LOCAL DSL_POETRY_DETAIL_T * poetry_detail_infos = NULL;
...
LOCAL void Read_list_data_from_tf_and_parse(uint16 gradeid)
{
    ...
    if(dsl_file_exist(file_path)){
        ...
        if(data_buf != PNULL && file_len > 2){
            parseGradeList(1, data_buf , file_len, gradeid);
            SCI_FREE(data_buf);
        }else{
...
LOCAL void Read_detail_data_from_tf_and_parse(void)
{
    ...
    if(dsl_file_exist(file_path)){
        ...
        if(data_buf != PNULL && file_len > 0){
            parsePoetryDetail(1, data_buf, file_len, poetry_cur_post_poem_id);
            SCI_FREE(data_buf);
        }else{
            getPoetryDetail(poetry_cur_post_poem_id);
        }
    }else{
        getPoetryDetail(poetry_cur_post_poem_id);
    }        
...
LOCAL void auto_play_tip_timer(MMI_WIN_ID_T win_id)
{
    if(0 != autoplay_tip_timer)
    {
        MMK_StopTimer(autoplay_tip_timer);
    ...
    autoplay_tip_timer = MMK_CreateTimerCallback(2000, auto_play_tip_timeout,(MMI_WIN_ID_T)win_id, FALSE);
    MMK_StartTimerCallback(autoplay_tip_timer, 2000, auto_play_tip_timeout, (MMI_WIN_ID_T)win_id, FALSE);
}
	...
	if(param != PNULL && handle > 0)
	{
		...
		if(report_ptr != PNULL && handle == poetry_player_handle)
		{
			switch(report_ptr->report)
			{
				...
				{
					if(audio_play_progress + 1 < poetry_detail_infos->sentences_num){
						audio_play_progress ++;
...
LOCAL void parseDownloadAudio(BOOLEAN is_ok,uint8 * pRcv,uint32 Rcv_len,uint32 err_id)
{
    if (is_ok && pRcv != PNULL && Rcv_len> 2)
    {
        ...
        if(dsl_tfcard_exist() && dsl_tfcard_get_free_kb() > 300 * 1024)
        {
            dsl_file_data_write(pRcv, Rcv_len, file_path);
        }
        else
        {
...
LOCAL void startPlayAudio(void)
{
    ...
    else
    {
        ...
        MMIZDT_HTTP_AppSend(TRUE, url, PNULL, 0, 1000, 0, 0, 3000, 0, 0, parseDownloadAudio);
    }
}
...
LOCAL void startDownloadAudio(void)
{
    ...
    if(dsl_file_exist(file_path))
    {
        ...
        if(audio_download_progress + 1< poetry_detail_infos->sentences_num){
            audio_download_progress++;
            startDownloadAudio();           
        }
    }
...
LOCAL void parseGradeList(BOOLEAN is_ok,uint8 * pRcv,uint32 Rcv_len,uint32 err_id)
{
    ...
    if (is_ok && pRcv != PNULL && Rcv_len> 2)
    {
        ...
        if(responseNo->valueint==10000)
        {
            ...
            {
                memset(&grade_all_list.all_grade[poetry_cur_post_grade_id-1], 0, sizeof(DSL_POETRY_GRADE_LIST_T));
                ...
                if(cJSON_GetArraySize(poetry_arr) == 0){
                    ...
                    if(dsl_tfcard_exist()&& dsl_tfcard_get_free_kb() > 100 * 1024){
                        char file_path[80] = {0};
                        uint32 file_len = 0;
                        sprintf(file_path,"E:/Poetry/Poetry_offline/grade_%d/list.bin",poetry_cur_post_grade_id-1);
                        if(!dsl_file_exist(file_path)){                           
...
LOCAL BOOLEAN getPoemIsFavorite(char * poem_id)
{
...
LOCAL void changeFavoritePoem(char * poetry_id,BOOLEAN is_add)
{
    uint16 i = 0;
    char *out = PNULL;
    cJSON *root;
    ...
    if(dsl_tfcard_exist()){
        ...
        dsl_file_data_write(out, strlen(out), file_path);
        parsePoetryFavoriteList(1, out, strlen(out), 0);
    }
    SCI_FREE(out);
}

LOCAL void releasePoetryDetail(void)
{
    uint8 i = 0;
...
LOCAL void parsePoetryDetail(BOOLEAN is_ok,uint8 * pRcv,uint32 Rcv_len,uint32 err_id)
{
    ...
    if (is_ok && pRcv != PNULL && Rcv_len> 2)
    {
        ...
        if(responseNo->valueint == 10000)
        {
            ...
            if (data != NULL && data->type != cJSON_NULL)
            {
                ...
                poetry_detail_infos = SCI_ALLOC_APPZ(sizeof(DSL_POETRY_DETAIL_T));
                memset(poetry_detail_infos, 0, sizeof(DSL_POETRY_DETAIL_T));
                ...
                for(i = 0; i < poetry_detail_infos->sentences_num; i++)
                {
                    ...
                    poetry_detail_infos->sen_audio[i] = SCI_ALLOC_APPZ(sizeof(DSL_POETRY_DETAIL_AUDIO_T));
                    memset(poetry_detail_infos->sen_audio[i], 0, sizeof(DSL_POETRY_DETAIL_AUDIO_T));
    ...
    {
        poetry_is_favorite = getPoemIsFavorite(poetry_detail_infos->id);
    }
...
LOCAL void Draw_GradeList(MMI_WIN_ID_T win_id)
{
    MMI_CTRL_ID_T ctrl_id = MMI_DSL_POETRY_GRADE_LIST_CTRL_ID;
    uint8 i = 0;
    char text_char[20] = {0};
    wchar text_str[20] = {0};
    MMI_STRING_T text_string = {0};
    ...
    DSL_POETRY_GRADE_NAME grade_str[POETRY_GRADE_TOTAL_NUM] = {
    ...
    for(i = 0;i < POETRY_GRADE_TOTAL_NUM; i++)
    {
        ...
        GUILIST_SetTextFont(ctrl_id, SONG_FONT_20, MMI_BLACK_COLOR);
...
LOCAL void PoetryWin_FULL_PAINT(MMI_WIN_ID_T win_id)
{
    ...
    GUIRES_DisplayImg(PNULL, &poetry_win_rect, PNULL, win_id, IMG_POETRY_BG, &lcd_dev_info);

    text_style.align = ALIGN_LVMIDDLE;
    text_style.font = SONG_FONT_20;
    text_style.font_color = MMI_WHITE_COLOR;
    MMIRES_GetText(READBOY_TXT_POETRY, win_id, &text_string);
    GUISTR_DrawTextToLCDInRect(
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
        &poetry_title_rect,
        &poetry_title_rect,
    ...
    if(auto_play == 1){
        ...
        MMIRES_GetText(READBOY_TXT_POETRY_AUTO, win_id, &text_string);
        text_style.font = SONG_FONT_16;
        GUISTR_DrawTextToLCDInRect(
        (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
...
{
    uint16 cur_idx = GUILIST_GetCurItemIndex(MMI_DSL_POETRY_GRADE_LIST_CTRL_ID);
    Read_list_data_from_tf_and_parse(cur_idx+1);
...
{
    MMI_CTRL_ID_T ctrl_id = MMI_DSL_POETRY_ITEM_LIST_CTRL_ID;
    uint8 i = 0;
    char text_char[50] = {0};
    wchar text_str[50] = {0};
    MMI_STRING_T text_string = {0};
...
LOCAL void PoetryItemWin_FULL_PAINT(MMI_WIN_ID_T win_id)
{
    ...
    if(MMK_IsActiveCtrl(MMI_DSL_POETRY_ITEM_LIST_CTRL_ID))
    {
        GUI_RECT_T rect = {0};
        //MMK_DestroyControl(MMI_DSL_POETRY_ITEM_LIST_CTRL_ID);
        GUILIST_SetRect(MMI_DSL_POETRY_ITEM_LIST_CTRL_ID, &rect);
    }
...
{
    uint16 cur_idx = GUILIST_GetCurItemIndex(MMI_DSL_POETRY_ITEM_LIST_CTRL_ID);
    if(!is_open_favorite){
...
{
    MMI_CTRL_ID_T ctrl_id = MMI_DSL_POETRY_APPRE_TEXT_ID;
    GUI_FONT_T text_font = SONG_FONT_18;
...
LOCAL void PoetryDetailWin_ShowText(MMI_WIN_ID_T win_id)
{
    ...
    if(text_char != NULL){
        ...
        text_string.wstr_ptr = text_str;
        text_string.wstr_len = MMIAPICOM_Wstrlen(text_string.wstr_ptr);
        GUITEXT_SetRect(MMI_DSL_POETRY_APPRE_TEXT_ID, &rect);
        GUITEXT_SetAlign(MMI_DSL_POETRY_APPRE_TEXT_ID, text_style.align);
        GUITEXT_SetString(MMI_DSL_POETRY_APPRE_TEXT_ID, text_string.wstr_ptr, text_string.wstr_len, TRUE);
...
LOCAL void PoetryDetailWin_FULL_PAINT(MMI_WIN_ID_T win_id)
{
    ...
    GUIRES_DisplayImg(PNULL, &poetry_win_rect, PNULL, win_id, IMG_POETRY_DETAIL_BG, &lcd_dev_info);
    ...
    if(poetry_is_favorite){
        GUIRES_DisplayImg(PNULL, &poetry_favorite_rect, PNULL, win_id, IMG_DSL_FAVORITE, &lcd_dev_info);
    }else{
        GUIRES_DisplayImg(PNULL, &poetry_favorite_rect, PNULL, win_id, IMG_DSL_NOTFAVORITE, &lcd_dev_info);
    }
    ...
    {
        MMIRES_GetText(READBOY_TXT_POETRY_LOADING, win_id, &text_string);
        GUITEXT_SetRect(MMI_DSL_POETRY_APPRE_TEXT_ID, &poetry_content_other_rect);
        GUITEXT_SetAlign(MMI_DSL_POETRY_APPRE_TEXT_ID, text_style.align);
        GUITEXT_SetString(MMI_DSL_POETRY_APPRE_TEXT_ID, text_string.wstr_ptr, text_string.wstr_len, FALSE);
    }
...
{
    BOOLEAN is_favorite = getPoemIsFavorite(poetry_detail_infos->id);
    SCI_TRACE_LOW("%s: is_favorite = %d, poetry_is_favorite = %d", __FUNCTION__, is_favorite, poetry_is_favorite);
    if(is_favorite){
        if(!poetry_is_favorite){
            changeFavoritePoem(poetry_detail_infos->id, FALSE);
        }
    }else{
        if(poetry_is_favorite){
