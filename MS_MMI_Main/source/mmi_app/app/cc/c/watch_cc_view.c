...
**                         MACRO DEFINITION                                  *
**---------------------------------------------------------------------------*/
#define WATCH_CC_VOLUME_MIN          6   //added by bao for change call volume
...
#define CC_WATCH_WAITING_CALL_AUTO_ANSWER   15000    //15秒不接听自动接听
...
#define CC_VOLUME_STATE_RECT  {12, 10, 60, 60}
#define CC_PORTRAIT_IMG_RECT  {72, 40, 200, 200}
#define CC_CALL_NAME_RECT	  {0, 140, 240, 170}
#define CC_CALL_STATE_RECT 	  {0, 170, 240, 190}
#define CC_CALL_HAND_UP_RECT  {12, 214, 228, 254}

#define CC_CALL_TIME_RECT 	  {0, 170, 240, 190}

#define CC_INCOM_HAND_UP_RECT {12, 213, 120, 260}
#define CC_INCOM_ACEPT_RECT   {124, 213, 230, 260}
#define CC_CALL_END_IMG_RECT  {12, 214, 228, 254}
#else
#define CC_VOLUME_STATE_RECT  {12, 10, 60, 60}
#define CC_PORTRAIT_IMG_RECT  {72, 30, 200, 190}
#define CC_CALL_NAME_RECT	  {0, 130, 240, 160}
#define CC_CALL_STATE_RECT 	  {0, 160, 240, 180}
#define CC_CALL_HAND_UP_RECT  {12, 190, 228, 240}

#define CC_CALL_TIME_RECT 	  {0, 160, 240, 180}

#define CC_INCOM_HAND_UP_RECT {12, 200, 120, 240}
#define CC_INCOM_ACEPT_RECT   {124, 200, 230, 240}
#define CC_CALL_END_IMG_RECT  {12, 190, 228, 240}
#endif

uint8 call_current_volume = 0;
uint8 cc_call_current_volume = 0;
LOCAL uint8  cc_call_waiting_timer_id = 0;
/**--------------------------------------------------------------------------*
**                          GLOBAL DEFINITION                                *
**--------------------------------------------------------------------------*/
...
/*****************************************************************************/
//  Description : handle voice call end window messages
//  Global resource dependence :
//  Author: fys 10/29
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleCcCallEndWinMsg(
                                          MMI_WIN_ID_T win_id,
                                          MMI_MESSAGE_ID_E msg_id,
                                          DPARAM param
                                          );
...
WINDOW_TABLE(WATCHCC_CALL_ENDED_WIN_TAB) =
{
    WIN_ID(WATCHCC_CALL_ENDED_WIN_ID),
    WIN_FUNC((uint32)HandleCcCallEndWinMsg),
    WIN_HIDE_STATUS,
    WIN_STYLE(WS_DISABLE_RETURN_WIN),
    END_WIN
};//mo call end win
...
WINDOW_TABLE(WATCHCC_MTCALLING_WIN_TAB) =
{
    ...
    WIN_STYLE(WS_DISABLE_RETURN_WIN),
#ifndef ZTE_WATCH
    CREATE_BUTTON_CTRL(IAMGE_COMMING_ACEPT, WATCHCC_DIAL_BUTTON_CTRL_ID),
    CREATE_BUTTON_CTRL(IAMGE_COMMING_REJECT, WATCHCC_HANGUP_BUTTON_CTRL_ID),
...
#else
	MMK_CreateWin((uint32 *)WATCHCC_CALL_ENDED_WIN_TAB, PNULL);
#endif
}
...
PUBLIC BOOLEAN MMICC_UpdateCallStatusDisplay(MMICC_UPDATESTATUS_TYPE_E type)
{
    ...
    switch(type)
    {
        ...
        {
            if(MMIZDT_ShouldStayInClassModeWin())//上课禁用拒接电话 感觉有点不合理
            {
...
//  Note:
/*****************************************************************************/
LOCAL void  CC_OpenWatchMoCallWin(void)//into calling win edit fys
{
    uint8                   telNum[MMICC_PHONE_NUM_MAX_LEN + 2] = {0};
    uint8                   telNumLen = 0;
...
LOCAL void CC_MoCall_DrawHeadPhotoAndName(MMI_WIN_ID_T win_id, MMI_IMAGE_ID_T head_photo, MMI_STRING_T str_data)
{
	...
	GUI_RECT_T			img_rect = CC_PORTRAIT_IMG_RECT;
	GUI_RECT_T          nameDispRect   = CC_CALL_NAME_RECT;

#ifdef ZTE_WATCH
	GUIRES_DisplayImg(PNULL, &img_rect, PNULL, win_id, head_photo, &devInfo);
#else
	GUIRES_DisplayImg(&start_point, PNULL, PNULL, win_id, head_photo, &devInfo);
#endif

    nameStyleInfo.align    = ALIGN_HVMIDDLE;
    nameStyleInfo.font     = SONG_FONT_24;
...
LOCAL void DrawCallEndWin(MMI_WIN_ID_T winId)
{
	GUI_LCD_DEV_INFO    devInfo     = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
	GUISTR_STYLE_T      string  	= {0};
	GUISTR_STYLE_T      style  		= {0};
    GUISTR_STATE_T      state       = GUISTR_STATE_ALIGN | GUISTR_STATE_WORDBREAK;
	GUI_RECT_T          txt_rect    = {20, 90, 220, 120};
	GUI_RECT_T          img_rect    = CC_CALL_END_IMG_RECT;
	

	style.align    = ALIGN_HVMIDDLE;
	style.font     = SONG_FONT_24;
	style.font_color = MMI_WHITE_COLOR;
	MMIRES_GetText(TXT_CC_CALLENDED, winId, &string);
	GUISTR_DrawTextToLCDInRect(&devInfo, &txt_rect, &txt_rect, &string, &style, state, GUISTR_TEXT_DIR_AUTO);

	GUIRES_DisplayImg(PNULL, &img_rect, PNULL, winId, IAMGE_CALL_END, &devInfo);
}
...
LOCAL void DrawMoCallingWin(MMICC_DISPINFO_T *contact_info)
{
...
#ifdef ZTE_WATCH
	GUI_RECT_T          volume_state_rect   = CC_VOLUME_STATE_RECT;
    GUI_RECT_T          stateRect   = CC_CALL_STATE_RECT;
	GUI_RECT_T          img_rect    = CC_CALL_HAND_UP_RECT;
#else
	GUI_RECT_T          stateRect   = CC_CALL_STATE_RECT;
#endif
    ...
    if(NULL != contact_info)
    {
        ...
        GUILABEL_SetFont(labelCtrlId, SONG_FONT_20, GUI_RGB2RGB565(0x80,0x80,0x80));
        // 显示挂断图片
     #ifdef ZTE_WATCH
	 	GUIRES_DisplayImg(PNULL, &img_rect, PNULL, winId, IAMGE_DIALLING_REJECT, &devInfo); 
     #else
...
LOCAL void DrawMtCallingWin(MMICC_DISPINFO_T *contact_info)
{
    ...
    MMI_STRING_T        strData    = {0};
    MMI_STRING_T        dispStr    = {0};
	GUI_RECT_T			reject_rect = CC_INCOM_HAND_UP_RECT;
	GUI_RECT_T			acept_rect  = CC_INCOM_ACEPT_RECT;
	...
	#else
        GUIRES_DisplayImg(PNULL, &reject_rect, PNULL, winId, IAMGE_COMMING_REJECT, &devInfo);
        GUIRES_DisplayImg(PNULL, &acept_rect, PNULL, winId, IAMGE_COMMING_ACEPT, &devInfo);
	#endif
#ifndef ZTE_WATCH
...
/*****************************************************************************/
//  Description : handle call end window messages
//  Global resource dependence :
//  Author: fys 10/29
//  Note:
/*****************************************************************************/
                                          ...
                                          )
{
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
			MMK_CreateWinTimer(win_id, 1000, FALSE);
            break;
        }
        case MSG_FULL_PAINT:
        {
			WATCHCOM_DisplayBackground(win_id);
			DrawCallEndWin(win_id);
			break;
		}
		case MSG_TIMER:
		{
		...
		case MSG_TP_PRESS_UP:
		{
			...
			if((point.x > 12 && point.x < 214) && (point.y > 214 && point.y < 254))
			{
				MMK_CloseWin(win_id);
			}
			break;
                                          ...
                                          )
{
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
			...
			call_current_volume = cur_volume;
			cc_call_current_volume = cc_cur_volume;
			SCI_TRACE_LOW("%s: cur_volume = %d, cc_call_current_volume = %d", __FUNCTION__, cur_volume, cc_call_current_volume);
		#endif
            break;
        }
        case MSG_CLOSE_WINDOW:
        {
			...
			//MMIAPICC_SetCallVolume(cc_call_current_volume);
		#endif
            break;
        }
        case MSG_FULL_PAINT:
		...
		case MSG_TP_PRESS_UP:
		{
			...
			point.y = MMK_GET_TP_Y(param);
			#ifdef ZTE_WATCH
			if((point.x > img_rect.left && point.x < img_rect.right) &&
				(point.y > img_rect.top && point.y < img_rect.bottom))
			{
                                          ...
                                          )
{
    ...
    GUI_RECT_T                  timeRect = CC_CALL_TIME_RECT;
    MMI_CTRL_ID_T               timeLabelId = WATCHCC_CALLTIME_LABEL_CTRL_ID;
    LOCAL uint8 last_call_volume;
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
            ...
            last_call_volume = ual_tele_call_get_call_volume();
		#endif
            // 显示通话时长
            ...
            GUILABEL_SetFont(timeLabelId, SONG_FONT_20, GUI_RGB2RGB565(0x80,0x80,0x80));
#ifdef CALL_RECORD_SUPPORT
                                          ...
                                          )
{
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
			...
			call_current_volume = cur_volume;
			cc_call_current_volume = cc_cur_volume;
            if(cc_call_waiting_timer_id == 0 && MMIZDT_Check_Auto_Answer_Call())
            {
                cc_call_waiting_timer_id = MMK_CreateWinTimer( win_id, CC_WATCH_WAITING_CALL_AUTO_ANSWER, FALSE);
            }
		#endif
        ...
        case MSG_CLOSE_WINDOW:
        {
            ...
            if (cc_call_waiting_timer_id != 0)
            {
                MMK_StopTimer(cc_call_waiting_timer_id);
                cc_call_waiting_timer_id = 0;
            }
		...
		#ifdef ZTE_WATCH
			if(GUI_PointIsInRect(tp_point, reject_rect))
			{
				MMICC_ReleaseCurrentCallReq(MN_CAUSE_USER_BUSY);
			}
			else if(GUI_PointIsInRect(tp_point, acept_rect))
			{
				MMICC_AnswerCall();
			}
		#endif
        ...
        case MSG_TIMER:
        {
            ...
            if(cc_call_waiting_timer_id ==  *(uint8*)param && MMIZDT_Get_Auto_Answer_Call_Status() == 1)
            {
                MMK_StopTimer(cc_call_waiting_timer_id);
                cc_call_waiting_timer_id = 0;
                MMICC_AnswerCall();
