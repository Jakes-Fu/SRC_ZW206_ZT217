...
LOCAL BOOLEAN IsHorizontalMotion(GUI_POINT_T press_pos, GUI_POINT_T move_pos)
{
    uint16 offset_x = abs(move_pos.x - press_pos.x);
    uint16 offset_y = abs(move_pos.y - press_pos.y);
    return (offset_x > 5) && (offset_y < 20) && offset_x > offset_y;
}
...
LOCAL BOOLEAN PrepareAutoSlideAfterTpUp(tWatchSlidePage *slide_page)
{
    ...
    int16 offset_Y =slide_page->tp_up_point.y - slide_page->tp_press_point.y;
...
//    int16 slide_page_offset2 = 0;
    ...
    TRACE_APP_SLIDEPAGE("offset = %d offset_Y:%d", offset, offset_Y);
    if (abs(offset) > SLIDE_CHANGE_PAGE_THR && abs(offset_Y) < SLIDE_PAGE_HEIGHT*4/5)
    {
