...
#ifdef ZTE_WATCH
	GUIRES_DisplayImg(PNULL, &rect, PNULL, win_id, IMAGE_ZTE_DROP_PROGRESS, lcd_dev_info);
#else
...
LOCAL void Show_Download_Qrcode()
{
    ...
    sprintf(qrcode_str,APP_DOWNLOAD_URL,yx_DB_Set_Rec.app_domain,g_zdt_phone_imei_1);
    #endif
    GT_Qcode_show(qrcode_str, qrcode_rect.left-5, qrcode_rect.top-5, qrcode_rect.right+10, qrcode_rect.bottom+10);
}
...
LOCAL void Show_Bind_Qrcode()
{
    ...
    GT_Qcode_show(qrcode_str, qrcode_rect.left, qrcode_rect.top, qrcode_rect.right, qrcode_rect.bottom);
}
                                      ...
                                      )
{
    ...
    switch (msg_id)
    {
        ...
        case MSG_GET_FOCUS:
            ...
            if(setup_wizard_status == SETUP_WIZARD_STATUS_INSERT_SIM && ZDT_SIM_Exsit())//在开机向导中提示插入SIM 这时候插卡则跳过
            {
                setup_wizard_status = SETUP_WIZARD_STATUS_DOWNLOAD_APP;
                MMK_SendMsg(win_id,MSG_FULL_PAINT, PNULL);
            }
            ...
            else
            {
                ...
                switch(setup_wizard_status)
                {
                	case SETUP_WIZARD_STATUS_INSERT_SIM:
						{
							GUI_POINT_T point = {SETUP_WIZARD_INSERT_SIM_START_X, SETUP_WIZARD_INSERT_SIM_START_Y};
                    ...
                    case SETUP_WIZARD_STATUS_DOWNLOAD_APP:
                        {
							GUI_POINT_T point = {SETUP_WIZARD_QRCODE_BG_START_X, SETUP_WIZARD_QRCODE_BG_START_Y};
...
{
    MMI_WIN_ID_T win_id = MMISETUP_WIZARD_WIN_ID;
    ...
    win_handle = MMK_CreateWin( (uint32 *)SETUP_WIZRD_WINTAB, PNULL );
    MMK_SetWinRect(win_handle, &win_rect);
}
