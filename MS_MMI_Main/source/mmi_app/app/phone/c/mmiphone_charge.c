...
LOCAL MMI_RESULT_E HandleChangeLowtempInd(void);
...
case CHR_MSG_CHARGE_LOW_TEMPTURE_IND:
    s_charge_status_remain = CHR_MSG_CHARGE_LOW_TEMPTURE_IND;
        MMIDEFAULT_SetBackLight(TRUE);
        // charging over temp
        result = HandleChangeLowtempInd();
        break;
        
case CHR_MSG_CHARGE_HOT_TEMPTURE_IND:
    s_charge_status_remain = CHR_MSG_CHARGE_HOT_TEMPTURE_IND;
        MMIDEFAULT_SetBackLight(TRUE);
        // charging over temp
        result = HandleChangeOvertempInd();
        break;
    ...
    case STARTUP_NORMAL:
        {
            ...
            {
                #ifdef ZTE_WATCH
                if(MMIPHONE_CHARGE_WARNING_CAP_TWENTY == bat_cap)//20提示一次
                #else
                ...
                #ifdef ZTE_WATCH
                else if(MMIPHONE_CHARGE_WARNING_CAP_LOW_POWER >= bat_cap)
                {
                    WatchLAUNCHER_BatteryLow_Enter();
                }
                #endif
...
LOCAL MMI_RESULT_E HandleChargeStartInd(void)
{
    ...
    switch (MMIAPIPHONE_GetStartUpCondition())
    {
        case STARTUP_CHARGE: //关机充电
        {
            
#ifdef ZDT_APP_SUPPORT// wuxx add for W217
            ZDT_SetIsCharge(TRUE);
            if (NULL == MMK_IsOpenWin(MMIZDT_CHARGING_WIN_ID))//bug 2113365
            {
                MMK_CreateTimerCallback(MMIPHONE_CHARGE_STARTUP_DELAY,  WatchLAUNCHER_Charge_Enter, NULL, FALSE);
            }
#else
...
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleChrWarningInd()
{
#ifndef MMI_DUAL_BATTERY_SUPPORT
    uint8               bat_level = 0;
#endif
...
LOCAL MMI_RESULT_E HandleChangeOvertempInd(void)
{
    ...
    case STARTUP_CHARGE:
        if(ZDT_GetIsCharge())
        {
            MMIPUB_OpenAlertFailWin(TXT_CHARGE_OVER_TEMP2);
        }
        else
        {
...
LOCAL MMI_RESULT_E HandleChangeLowtempInd(void)
{
    ...
    case STARTUP_CHARGE:
        if(ZDT_GetIsCharge())
        {
            MMIPUB_OpenAlertFailWin(TXT_CHARGE_LOW_TEMP2);
        }
        else
        {
            MMIPUB_OpenAlertFailWin(TXT_CHARGE_LOW_TEMP);
        }
        break;
