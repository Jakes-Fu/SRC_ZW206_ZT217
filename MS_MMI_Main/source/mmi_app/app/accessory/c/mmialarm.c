...
//  Note:   
/*****************************************************************************/
PUBLIC void StopRingOrVib(BOOLEAN is_close_ring_timer);
...
//  Note:   
/*****************************************************************************/
PUBLIC void StartRingOrVib(void);
...
//  Note:    
/*****************************************************************************/
PUBLIC BOOLEAN CloseDelayAlarm(
                              uint16 event_id, 
...
//  Note:    
/*****************************************************************************/
PUBLIC BOOLEAN SetDelayAlarmTime(
                                uint16 event_id, 
...
//  Note:    
/*****************************************************************************/
PUBLIC void CheckandStartExpiredEvent( void );
...
PUBLIC void MMI_Alarm_CreateAliveWin();
...
PUBLIC BOOLEAN MMIALARM_ModifyOneAlarm( uint16 event_id, MMIACC_SMART_EVENT_FAST_TABLE_T* event_ptr )
{
    ...
    if(Check_Date_Data(event_ptr))
    {
        SCI_TRACE_LOW("error date ");
        return FALSE;
    }
...
#endif            
            StopRingOrVib(TRUE);
            CloseDelayAlarm(s_arrived_event_id, s_arrived_event.event_fast_info, FALSE);//add by fys
...
#endif
            // 设置延时的闹钟
            SetDelayAlarmTime(s_arrived_event_id, s_arrived_event, TRUE);//add by fys
            //            MMK_CloseWin(win_id);
...
#endif
#ifndef LOCAL_ALARM_CLOCK_SUPPORT
        func = (MMIPUB_HANDLE_FUNC )HandleEventAliveWinMsg;
#endif
#ifdef MMI_ETWS_SUPPORT
...
#define ALARM_RING_IMG_RECT {70,92,200,200}
#define BOTTOM_BUTTON_RECT {12,214,240,260}
#else
#define ALARM_RING_IMG_RECT {70,72,200,180}
#define BOTTOM_BUTTON_RECT {12,190,240,236}
#endif
...
LOCAL MMI_RESULT_E HandleAlarmClockWinMsg_FULL_PAINT(MMI_WIN_ID_T win_id)
{
	...
	GUI_RECT_T win_rect = {0};
	GUI_RECT_T img_rect = ALARM_RING_IMG_RECT;
	GUI_RECT_T bottom_img_rect = BOTTOM_BUTTON_RECT;
...

	{
		...
		SCI_TRACE_LOW("HandleAlarmClockWinMsg_FULL_PAINT event_fast_info.hour = %d",s_arrived_event.event_fast_info.hour);
		SCI_TRACE_LOW("HandleAlarmClockWinMsg_FULL_PAINT event_fast_info.minute = %d",s_arrived_event.event_fast_info.minute);
		clock_hour = s_arrived_event.event_fast_info.hour;
		clock_minute = s_arrived_event.event_fast_info.minute;
	...
	GUIRES_DisplayImg(NULL,&img_rect,NULL,win_id,res_alarm_ring,&lcd_dev_info);
	    ...
	    case MSG_TP_PRESS_UP:
		    {
	    		...
	    		point.x = MMK_GET_TP_X(param);
			    point.y = MMK_GET_TP_Y(param);
                if(GUI_PointIsInRect(point, bottom_img_rect))
                {
                    MMK_SendMsg(win_id, MSG_APP_CANCEL, PNULL);
                }
		    }
...
WINDOW_TABLE(MMI_ALARM_ALIVE_WIN_TAB) = 
{
	WIN_ID(WATCHALM_ALIVE_WIN_ID),
...
PUBLIC void MMI_Alarm_CreateAliveWin()
{
    if(MMK_IsOpenWin(WATCHALM_ALIVE_WIN_ID))
    {
        MMK_CloseWin(WATCHALM_ALIVE_WIN_ID);
    }
	MMK_CreateWin( (uint32 *)MMI_ALARM_ALIVE_WIN_TAB,PNULL);
}