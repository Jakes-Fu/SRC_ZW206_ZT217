...
LOCAL void AlarmListDrawList( MMI_WIN_ID_T win_id, MMI_CTRL_ID_T list_ctr_id, uint16 event_num )
{
    ...
    THEMELIST_ITEM_STYLE_T* itemstyle_ptr = THEMELIST_GetStyle(ALARM_CHECK_LIST_STYLE);
	 ...
	 GUI_POINT_T  point = {0};
		...
		uint16   al_num=0;
		 MMI_TEXT_ID_T img_id_list[3] =
    {
        res_aw_alarm_ic_al1,//       TXT_SHORT_IDLE_SUNDAY,
        res_aw_alarm_ic_al2,//       TXT_SHORT_IDLE_MONDAY,
        res_aw_alarm_ic_al3,//      TXT_SHORT_IDLE_TUESDAY,
    ...
    /*for(i=0;i<eventNum;i++){
			al_num=i%3;
			 ...
			 if(i>0)
			 {
			  ...
			  GUIRES_DisplayImg(&point,PNULL,PNULL,win_id, img_id_list[al_num],&lcd_dev_info);
			 
			   point.x+=110;
			  // point.y-=3;
					GUIRES_DisplayImg(&point,PNULL,PNULL,win_id,res_aw_alarm_ic_icon,&lcd_dev_info);
				point.y-=8;
				if(i>0){
...
LOCAL void Alarm_MainList_OPEN_WINDOW( MMI_WIN_ID_T win_id )
{
    ...
    CTRLLIST_SetBgImage(listCtrlId, res_aw_alarm_ic_bg, FALSE);
