 ...
 **--------------------------------------------------------------------------*/
 #ifdef ZTE_WATCH
#define ALARM_REPEAT_LIST_MAX_ITEM            (8)// (7)
#define ALARM_REPEAT_LIST_ITEM_ONCE_INDEX    (0)
//#define ALARM_REPEAT_LIST_ITEM_REPEATE_INDEX    (1)
#define ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX    (1)
#define ALARM_REPEAT_LIST_ITEM_SATURDAY_INDEX  (7)
#else
...
extern PUBLIC ALARM_EDITINFO_TABLE_T       pEditInfoTable;
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
//日期
#ifdef ZTE_WATCH
LOCAL WATCHCOM_LIST_ITEM_STYLE_1STR_CHECK_ST s_alarm_repeat_item_norepeat     = { TXT_ALARM_NOREPEAT };
...
#else
LOCAL WATCHCOM_LIST_ITEM_STYLE_1STR_CHECK_ST s_alarm_repeat_item_norepeat     = { TXT_ALARM_NOREPEAT };
LOCAL WATCHCOM_LIST_ITEM_STYLE_1STR_CHECK_ST s_alarm_repeat_item_everyday     = { TXT_ALARM_EVERYDAY };
...
#else
    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_CHECK,   &s_alarm_repeat_item_norepeat,    PNULL },
...
LOCAL void SetButtonParam(MMI_CTRL_ID_T ctrl_id, GUI_RECT_T rect, MMI_TEXT_ID_T text_id)
{
...
LOCAL MMI_RESULT_E HandleAlarmPressNextBtn( void )
{
    ...
    if( WATCH_ALARM_ADD == pEditWin->type )
        WatchAlarm_RepeatWin_Enter(&repeatWin);
    else
    {
       // MMK_SendMsg(WATCHALM_EDITLIST_WIN_ID, MSG_ALARM_TIME_DATA, (DPARAM)&repeatWin);
       pEditInfoTable.hour = hour;
       pEditInfoTable.minute= minute;
...
LOCAL MMI_RESULT_E HandleAlarmPressRepeatFixBtn( void )
{
    ...
    {
        uint16 index = 0;//ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX;
        pEventTable->fre_mode   = ALM_MODE_EVERYWEEK;
        pEventTable->fre        = 0;
        for( index = 0; index < ALARM_REPEAT_LIST_MAX_ITEM; index++ )
        {
            ...
            {
                if(ALARM_REPEAT_LIST_ITEM_ONCE_INDEX == index)
                {
                    TRACE_APP_ALARM("only alarm once!");
                    pEventTable->fre_mode   = ALM_MODE_ONCE;
                    pEventTable->fre        = 0;
                ...
                {
                    uint16 weekdayIndex = index-ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX;
    ...
    {
        ALARM_EDITINFO_TABLE_T       tempEditInfoTable = {0};
        tempEditInfoTable.fre = pEventTable->fre;
        tempEditInfoTable.fre_mode = pEventTable->fre_mode;
        //MMK_SendMsg(WATCHALM_EDITLIST_WIN_ID, MSG_ALARM_REPEAT_DATA, (DPARAM)&tempEditInfoTable);
        pEditInfoTable.fre = tempEditInfoTable.fre;
        pEditInfoTable.fre_mode= tempEditInfoTable.fre_mode;
    }
...
LOCAL void AlarmEdit_OPEN_WINDOW( MMI_WIN_ID_T win_id )
{
    ...
    activeFont.color = MMI_CUSTOM_BLUE_COLOR;//MMI_DARK_RED_COLOR;//MMI_WHITE_COLOR  MMI_RED_COLOR
    ...
    else
    {
        ...
        timeValue.hour = pEditInfoTable.hour;
        timeValue.min  = pEditInfoTable.minute;
    }

    GUIEDIT_SetTime( editCtrId, timeValue.hour, timeValue.min, 0 );
    MMK_SetAtvCtrl( win_id, editCtrId );

    SetButtonParam(nextBtn,btnRect,STXT_OK);
}
...
LOCAL void AlarmRepeat_OPEN_WINDOW( MMI_WIN_ID_T win_id )
{
    ...
    {
        //pEventTable = MMIALM_GeEvent( pWinData->eventId );
        if( ALM_MODE_EVERYWEEK == pEditInfoTable.fre_mode )
        {
            uint16 index = ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX;   // sunday

            for( index = ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX; index < ALM_MODE_WEEK_NUM; index++ )
            {
                if( s_adult_alarm_repeat_week_mask[index-ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX] & pEditInfoTable.fre )
                {
        ...
        else  if( ALM_MODE_ONCE == pEditInfoTable.fre_mode )
        {
            GUILIST_SetSelectedItem( listCtrlId, 0, TRUE );
        }
        else
        {
    ...
    SetButtonParam(fixedBtn,btnRect,STXT_OK);
...
LOCAL void AlarmRepeat_List_Select(MMI_WIN_ID_T winId )
{
    ...
    else
    {
        ...
        #ifdef ZTE_WATCH
        if(curIndex == ALARM_REPEAT_LIST_ITEM_ONCE_INDEX)
        {
            int i = ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX;
           
            for(i = ALARM_REPEAT_LIST_ITEM_SUNDAY_INDEX;i<=ALARM_REPEAT_LIST_ITEM_SATURDAY_INDEX;i++)
            {
                reCode = GUILIST_SetSelectedItem( listCtrlId, i, FALSE );
                if( FALSE == reCode )
                {
                    TRACE_APP_ALARM("GUILIST_SetSelectedItem set curIndex TRUE fail");
                }
        ...
        {
            GUILIST_SetSelectedItem( listCtrlId, ALARM_REPEAT_LIST_ITEM_ONCE_INDEX, FALSE );
        }
        #endif
    }
...
#endif
    CREATE_BUTTON_CTRL( IMAGE_COMMON_ONEBTN_KBBG_IMAG, MMIALM_EDIT_ALARM_TIME_NEXT_BTN_CTRL_ID ),
...
WINDOW_TABLE( MMIALARM_REPEAT_WIN_TAB ) =
{
    ...
    CREATE_BUTTON_CTRL( IMAGE_COMMON_ONEBTN_KBBG_IMAG, MMIALM_REPEAT_FIXED_CTRL_ID ),
#ifndef TOUCH_PANEL_SUPPORT
    WIN_SOFTKEY( STXT_OK, TXT_NULL, STXT_RETURN ),
#endif
    END_WIN
