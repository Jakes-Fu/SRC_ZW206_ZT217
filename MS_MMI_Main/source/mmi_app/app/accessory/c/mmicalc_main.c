...
{    
    ...
    point.x = 4;
    point.y = 4;
    GUIRES_DisplayImg(&point,PNULL,PNULL,win_id,res_input_bg,&lcd_dev_info);
    point.x = CALC_BACKSPACE_LEFT;
    point.y = CALC_BACKSPACE_TOP;
    GUIRES_DisplayImg(&point,PNULL,PNULL,win_id,IMAGE_TOOLS_CALCULATOR_DEL,&lcd_dev_info); 
}
...
LOCAL void CalcRefreshContent(uint32 dis_type)
{
    GUI_RECT_T               rect0 = {0,0,MMI_MAINSCREEN_WIDTH,MMI_MAINSCREEN_HEIGHT};
    GUI_RECT_T               rect1 = {0};
    GUI_RECT_T			     rect = {6, 6, CALC_BACKSPACE_LEFT,28};
    uint8				    ch[2] = {0,0};    
    uint8				    chs[5] = {0};
    GUI_LCD_DEV_INFO lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    ...
    GUI_RECT_T input_rect = {6,6,122,28};
    GUI_POINT_T point = {4,4};
    GUI_FillRect(&lcd_dev_info,input_rect,MMI_BLACK_COLOR);
    GUIRES_DisplayImg(&point,PNULL,PNULL,MMIACC_CALC_WIN_ID,res_input_bg,&lcd_dev_info);
    point.x = CALC_BACKSPACE_LEFT;
    point.y = CALC_BACKSPACE_TOP;
    GUIRES_DisplayImg(&point,PNULL,PNULL,MMIACC_CALC_WIN_ID,IMAGE_TOOLS_CALCULATOR_DEL,&lcd_dev_info);
    if(2 == s_CALC_LINE_NUM)
    {
        ...
        if(dis_type & ACC_CALC_DISPLAY_M_SYMBOL)//display the 'M' symbol
        {
            ...
            {            
                ...
                // style
#ifdef MMI_CALCULATOR_MINI_SUPPORT
				SetCalcTextStyle(&text_style, align_type, DP_FONT_14, font_color);//fix bug642342 DP_FONT_14->DP_FONT_12
#else
...
LOCAL void DisplayTpDownImage(MMI_WIN_ID_T win_id,uint8 index)
{
    ...
    if(index > -1 && index < 16)
    {
        ...
        GUIRES_DisplayImg(&dis_point,PNULL,PNULL,win_id,IMAGE_TOOLS_CALCULATOR_TCH,&lcd_dev_info);
    }
}
...
LOCAL void CalcTpUp(MMI_WIN_ID_T win_id,GUI_POINT_T point)
{
    ...
    if(index > -1)
    {
        ...
        {
            msg_id = MSG_KEYDOWN_1+index%4;//3/2/1
        }
        else if(index == 3)
        {
            msg_id = MSG_KEYDOWN_RIGHT;// /
									...
									)
{
   ...
   //  GUIRES_DisplayImg(&point123,PNULL,PNULL,win_id,IMAGE_CALC_BG,&lcd_info_123); 
