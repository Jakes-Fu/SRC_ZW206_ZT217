...
LOCAL WATCHCOM_LIST_ITEM__ST s_sound_call_ring_list[] =
{
//#if defined( ZTE_WATCH)&&defined(ZTE_SUPPORT_240X284)
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_call_ring_item_one,     PNULL },
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_call_ring_item_two,     PNULL },
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_call_ring_item_thr,     PNULL },
//#else
...
{
//#if defined( ZTE_WATCH)&&defined(ZTE_SUPPORT_240X284)
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_note_only_ring,    PNULL },
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_note_only_vibrate, PNULL },
//    { WatchCOM_List_Item_Visible_Default,   GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1,   &s_sound_note_ring_vibrate, PNULL },
...
#define SETTINGS_VIBRATE_PRIVIEW_REPEAT_TIMES           (1)//(20) 选择震动时震动一次
...
PUBLIC void Settings_CallRingSelectWin_Enter( void )
{
    ...
    if (MMK_IsOpenWin(MMISET_CALLRINGSEL_WIN_ID))
    {
        MMK_CloseWin(MMISET_CALLRINGSEL_WIN_ID);
    }
    win_handle = MMK_CreateWin( (uint32 *)MMISET_CALLRINGSEL_WIN_TAB, PNULL );
    if(win_handle != NULL)
...
LOCAL MMI_RESULT_E Handle_NoteTypeSelectWin( MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param )
{
    ...
    switch(msg_id)
    {
        ...
        case MSG_NOTIFY_LIST_SET_SELECT:
        {
            ...
            else if( SETTINGS_SOUND_NOTE_TYPE_VIBRA == curIdx ) //只震动
            {
                //MMISET_RING_TYPE_MAX not play ring
...
PUBLIC void Settings_NoteTypeSelectWin_Enter( void )
{
    ...
    if (MMK_IsOpenWin(MMISET_NOTE_TYPE_SELECT_WIN_ID))
    {
        MMK_CloseWin(MMISET_NOTE_TYPE_SELECT_WIN_ID);
    }
     win_handle = MMK_CreateWin( (uint32 *)MMISET_NOTE_TYPE_SELECT_WIN_TAB, PNULL );
    if(win_handle != NULL)
        MMK_SetWinRect(win_handle, &rect);
}
...
LOCAL uint16 SoundGetNoteTypeSelectedIndex( void )
{
    uint16          index = SETTINGS_SOUND_NOTE_TYPE_RING;//SETTINGS_SOUND_NOTE_TYPE_RING_AND_VIBRA;
    MN_DUAL_SYS_E   dualSys = MMIAPISET_GetActiveSim();
    // use call type as all ring type(call msg alarm)
    uint8           noteType = MMIAPIENVSET_GetActiveModeOptValue( dualSys, CALL_RING_TYPE );
