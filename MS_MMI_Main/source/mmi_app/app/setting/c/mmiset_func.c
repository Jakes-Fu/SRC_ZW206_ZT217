...
//  Note:
/*****************************************************************************/
static uint32 Aging_saveTimes = 0;
static uint8 infoReset = 1;
static void MMIZDT_Read_SaveNV()        //  读恢复出厂需要保存的NV
{
    infoReset = MMIZDT_Get_InfoReset();
    Aging_saveTimes = MMIZDT_Get_Aging_Times();
    infoReset = 1 ;
}

static void MMIZDT_Reset_SaveNV()           // 恢复需要保存的NV
{
    MMIZDT_NV_Set_InfoReset(infoReset); 
    MMIZDT_NV_Set_Aging_Times(Aging_saveTimes);
}
