...
PUBLIC BOOLEAN g_is_zdt_reset_fly_mode = FALSE;
BOOLEAN MMIAPI_ZDT_FlyMode_IsReset(void)
{
    return g_is_zdt_reset_fly_mode;
}

void MMIAPI_ZDT_FlyMode_SetReset(BOOLEAN is_reset)
{
    g_is_zdt_reset_fly_mode = is_reset;
}

BOOLEAN MMIAPI_ZDT_Set_FlyMode(BOOLEAN is_fly_mode_on)
{
    BOOLEAN ret = FALSE;
    uint32  i = 0;
    uint32 dual_sys =MN_DUAL_SYS_1;
    SCI_TRACE_LOW("MMIAPI_ZDT_Set_FlyMode %d",is_fly_mode_on);
    if(FALSE == is_fly_mode_on)
    {
        BOOLEAN is_ps_deactive = TRUE;
        if(!MMIAPIPHONE_GetIsPsDeactiveCnf(dual_sys))
        {
            is_ps_deactive = FALSE;
        }
        SCI_TRACE_LOW("MMIAPI_ZDT_Set_FlyMode is_ps_deactive=%d",is_ps_deactive);
        if (is_ps_deactive)
        {
            MNPHONE_StartupPsEx(MN_DUAL_SYS_1,MN_GMMREG_RAT_GPRS);
            ret = TRUE;
        }
    }
    else
    {
        ...
        SCI_TRACE_LOW("MMIAPI_ZDT_Set_FlyMode is_ps_ready=%d,dual_sys=%d",is_ps_ready,dual_sys);
        if (is_ps_ready)
        {
