...
#endif
    WIN_HIDE_STATUS,
    WIN_STYLE(WS_NO_DEFAULT_STYLE|WS_DISABLE_FLING_CLOSE_WIN),
    END_WIN
};
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:	
        ...
        if (MMISET_CONTRAST_SEVEN < s_setting_brightness_value)
            s_setting_brightness_value = MMISET_CONTRAST_SEVEN;
        WATCHCOM_ProgressBarByIndex(win_id, progressrect, s_setting_brightness_value,MMISET_CONTRAST_SEVEN,res_progress_bg, res_progress_fg, res_progress_hand);
    ...
    case MSG_CLOSE_WINDOW:
#ifdef ZTE_WATCH
        s_setting_brightness_value= MMIAPISET_GetCurrentContrast(); //update LCDbrightness
        MMIAPISET_SetCurrentContrast(s_setting_brightness_value);
        MMIAPISET_UpdateLCDContrast(GUI_MAIN_LCD_ID, s_setting_brightness_value);
#endif
        break;
                                           ...
                                           )
{
	...
	case MSG_OPEN_WINDOW:
        ...
        WATCHCOM_DisplayTitleEx(win_id,TXT_SOUND_CONTROL_ZX,MMI_SPECIAL_TITLE_HEIGHT,MMI_CUSTOM_BLUE_COLOR,ALIGN_HVMIDDLE);
#else
        WATCHCOM_DisplayBackgroundEx(win_id,IMAGE_THEME_BLACK_BG_SPEC);
        WATCHCOM_DisplayTitleEx(win_id,text_volume_settings,MMI_SPECIAL_TITLE_HEIGHT,MMI_WHITE_COLOR,ALIGN_LVMIDDLE);
#endif	
...
#else
        WATCHCOM_DisplayBackgroundEx(win_id,IMAGE_THEME_BLACK_BG_SPEC);
        WATCHCOM_DisplayTitleEx(win_id,text_call_volume_setting,MMI_SPECIAL_TITLE_HEIGHT,MMI_WHITE_COLOR,ALIGN_LVMIDDLE);
                                           ...
                                           )
{
    ...
    GUI_RECT_T progress_volume_touchrect = SET_PROGRESSBAR_TOUCHRECT;
    GUI_RECT_T setok_btn_rect =  WATCH_SETOK_BTN_RECT;
    uint32      cur_item_index = 0;
    uint32  volume_item_total = MMISET_VOL_MAX+1;
    LOCAL GUI_POINT_T down_point = {0};
...
#else
        WATCHCOM_DisplayBackgroundEx(win_id,IMAGE_THEME_BLACK_BG_SPEC);
        WATCHCOM_DisplayTitleEx(win_id,text_ringtone_volume_setting,MMI_SPECIAL_TITLE_HEIGHT,MMI_WHITE_COLOR,ALIGN_LVMIDDLE);
...
#endif
            CTRLBUTTON_SetRunSheen(MMISET_BRIGHTNESS_L_BTN_CTRL_ID,FALSE); //不显示按下光晕
            CTRLBUTTON_SetRunSheen(MMISET_BRIGHTNESS_R_BTN_CTRL_ID,FALSE);
...
PUBLIC void MMIAPISET_ZdtPowerOffWin(void)
{
    ...
    if (MMK_IsOpenWin(MMISET_SET_POWER_RESTART_WIN_ID))
    {
        MMK_CloseWin(MMISET_SET_POWER_RESTART_WIN_ID);
    }
...
PUBLIC void MMIAPISET_CallVolumeWin(void)
{
    ...
    GUI_RECT_T rect = MMITHEME_GetFullScreenRect();
    if (MMK_IsOpenWin(MMISET_SET_CALL_VOLUME_WIN_ID))
    {
        MMK_CloseWin(MMISET_SET_CALL_VOLUME_WIN_ID);
    }
    win_handle = MMK_CreateWin((uint32 *) MMISET_CALL_VOLUME_WIN_TAB, PNULL);
    if(win_handle != NULL)
    MMK_SetWinRect(win_handle, &rect);
}
...
PUBLIC void MMIAPISET_RingToneVolumeWin(void)
{
    MMI_HANDLE_T win_handle = 0;
    GUI_RECT_T rect = MMITHEME_GetFullScreenRect();
    if (MMK_IsOpenWin(MMISET_SET_RINGTONE_VOLUME_WIN_ID))
    {
        MMK_CloseWin(MMISET_SET_RINGTONE_VOLUME_WIN_ID);
    }
    win_handle = MMK_CreateWin((uint32 *) MMISET_RINGTONE_VOLUME_WIN_TAB, PNULL);
    if(win_handle != NULL)
        MMK_SetWinRect(win_handle, &rect);
...
PUBLIC void MMIAPISET_ZdtVolumeWin(void)
{
    MMI_HANDLE_T win_handle =0;
    GUI_RECT_T rect = MMITHEME_GetFullScreenRect();
    if (MMK_IsOpenWin(MMISET_SET_VOLUME_WIN_ID))
    {
        MMK_CloseWin(MMISET_SET_VOLUME_WIN_ID);
    }
    win_handle = MMK_CreateWin((uint32*)MMISET_SET_VOLUME_WIN_TAB,PNULL);
    if(win_handle != NULL)
        MMK_SetWinRect(win_handle, &rect);
}
