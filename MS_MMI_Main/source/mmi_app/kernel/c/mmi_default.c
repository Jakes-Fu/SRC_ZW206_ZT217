...
#ifdef ZTE_WATCH
    MMIZDT_CheckLowBatteryMode();//check 长续航模式状态，如果是低电自动进入长续航模式就自动退出
#endif
#ifdef XYSDK_SUPPORT
{
...
//@tao.xue
         ...
         && !MMIAPICC_IsInState(CC_MT_CONNECTING_STATE)
         //&& !MMIAPICC_IsInState(CC_CALL_CONNECTED_STATE) //展锐patch更新后已解决电流声问题，此处关闭 //added by bao 通话中熄屏会有电流声不熄屏
         //&& !MMIAPICC_IsInState(CC_MO_CONNECTING_STATE) //展锐patch更新后已解决电流声问题，此处关闭 //added by bao 通话中熄屏会有电流声不熄屏
        )
    {
        MMIAPISET_GetBackLightInfo(&backlight_info);
