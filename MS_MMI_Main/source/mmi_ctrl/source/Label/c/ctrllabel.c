...
)
{
    ...
    if (result)
    {
        ...
        //set start y
        if (shrect_height > pic_height + 1 && (TEXT_DIRECTION_BOTTOM != label_ctrl_ptr->text_position || TEXT_DIRECTION_TOP == label_ctrl_ptr->text_position))
        {
...
)
{
    ...
    {
        //get icon width and height
        GUIRES_GetImgWidthHeight (&icon_width, &icon_height, icon_id, label_ctrl_ptr->win_handle);
    ...
    if (result)
    {
        ...
        if(TEXT_DIRECTION_BOTTOM == label_ctrl_ptr->text_position)
        {
            str_rect.left = (int16) str_rect.left-icon_width - label_ctrl_ptr->theme.icon_space;
            str_rect.top = (int16) (str_rect.top + icon_height + label_ctrl_ptr->theme.icon_space);
        }
    }
...
/*****************************************************************************/
// 	Description : set label text postion on icon
//	Global resource dependence :
//  Author: zdt
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLLABEL_SetTextPosition (
    MMI_CTRL_ID_T ctrl_id,    //in
    TEXT_DIRECTION_SET_E     position//in
)
{
    BOOLEAN             result = FALSE;
    CTRLLABEL_OBJ_T		*label_ctrl_ptr = PNULL;
    ...
    if (PNULL != label_ctrl_ptr)
    {
        label_ctrl_ptr->text_position = position;
        result = TRUE;
    }
