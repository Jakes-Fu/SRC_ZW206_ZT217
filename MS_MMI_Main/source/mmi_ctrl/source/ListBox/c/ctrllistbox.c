...
//  Note:
/*****************************************************************************/
PUBLIC CTRLLIST_STYLE_TD_E CtrlList_GetDrawListBgStyle(GUIITEM_STYLE_E style)
{
	if(style == GUIITEM_STYLE_TWO_TEXT_AND_ONE_ICON_MS )	
	{
		return CTRLLIST_STYLE_ALARMCLOCK;
	}
}

/*****************************************************************************/
//  Description : get gpt list style
//  Global resource dependence :
//  Author:fys 2024/05/18
//  Note:
/*****************************************************************************/
#ifdef ZMT_GPT_SUPPORT
LOCAL ZMTLIST_STYLE_TD_E ZMTGPT_GetGPTListStyle(GUIITEM_STYLE_E style)
{
    ZMTLIST_STYLE_TD_E zmt_style = ZMTLIST_STYLE_NONE;
    switch(style)
    {
        case GUIITEM_STYLE_GPT_MINE_TEXT_MS:
            zmt_style = ZMTLIST_STYLE_GPT_MINE;
            break;
        case GUIITEM_STYLE_GPT_AI_TEXT_MS:
            zmt_style = ZMTLIST_STYLE_GPT_AI;
            break;
        case GUIITEM_STYLE_GPT_MINE_2LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_MINE_3LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_MINE_4LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_MINE_5LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_MINE_6LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_MINE_7LINE_TEXT_MS:
            zmt_style = ZMTLIST_STYLE_GPT_MINE_MORE;
            break;
        case GUIITEM_STYLE_GPT_AI_2LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_AI_3LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_AI_4LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_AI_5LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_AI_6LINE_TEXT_MS:
        case GUIITEM_STYLE_GPT_AI_7LINE_TEXT_MS:
            zmt_style = ZMTLIST_STYLE_GPT_AI_MORE;
            break;
        default:
            break;
    }
...
/*****************************************************************************/
//  Description : Draw Bg && Sting
//  Global resource dependence :
//  Author:fys 2024/05/18
//  Note:
/*****************************************************************************/
LOCAL void ZMTGPT_ListDrawBgString(
    CTRLLIST_OBJ_T              *list_ctrl_ptr,
    const GUI_LCD_DEV_INFO      *dev_info_ptr,
    const wchar                 *wstr_ptr,
    uint16                      wstr_len,
...
)
{
    ...
    bg_rect = *disp_rect_ptr;
    text_rect = *disp_rect_ptr;
    str_state = GUISTR_STATE_ALIGN |GUISTR_STATE_WORDBREAK;
    str_style.font = SONG_FONT_16;  
    switch(ZMTGPT_GetGPTListStyle(item_ptr->item.style_id))
    {
        case ZMTLIST_STYLE_GPT_AI:
            {
                bg_color = GUI_RGB2RGB565(255, 255, 255);
                str_style.font_color = GUI_RGB2RGB565(0, 0, 0);
                str_style.align = ALIGN_LVMIDDLE;
                text_rect.left += 5;
                width = GUI_CalculateStringPiexlNum(str_info.wstr_ptr, str_info.wstr_len, str_style.font, 0);
                if(width != 0){
                    bg_rect.right = bg_rect.left + width + 10;
                }
            }
            break;
        case ZMTLIST_STYLE_GPT_MINE:
            {
                bg_color = GUI_RGB2RGB565(0, 255, 0);
                str_style.font_color = GUI_RGB2RGB565(255, 255, 255);
                str_style.align = ALIGN_RVMIDDLE;
                text_rect.right -= 5;
                width = GUI_CalculateStringPiexlNum(str_info.wstr_ptr, str_info.wstr_len, str_style.font, 0);
                if(width != 0){
                    bg_rect.left = bg_rect.right - width - 10;
                }
            }
            break;
        case ZMTLIST_STYLE_GPT_AI_MORE:
            {
                bg_color = GUI_RGB2RGB565(255, 255, 255);
                str_style.font_color = GUI_RGB2RGB565(0, 0, 0);
                str_style.align = ALIGN_RVMIDDLE;
            ...
            break;
        case ZMTLIST_STYLE_GPT_MINE_MORE:
            {
                bg_color = GUI_RGB2RGB565(0, 255, 0);
                str_style.font_color = GUI_RGB2RGB565(255, 255, 255);
                str_style.align = ALIGN_RVMIDDLE;
    ...
    LCD_FillRoundedRect(dev_info_ptr, bg_rect, bg_rect, bg_color);
    LCD_DrawRoundedRect(dev_info_ptr, bg_rect, bg_rect, bg_color);
...
/*#ifdef ZMT_GPT_SUPPORT
    ...
    if(item_ptr->item.style_id == GUIITEM_STYLE_ONE_LINE_TEXT_FACTORY)
    {
        if(item_ptr->item.data_ptr->item_content[content_index].is_custom_font_color){
            str_style.font_color = item_ptr->item.data_ptr->item_content[content_index].custom_font_color;
        }
...
PUBLIC uint16 CTRLLIST_GetLastPageTopIndex(
        MMI_CTRL_ID_T ctrl_id
        )
{
    CTRLLIST_OBJ_T *list_ctrl_ptr = ListGetPtr(ctrl_id);
    ...
    return LIST_GetLastPageItemNum(list_ctrl_ptr);
}
...
LOCAL BOOLEAN isNeedDrawListItemBG(uint32 style_id)
{
    ...
    if( style_id == GUIITEM_STYLE_1STR ||
        ...
        style_id == GUIITEM_STYLE_1ICON_1STR_1LINE_BIG_LAYOUT1 ||
        style_id == GUIITEM_STYLE_1STR_1LINE_BIG_LAYOUT1 ||
        style_id == GUIITEM_STYLE_1STR_1LINE_RADIO_BIG_LAYOUT1 ||
