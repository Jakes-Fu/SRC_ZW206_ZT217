...
#ifdef BAIDU_AI_SUPPORT
// baidu add start
        else if ( APP_DUER_CALL_FUNC == (*signal_ptr)->SignalCode )
        {
        #ifndef WIN32
            MMK_duer_call_func_struct *pParam = (MMK_duer_call_func_struct *)(*signal_ptr);
            // SCI_TRACE_HIGH("MMK_DispatchExtSig---APP_DUER_CALL_FUNC");
            MMK_duer_call_func(pParam);
        #endif
        }
// baidu add end
#endif
                          ...
                          )
{
    ...
    if(signal_ptr->SignalCode == APP_SFR_LWM2M_NOTIFY_MSG)
    {
        SCI_TRACE_LOW("msg is from LWM2M");
        MMISFR_CMCC_ProcessLwm2mMsg(signal_ptr);
        return;
    }
#endif
                                ...
                                )
{
    ...
    {
        if(!MMK_GetWinDisplayStyleState( win_handle, WS_DISABLE_RETURN_WIN) 
