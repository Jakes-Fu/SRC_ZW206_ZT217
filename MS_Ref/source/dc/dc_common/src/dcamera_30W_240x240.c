...
#if defined(DRV_CAMERA_BF20A6_CUSTOM)
//SDR 模式
...
static uint32 Preview_init_config(DC_CAMERA_CFG* param)
{
    ...
    pCfg->hal_cfg.spi_pixels_per_line   = 640;//240;                 //240; //IspOutWidth;
    pCfg->hal_cfg.spi_pixels_per_column = 480;//240;                 //IspOutHeight;
...
static uint32 VideoCall_init_config(DC_CAMERA_CFG* param)
{
...
#if defined(BAIDU_VIDEOCHAT_SUPPORT)&&defined(MAINLCD_DEV_SIZE_240X284)//BAIDU_VIDEO_SIZE 240x284 wuxx 20231202
    pCfg->hal_cfg.rstActiveH            = FALSE;
    pCfg->hal_cfg.pdnActiveH            = TRUE;
    pCfg->hal_cfg.dropFrame             = FALSE;
    pCfg->hal_cfg.camClkDiv             = 42;//g_MclkDiv;
    ...
    pCfg->hal_cfg.cropEnable     = TRUE;
    pCfg->hal_cfg.dstWinColStart = 0;//80;
    pCfg->hal_cfg.dstWinColEnd   = 568;//640-80;
    pCfg->hal_cfg.dstWinRowStart = 0;
    pCfg->hal_cfg.dstWinRowEnd   = 480;
