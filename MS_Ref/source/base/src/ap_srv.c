...
LOCAL uint32 Handle_Ap_Bat_Cap(
    uint8 **dest_ptr,       // Pointer of the response message.
    uint16 *dest_len_ptr,   // Pointer of size of the response message in uin8.
    const uint8 *src_ptr,   // Pointer of the input message. 
    uint16 src_len         // Size of the source buffer in uint8.
...
{    
    uint16 ret = 1;
    uint32 battery_vol = 0;
    uint32 battery_percent = 0;
    CHGMNG_STATE_INFO_T* chg_state_st = NULL;

    SCI_TRACE_LOW("APSRV: Handle_Ap_Bat_Cap\r\n");
    
    // Build response
    *dest_ptr = SCI_ALLOC (sizeof (MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_T) + sizeof(TOOL_DIAG_AP_BAT_CAP_CNF_T));
    if(NULL == *dest_ptr)
    {
        return 0;
    }
    ...
    chg_state_st = CHGMNG_GetModuleState();
    if(chg_state_st != NULL){
        battery_vol = chg_state_st->bat_cur_vol;
        battery_percent = CHGMNG_VoltagetoPercentumVBat(battery_vol);
        ret = 0;
    }    

    SCI_TRACE_LOW("APSRV: bat_remain_cap:%d\r\n", battery_vol);

    *dest_len_ptr = sizeof (MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_T) + sizeof(TOOL_DIAG_AP_BAT_CAP_CNF_T);
    memset(*dest_ptr, 0, *dest_len_ptr);
    memcpy (*dest_ptr,  src_ptr,  sizeof (MSG_HEAD_T));
    ( (MSG_HEAD_T *) (*dest_ptr))->len = *dest_len_ptr;
    ( (TOOLS_DIAG_AP_CNF_T *)( (*dest_ptr)+sizeof(MSG_HEAD_T)))->status = ret;
    ( (TOOLS_DIAG_AP_CNF_T *)( (*dest_ptr)+sizeof(MSG_HEAD_T)))->length = sizeof(TOOL_DIAG_AP_BAT_CAP_CNF_T);
     ( (TOOL_DIAG_AP_BAT_CAP_CNF_T *) ((*dest_ptr)+sizeof(MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_T)))->bat_cap = battery_percent;
     ( (TOOL_DIAG_AP_BAT_CAP_CNF_T *) ((*dest_ptr)+sizeof(MSG_HEAD_T) + sizeof(TOOLS_DIAG_AP_CNF_T)))->bat_v = battery_vol;
...
PUBLIC void Register_ApTest_CmdRoutine (void)
{
    ...
    DIAG_RegisterApCallback( DIAG_AP_CMD_BAT_CAP, Handle_Ap_Bat_Cap);
