...
#ifdef ZDT_SHIPPING_MODE_CMD
uint8   need_ship_mode_alert = 0;				/*是否提示进入船运模式*/
#endif
...
#ifdef ZDT_SHIPPING_MODE_CMD
//恢复开机正常模式
PUBLIC void REFPARAM_ResetShipModeRefParam(void)
{
    uint8 *imei = NULL;
    uint8 pImeiLen = 0;
    uint16         nv_id_imei = 0;
    ...
    imei = "910575601400468";
    pImeiLen = 15;
    nv_id_imei = (uint16)NV_IMEI3;
...
//判断是否直接进入船运模式
PUBLIC BOOLEAN REFPARAM_CheckShipModeReset(void)
{
    BOOLEAN res = FALSE;
    uint8 pImei[16] = {0};
    uint16         nv_id_imei = 0;
    ...
    if (NVERR_NONE != EFS_NvitemRead(NV_IMEI3, sizeof(MN_IMEI_T), imei_arr))
    {
        return FALSE;
    }
    IMEITransformBcdToStr((uint8*)imei_arr, sizeof(imei_arr), pImei);
    if(0 == strncmp(pImei,"111111111111111",15))
    {
        res = TRUE;
    }
    return res;
...
#ifdef ZDT_SHIPPING_MODE_CMD
    if(REFPARAM_CheckShipModeReset())
    {
        need_ship_mode_alert = 1;
    }
#endif
