...
typedef enum  _DEVICE_AUTOTEST_ID_E
{
    ...
    DEVICE_AUTOTEST_SPEAKMIC= 34,//
#ifdef ZDT_SHIPPING_MODE_CMD
    DEVICE_AUTOTEST_SHIPPING_MODE= 35,//船运模式
#endif
...
LOCAL uint32 Handle_SpeakerMicAutotest(
    uint8 **dest_ptr,       // Pointer of the response message.
    uint16 *dest_len_ptr,   // Pointer of size of the response message in uin8.
    const uint8 *src_ptr,   // Pointer of the input message.
    uint16 src_len         // Size of the source buffer in uint8.
...
#ifdef ZDT_SHIPPING_MODE_CMD
LOCAL uint32 Handle_ShipModeAutotest(
    uint8 **dest_ptr,       // Pointer of the response message.
    uint16 *dest_len_ptr,   // Pointer of size of the response message in uin8.
    const uint8 *src_ptr,   // Pointer of the input message.
    uint16 src_len         // Size of the source buffer in uint8.
...
PUBLIC void Register_DeviceAutoTest_CmdRoutine (void)
{
    ...
    DIAG_RegisterAutotestCallback( DEVICE_AUTOTEST_SPEAKMIC, Handle_SpeakerMicAutotest);
#ifdef ZDT_SHIPPING_MODE_CMD
    DIAG_RegisterAutotestCallback( DEVICE_AUTOTEST_SHIPPING_MODE, Handle_ShipModeAutotest);
#endif
...
typedef struct _DIAG_MSG_AUDIOTEST_T
{
    MSG_HEAD_T   msg_head;
    uint8        cmmand;
    uint8        data;
}DIAG_MSG_AUDIOTEST_T;
 ...
 )
{
    ...
    uint8 *micbuff_ptr = 0;
    MSG_HEAD_T *msg_head = (MSG_HEAD_T *)src_ptr;
    DIAG_MSG_AUDIOTEST_T  *MSG_HEAD = (DIAG_MSG_AUDIOTEST_T *)src_ptr;

    pcm_len = (uint32)(msg_head->len - sizeof(DIAG_MSG_AUDIOTEST_T) + 1);

    switch(MSG_HEAD->cmmand)
    {
        case 0x1:
            // 播放初始化
            pcm_len = *(uint32 *) (src_ptr+sizeof(DIAG_MSG_AUDIOTEST_T));
            SCI_TRACE_LOW("Autotest_SpeakRecDataInit 01 pcm_len = %d",pcm_len);
            Autotest_SpeakRecDataInit(pcm_len);
            ret = 0x1;
            *dest_len_ptr = sizeof(MSG_HEAD_T);
            *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
            SCI_ASSERT(SCI_NULL != *dest_ptr);
        ...
        case 0x02:
            // 发送播放音频数据
            pcm_len = * (uint32 *) (src_ptr+sizeof(DIAG_MSG_AUDIOTEST_T));
            SCI_TRACE_LOW("Autotest_SpeakRcvData 02 pcm_len = %d",pcm_len);
            if(0 == Autotest_SpeakRcvData(src_ptr + sizeof(DIAG_MSG_AUDIOTEST_T)+4, pcm_len))
            {
                ret = 0x1;
            }
        ...
        case 0x03:
            if(0 == Autotest_SpeakStart(MSG_HEAD->data)) {
            ret = 0x1;
            }
            *dest_len_ptr = sizeof(MSG_HEAD_T);
            *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
...
#if 1
        case 0x05:
            Autotest_MicStart(MSG_HEAD->data);
            ret = 0x1;
            *dest_len_ptr = sizeof(MSG_HEAD_T);
            *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
            SCI_ASSERT(SCI_NULL != *dest_ptr);
        ...
        case 0x06:
            ...
            *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
            ...
            SCI_MEMCPY(*dest_ptr, MSG_HEAD, sizeof(MSG_HEAD_T));
            SCI_MEMCPY(*dest_ptr+sizeof(MSG_HEAD_T), &pcm_len, 4);
            ((MSG_HEAD_T  *)(*dest_ptr))->len = *dest_len_ptr;
            ((MSG_HEAD_T  *)(*dest_ptr))->subtype  = ret;
        break;
        ...
        case 0x07:
            pcm_len = *(uint32 *) (src_ptr+sizeof(DIAG_MSG_AUDIOTEST_T));
            micbuff_ptr = Autotest_MicGetData(pcm_len);
            
            ret = 0x1;
            if(micbuff_ptr != 0)
            {
                *dest_len_ptr = sizeof(MSG_HEAD_T) + 1024;
                *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
                SCI_ASSERT(SCI_NULL != *dest_ptr);
                //SCI_ASSERT(SCI_NULL != micbuff_ptr);
                SCI_MEMCPY(*dest_ptr, MSG_HEAD, sizeof(MSG_HEAD_T));
                SCI_MEMCPY(*dest_ptr+sizeof(MSG_HEAD_T), micbuff_ptr, 1024);
                ((MSG_HEAD_T  *)(*dest_ptr))->len = *dest_len_ptr;
                ((MSG_HEAD_T  *)(*dest_ptr))->subtype  = ret;
            }
        break;
...
#ifdef ZDT_SHIPPING_MODE_CMD
typedef struct _DIAG_MSG_SHIPTEST_T
{
    MSG_HEAD_T   msg_head;
    uint8        cmmand;
    uint8        data;
 ...
 )
{
    ...
    uint8 *micbuff_ptr = 0;
    MSG_HEAD_T *msg_head = (MSG_HEAD_T *)src_ptr;
    DIAG_MSG_SHIPTEST_T  *MSG_HEAD = (DIAG_MSG_SHIPTEST_T *)src_ptr;
    
    switch(MSG_HEAD->cmmand)
    {
        case 0x1:
                // 恢复出厂默认值并进入船运模式
            ZDT_open_shipping_mode_factory();
            ret = 0x1;
            *dest_len_ptr = sizeof(MSG_HEAD_T);
            *dest_ptr = (uint8 *)SCI_ALLOC_APP(*dest_len_ptr);
            SCI_ASSERT(SCI_NULL != *dest_ptr);
