...
LOCAL void AUDDEV_Downlink_WriteData(const uint16 *puiSrcData, uint32 uiCount, uint32 *puiDataLen)
{
    ...
    memcpy(dac_buffer, puiSrcData, uiCount);
#else
...
#ifdef BAIDU_VIDEOCHAT_SUPPORT
    voice_process_set_param(CUR_MODE, AUDIO_MODE_APP_TYPE_CUR_MODE, AUDIO_DEVICE_MODE_HANDFREE, AUDIO_VOLUME_CUR);
#else
...
#ifdef VOICE_PROCESS_NEW
#ifdef BAIDU_VIDEOCHAT_SUPPORT
    voice_process_set_param(CUR_MODE, AUDIO_MODE_APP_TYPE_CUR_MODE, AUDIO_DEVICE_MODE_HANDFREE, uiVol);
#else
...
#ifdef BAIDU_VIDEOCHAT_SUPPORT
    voice_samplerate = VoiceProcess_get_samplerate();
    uiSampleRate = voice_samplerate > 0 ? voice_samplerate : uiSampleRate;
#else
